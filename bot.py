...
try:
    from Akbots.widevine_bootstrap import ensure_widevine_device
except ImportError:
    ensure_widevine_device = None
...
class Bot(Client):
    ...
    async def start(self, **kwargs):
        ...
        if ensure_widevine_device and not getattr(self, "_widevine_started", False):
            try:
                ensure_widevine_device()
                self._widevine_started = True
            except Exception as e:
                logger.warning(f"widevine device bootstrap failed: {e}")
        ...
        while True:
            try:
                ...
                if "database is locked" in err_text:
                    ...
                    logger.error(f"Critical Startup Error: {type(e).__name__}: {e}", exc_info=True)
