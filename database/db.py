...
class Database:
    ...
    def __init__(self, uri, database_name):
        ...
        self.ui_cache_col = self.db.ui_cache  # generic short-lived UI-state cache — see set_ui_cache()/get_ui_cache() below
        ...
        self.fwd_dupe_col = self.db.fwd_dupes  # Akbots/forward.py "Skip Duplicate" — per-user forwarded file_unique_ids
        self.group_warns_col = self.db.group_warns  # Akbots/anticopyright_guard.py — per (chat_id, user_id) strike counts
        self.gemini_history_col = self.db.gemini_history  # Akbots/gemini_chat.py — per-user /gemini conversation turns
        self.groq_history_col = self.db.groq_history  # Akbots/groq_chat.py — per-user /groq conversation turns + chosen model
        self.gpt_history_col = self.db.gpt_history  # Akbots/openai_chat.py — per-user /gpt conversation turns + chosen model/mode
        self.bypass_stats_col = self.db.bypass_stats  # Akbots/nova_bypasser/ — per-domain "which method worked last" (lightweight learning)
    ...
    _ENC_DEFAULTS = {
        ...
        'enc_notify': True,            # False = send the finished encode silently (no push notification)
        'enc_autodel': False,          # delete the original replied-to message after a successful encode
    }
    ...
    # Uptime Monitor (ported from UptimeRobot-Shanks) — per-user watched
    # URLs, stored as {url: {status, response_time, response_status,
    # down_since}} on the same user document RSS feeds use above.
    async def add_uptime_url(self, id, url):
        user = await self.col.find_one({'id': int(id)})
        urls = (user or {}).get('uptime_urls', {}) or {}
        if url in urls:
            return False
        urls[url] = {'status': False, 'response_time': 0, 'response_status': 0, 'down_since': None}
        await self.col.update_one({'id': int(id)}, {'$set': {'uptime_urls': urls}}, upsert=True)
        return True

    async def get_uptime_urls(self, id):
        user = await self.col.find_one({'id': int(id)})
        return (user or {}).get('uptime_urls', {}) or {}

    async def remove_uptime_url(self, id, url):
        urls = await self.get_uptime_urls(id)
        if url in urls:
            del urls[url]
            await self.col.update_one({'id': int(id)}, {'$set': {'uptime_urls': urls}})
            return True
        return False

    async def remove_all_uptime_urls(self, id):
        await self.col.update_one({'id': int(id)}, {'$set': {'uptime_urls': {}}})

    async def set_uptime_status(self, id, url, **fields):
        urls = await self.get_uptime_urls(id)
        if url not in urls:
            return
        urls[url].update(fields)
        await self.col.update_one({'id': int(id)}, {'$set': {'uptime_urls': urls}})

    async def get_all_uptime_users(self):
        return self.col.find({'uptime_urls': {'$exists': True, '$ne': {}}})

    async def replace_uptime_urls(self, id, urls: dict):
        """Bulk-replace a user's whole uptime_urls map in one write — used
        by the concurrent checker so per-URL updates for the same user
        can't race each other (see Akbots/uptime_monitor.py)."""
        await self.col.update_one({'id': int(id)}, {'$set': {'uptime_urls': urls}})
    ...
    # --------------------------------------------------------
    # Audio Track Selector (Akbots/audio_select.py) — its own daily
    # counter, separate from the generic check_limit/add_traffic above,
    # so this feature's 15-free / 30-premium quota doesn't eat into (or
    # get eaten into by) other features' 10/day limit.
    # --------------------------------------------------------
    async def check_audio_select_limit(self, id):
        """Returns True if BLOCKED (daily limit reached), False if ALLOWED."""
        user = await self.col.find_one({'id': int(id)})
        if not user:
            return False
        limit = 30 if user.get('is_premium') else 15
        now = datetime.datetime.now()
        reset_time = user.get('audio_select_reset_time')
        if reset_time is None or now >= reset_time:
            await self.col.update_one(
                {'id': int(id)},
                {'$set': {'audio_select_usage': 0, 'audio_select_reset_time': None}}
            )
            return False
        return user.get('audio_select_usage', 0) >= limit

    async def add_audio_select_usage(self, id):
        user = await self.col.find_one({'id': int(id)})
        now = datetime.datetime.now()
        reset_time = user.get('audio_select_reset_time') if user else None
        if reset_time is None:
            new_reset_time = now + datetime.timedelta(hours=24)
            await self.col.update_one(
                {'id': int(id)},
                {'$set': {'audio_select_usage': 1, 'audio_select_reset_time': new_reset_time}}
            )
        else:
            await self.col.update_one({'id': int(id)}, {'$inc': {'audio_select_usage': 1}})

    async def get_audio_select_usage(self, id):
        user = await self.col.find_one({'id': int(id)})
        return (user or {}).get('audio_select_usage', 0)

    async def get_audio_select_limit(self, id):
        user = await self.col.find_one({'id': int(id)})
        return 30 if (user and user.get('is_premium')) else 15

    # Default output filename for Akbots/audio_select.py — caption reuses
    # the existing global /set_caption (db.get_caption) so there's only
    # one caption setting across the whole bot instead of a duplicate one.
    async def set_audio_select_name(self, id, filename):
        await self.col.update_one({'id': int(id)}, {'$set': {'audio_select_name': filename}})

    async def get_audio_select_name(self, id):
        user = await self.col.find_one({'id': int(id)})
        return (user or {}).get('audio_select_name', None)
    ...
    # --------------------------------------------------------
    # Generic short-lived UI-state cache (Akbots/hdhub.py's listing/
    # link-list button data, and anything else that needs "remember this
    # small blob for a while, keyed by a short id, surviving a restart").
    #
    # This exists specifically so plugins DON'T reach for a local SQLite
    # file for this: on Render/Railway/Heroku/most container hosts the
    # local filesystem is wiped on every redeploy (and often on restart),
    # so a same-process-only cache with a "durable" SQLite fallback isn't
    # actually durable in practice — it just fails less obviously. This
    # collection lives in the same MongoDB cluster as everything else in
    # this file, which genuinely does survive a redeploy.
    # --------------------------------------------------------
    async def set_ui_cache(self, key: str, value: str, ttl_seconds: int = 86400):
        expires_at = time.time() + ttl_seconds
        await self.ui_cache_col.update_one(
            {'key': key}, {'$set': {'value': value, 'expires_at': expires_at}}, upsert=True
        )

    async def get_ui_cache(self, key: str):
        doc = await self.ui_cache_col.find_one({'key': key})
        if not doc or doc.get('expires_at', 0) < time.time():
            return None
        return doc.get('value')

    async def delete_ui_cache(self, key: str):
        await self.ui_cache_col.delete_one({'key': key})
    ...
    async def get_fwd_settings(self, id):
        ...
        return {
            ...
            'keywords': user.get('fwd_keywords', []),          # allowlist: only forward if filename matches one
            'ext_block': user.get('fwd_ext_block', []),         # blocklist: skip these file extensions
            'size_limit_mb': user.get('fwd_size_limit_mb', 0),  # 0 = no limit; skip media above this size
            'skip_duplicate': user.get('fwd_skip_duplicate', False),
            'tag': user.get('fwd_tag', False),                  # True = real forward (keeps "Forwarded from"), no caption/button possible
            'poll': user.get('fwd_poll', False),                # ported from FTM Forward Bot's Extra Settings screen
            'secure_message': user.get('fwd_secure_message', False),
        }
    ...
    # forward" — only files whose name matches ONE of these get through;
    # empty list = no restriction. Distinct from fwd_filters' kw: token,
    # which is an exclusion/skip rule, not an allowlist.) --
    async def add_fwd_keyword(self, id, word):
        await self.col.update_one({'id': int(id)}, {'$addToSet': {'fwd_keywords': word.lower()}}, upsert=True)

    async def clear_fwd_keywords(self, id):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_keywords': []}})

    # -- Extension blocklist ("Files with these extensions will not
    # forward") — dot-prefixed, e.g. ".exe". --
    async def add_fwd_ext_block(self, id, ext):
        await self.col.update_one({'id': int(id)}, {'$addToSet': {'fwd_ext_block': ext.lower()}}, upsert=True)

    async def clear_fwd_ext_block(self, id):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_ext_block': []}})

    async def set_fwd_size_limit(self, id, mb: int):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_size_limit_mb': max(0, int(mb))}}, upsert=True)

    async def set_fwd_skip_duplicate(self, id, on: bool):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_skip_duplicate': bool(on)}}, upsert=True)

    async def set_fwd_tag(self, id, on: bool):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_tag': bool(on)}}, upsert=True)

    async def set_fwd_poll(self, id, on: bool):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_poll': bool(on)}}, upsert=True)

    async def set_fwd_secure_message(self, id, on: bool):
        await self.col.update_one({'id': int(id)}, {'$set': {'fwd_secure_message': bool(on)}}, upsert=True)

    # -- Skip Duplicate storage: one doc per (user, file_unique_id) ever
    # forwarded. file_unique_id is Telegram's content hash — stable across
    # re-uploads/resends of the same underlying file, unlike file_id. --
    async def is_fwd_duplicate(self, id, file_unique_id: str) -> bool:
        return await self.fwd_dupe_col.find_one({'uid': int(id), 'fuid': file_unique_id}) is not None

    async def record_fwd_duplicate(self, id, file_unique_id: str):
        await self.fwd_dupe_col.update_one(
            {'uid': int(id), 'fuid': file_unique_id},
            {'$set': {'uid': int(id), 'fuid': file_unique_id, 'ts': time.time()}},
            upsert=True
        )

    async def clear_fwd_duplicates(self, id):
        await self.fwd_dupe_col.delete_many({'uid': int(id)})

    async def reset_fwd_settings(self, id):
        """Clears every forward-related field for /reset: source, target,
        progress, caption, button, filters, keywords, extension blocklist,
        size limit, skip-duplicate, and tag. Deliberately leaves the login
        ...
        await self.col.update_one({'id': int(id)}, {'$unset': {
            ...
            'fwd_keywords': "", 'fwd_ext_block': "", 'fwd_size_limit_mb': "",
            'fwd_skip_duplicate': "", 'fwd_tag': "",
            'fwd_poll': "", 'fwd_secure_message': "",
        }})
        await self.clear_fwd_duplicates(id)
    ...
    async def add_titanium_bot(self, id, token, username, source="manual", bot_id=None):
        now = datetime.datetime.utcnow()
        entry = {
            "token": token, "username": username, "source": source, "bot_id": bot_id,
            "added_at": now, "last_used": 0,
            "status": "active", "last_validated": now,
        }
        await self.col.update_one({'id': int(id)}, {'$push': {'titanium_bots': entry}})

    async def touch_titanium_bot_validated(self, id, token):
        await self.col.update_one(
            {'id': int(id), 'titanium_bots.token': token},
            {'$set': {'titanium_bots.$.last_validated': datetime.datetime.utcnow(), 'titanium_bots.$.status': 'active'}}
        )

    # --------------------------------------------------------
    # Anti-piracy group guard strikes (Akbots/anticopyright_guard.py —
    # ported from COPYRIGHT2's helper/warnings.py, rewritten on motor
    # instead of blocking pymongo since the rest of Akbotz is async)
    # --------------------------------------------------------
    async def get_group_warnings(self, chat_id, user_id):
        data = await self.group_warns_col.find_one({"chat_id": int(chat_id), "user_id": int(user_id)})
        return data["warnings"] if data else 0

    async def add_group_warning(self, chat_id, user_id):
        result = await self.group_warns_col.find_one_and_update(
            {"chat_id": int(chat_id), "user_id": int(user_id)},
            {"$inc": {"warnings": 1}, "$set": {"last_warn": datetime.datetime.utcnow()}},
            upsert=True, return_document=True,
        )
        return result["warnings"]

    async def reset_group_warnings(self, chat_id, user_id):
        await self.group_warns_col.delete_one({"chat_id": int(chat_id), "user_id": int(user_id)})

    # --------------------------------------------------------
    # Download History & Favourites (/history, /fav, /favourites —
    # Akbots/history_favs.py). History keeps only the most recent 50
    # entries per user via $slice so the array can't grow unbounded.
    # --------------------------------------------------------
    async def add_download_history(self, id, entry):
        entry = dict(entry)
        entry["at"] = datetime.datetime.utcnow()
        await self.col.update_one(
            {'id': int(id)},
            {'$push': {'download_history': {'$each': [entry], '$position': 0, '$slice': 50}}},
            upsert=True,
        )

    async def get_download_history(self, id, limit=15):
        user = await self.col.find_one({'id': int(id)}) or {}
        return (user.get('download_history') or [])[:limit]

    async def add_favourite(self, id, url, title=None):
        user = await self.col.find_one({'id': int(id)}) or {}
        if any(f.get("url") == url for f in user.get("favourites", [])):
            return False
        entry = {"url": url, "title": title or url, "added_at": datetime.datetime.utcnow()}
        await self.col.update_one({'id': int(id)}, {'$push': {'favourites': entry}}, upsert=True)
        return True

    async def get_favourites(self, id):
        user = await self.col.find_one({'id': int(id)}) or {}
        return user.get('favourites', [])

    async def remove_favourite(self, id, url):
        result = await self.col.update_one(
            {'id': int(id)}, {'$pull': {'favourites': {'url': url}}}
        )
        return result.modified_count > 0

    async def remove_favourite_by_index(self, id, index):
        favs = await self.get_favourites(id)
        if index < 1 or index > len(favs):
            return None
        target = favs[index - 1]
        await self.remove_favourite(id, target["url"])
        return target
    ...
    # --------------------------------------------------------
    # Akbots/nova_bypasser/ — lightweight "learning": remember which
    # method last worked for a given domain, so future bypasses for that
    # domain try that method first instead of walking the full ladder
    # (direct extraction → cloudflare → generic → browser → AI) every time.
    # --------------------------------------------------------
    async def record_bypass_result(self, domain: str, method: str, success: bool):
        await self.bypass_stats_col.update_one(
            {'domain': domain},
            {
                '$set': {'last_method': method if success else None, 'last_success': success, 'updated_at': time.time()},
                '$inc': {f'attempts.{method}.tries': 1, f'attempts.{method}.wins': 1 if success else 0},
            },
            upsert=True,
        )

    async def get_best_bypass_method(self, domain: str):
        """Returns the method name most likely to work for this domain
        (highest win-rate with at least 2 tries), or None if we don't
        have enough history yet."""
        doc = await self.bypass_stats_col.find_one({'domain': domain})
        if not doc or not doc.get('attempts'):
            return None
        best_method, best_rate = None, 0.0
        for method, stats in doc['attempts'].items():
            tries = stats.get('tries', 0)
            if tries < 2:
                continue
            rate = stats.get('wins', 0) / tries
            if rate > best_rate:
                best_method, best_rate = method, rate
        return best_method if best_rate >= 0.5 else None
...
# ============================================================
# Maintenance Mode (Akbots/maintenance.py) — global on/off switch that,
# when enabled, makes the bot reply "under maintenance" to every
# non-admin message/callback instead of running any other plugin.
# ============================================================
async def _maint_get_mode(self) -> bool:
    doc = await self.db.settings.find_one({'_id': 'maintenance_mode'})
    return bool(doc.get('enabled', False)) if doc else False


async def _maint_set_mode(self, enabled: bool):
    await self.db.settings.update_one(
        {'_id': 'maintenance_mode'}, {'$set': {'enabled': bool(enabled)}}, upsert=True
    )


Database.get_maintenance_mode = _maint_get_mode
Database.set_maintenance_mode = _maint_set_mode


# ============================================================
# Stream/download link expiry (Akbots/set_expiry.py) — lets an admin
# change how long generated Stream/Download links stay valid at runtime,
# via /set_expiry, instead of only through the STREAM_LINK_EXPIRY env var.
# A value stored here (including explicit 0 = never expires) overrides
# the env var/config.py default; if nothing's ever been set, callers fall
# back to config.STREAM_LINK_EXPIRY.
# ============================================================
async def _expiry_get_seconds(self):
    """Returns the stored override in seconds, or None if never set (in
    which case the caller should fall back to config.STREAM_LINK_EXPIRY)."""
    doc = await self.db.settings.find_one({'_id': 'stream_link_expiry'})
    if doc is None or 'seconds' not in doc:
        return None
    return int(doc['seconds'])


async def _expiry_set_seconds(self, seconds: int):
    await self.db.settings.update_one(
        {'_id': 'stream_link_expiry'}, {'$set': {'seconds': int(seconds)}}, upsert=True
    )


async def _expiry_clear_override(self):
    """Removes the override entirely so get_link_expiry_seconds() goes
    back to returning None (i.e. callers fall back to the env default)."""
    await self.db.settings.delete_one({'_id': 'stream_link_expiry'})


Database.get_link_expiry_seconds = _expiry_get_seconds
Database.set_link_expiry_seconds = _expiry_set_seconds
Database.clear_link_expiry_override = _expiry_clear_override


# ============================================================
# File-to-Link persistent link storage (Akbots/filetolink/) — backs
# STREAM_LINK_EXPIRY so expiry survives a bot restart, instead of the
# in-memory-only tracking the first cut of this feature shipped with.
# ============================================================
async def _ftl_save_link(self, msg_id: int, file_name: str, hash_str: str, file_size: int = 0):
    await self.db.stream_links.update_one(
        {'_id': int(msg_id)},
        {'$set': {
            'file_name': file_name,
            'hash': hash_str,
...
async def _ftl_get_link_timestamp(self, msg_id: int):
    doc = await self.db.stream_links.find_one({'_id': int(msg_id)})
    return doc.get('timestamp') if doc else None


async def _ftl_ensure_indexes(self):
    try:
        await self.db.stream_links.create_index("timestamp")
    except Exception:
        pass


Database.save_stream_link = _ftl_save_link
Database.get_stream_link_timestamp = _ftl_get_link_timestamp
Database.ensure_stream_link_indexes = _ftl_ensure_indexes


# ============================================================
# Storage stats (Akbots/storageinfo.py — /storageinfo) — aggregates the
# stream_links collection (every video the File-to-Link feature has ever
# served a streaming link for) into a total file count + total bytes, so
# /storageinfo doesn't need to page through every document itself.
# ============================================================
async def _storage_get_stream_link_stats(self):
    count = await self.db.stream_links.count_documents({})
    total_size = 0
    pipeline = [{"$group": {"_id": None, "total_size": {"$sum": "$file_size"}}}]
    async for doc in self.db.stream_links.aggregate(pipeline):
        total_size = doc.get("total_size") or 0
    oldest = await self.db.stream_links.find_one(sort=[("timestamp", 1)])
    newest = await self.db.stream_links.find_one(sort=[("timestamp", -1)])
    return {
        "count": count,
        "total_size": total_size,
        "oldest_ts": oldest.get("timestamp") if oldest else None,
        "newest_ts": newest.get("timestamp") if newest else None,
    }


Database.get_stream_link_stats = _storage_get_stream_link_stats


# ============================================================
# GitHub multi-account storage (Akbots/github_accounts.py — /addaccount,
# /accounts, /useaccount) — ported from the "GitHub Auto Uploader Pro"
# CLI tool's config.json account-switching feature. There, one local
# config.json held one desktop user's saved accounts; here every Telegram
# user gets their own set of saved GitHub username/token pairs plus which
# one is "active", keyed by their Telegram user id in a new
# `gh_accounts` collection.
# ============================================================
async def _gh_save_account(self, user_id: int, username: str, token: str):
    """Save (or overwrite) a GitHub account for this Telegram user and mark it active."""
    await self.db.gh_accounts.update_one(
        {"_id": int(user_id)},
        {
            "$set": {
                f"accounts.{username}": {"username": username, "token": token},
...
async def _gh_list_accounts(self, user_id: int):
    """Return (accounts_dict, active_username) for this Telegram user."""
    doc = await self.db.gh_accounts.find_one({"_id": int(user_id)})
    if not doc:
        return {}, None
    return doc.get("accounts", {}), doc.get("active_username")


async def _gh_get_active_account(self, user_id: int):
    """Return (username, token) for this Telegram user's active GitHub account, or (None, None)."""
    accounts, active_username = await self.gh_list_accounts(user_id)
    account = accounts.get(active_username) if active_username else None
    if account and account.get("token"):
        return account["username"], account["token"]
    return None, None


async def _gh_set_active_account(self, user_id: int, username: str) -> bool:
    """Switch this Telegram user's active GitHub account. Returns False if not saved yet."""
    doc = await self.db.gh_accounts.find_one({"_id": int(user_id)})
    if not doc or username not in doc.get("accounts", {}):
        return False
    await self.db.gh_accounts.update_one(
        {"_id": int(user_id)}, {"$set": {"active_username": username}}
    )
    return True


async def _gh_remove_account(self, user_id: int, username: str) -> bool:
    doc = await self.db.gh_accounts.find_one({"_id": int(user_id)})
    if not doc or username not in doc.get("accounts", {}):
        return False
    await self.db.gh_accounts.update_one(
        {"_id": int(user_id)}, {"$unset": {f"accounts.{username}": ""}}
    )
    if doc.get("active_username") == username:
        remaining = [u for u in doc.get("accounts", {}) if u != username]
        new_active = remaining[0] if remaining else None
        await self.db.gh_accounts.update_one(
            {"_id": int(user_id)}, {"$set": {"active_username": new_active}}
        )
    return True


Database.save_gh_account = _gh_save_account
Database.gh_list_accounts = _gh_list_accounts
Database.get_gh_active_account = _gh_get_active_account
Database.set_gh_active_account = _gh_set_active_account
Database.remove_gh_account = _gh_remove_account


# ------------------------------------------------------------
# GitHub upload log — powers /exportgh's "Upload Summary" section.
# Every successful /uploadrepo push appends one entry here.
# ------------------------------------------------------------
async def _gh_log_upload(self, user_id: int, gh_username: str, repo_name: str):
    entry = {
        "gh_username": gh_username, "repo": repo_name,
        "at": datetime.datetime.utcnow(),
    }
    await self.db.gh_accounts.update_one(
        {"_id": int(user_id)},
        {"$push": {"upload_log": {"$each": [entry], "$position": 0, "$slice": 100}}},
        upsert=True,
    )


async def _gh_get_upload_log(self, user_id: int, limit: int = 20):
    doc = await self.db.gh_accounts.find_one({"_id": int(user_id)})
    if not doc:
        return []
    return (doc.get("upload_log") or [])[:limit]


Database.log_gh_upload = _gh_log_upload
Database.get_gh_upload_log = _gh_get_upload_log


# ============================================================
# Bot-wide GitHub / Heroku tokens, settable from inside Telegram (rather
# than only via config.py / environment variables) — /setgittoken in
# Akbots/github_accounts.py and /setherokutoken in Akbots/heroku_tools.py.
# Stored in the same single "global" settings document already used for
# other bot-wide config (see set_anime_channel above), under its own key
# so it doesn't collide with anything else there.
# ============================================================
async def _set_git_token(self, token: str):
    await self.settings_col.update_one(
        {'_id': 'global'}, {'$set': {'git_token': token}}, upsert=True
    )


async def _get_git_token(self):
    doc = await self.settings_col.find_one({'_id': 'global'})
    return (doc or {}).get('git_token')


async def _set_heroku_token(self, token: str):
    await self.settings_col.update_one(
        {'_id': 'global'}, {'$set': {'heroku_token': token}}, upsert=True
    )


async def _get_heroku_token(self):
    doc = await self.settings_col.find_one({'_id': 'global'})
    return (doc or {}).get('heroku_token')


Database.set_git_token = _set_git_token
Database.get_git_token = _get_git_token
Database.set_heroku_token = _set_heroku_token
Database.get_heroku_token = _get_heroku_token


# ============================================================
# Gemini AI chat history (Akbots/gemini_chat.py) — /gemini, /ai, /ask
# remember each user's last GEMINI_HISTORY_TURNS exchanges so follow-up
# questions have context; /resetai, /clearai wipe it. One document per
# user_id, storing a capped list of {role, text} turns.
# ============================================================
GEMINI_HISTORY_TURNS = 12  # exchanges kept (each exchange = 1 user + 1 model turn)


async def _get_gemini_history(self, user_id: int):
    doc = await self.gemini_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('turns', [])


async def _append_gemini_history(self, user_id: int, prompt: str, answer: str):
    doc = await self.gemini_history_col.find_one({'user_id': int(user_id)})
    turns = (doc or {}).get('turns', [])
    turns.append({'role': 'user', 'text': prompt})
    turns.append({'role': 'model', 'text': answer})
    # Keep only the most recent GEMINI_HISTORY_TURNS exchanges (2 entries each)
    turns = turns[-(GEMINI_HISTORY_TURNS * 2):]
    await self.gemini_history_col.update_one(
        {'user_id': int(user_id)}, {'$set': {'turns': turns}}, upsert=True
    )


async def _clear_gemini_history(self, user_id: int):
    await self.gemini_history_col.delete_one({'user_id': int(user_id)})


Database.get_gemini_history = _get_gemini_history
Database.append_gemini_history = _append_gemini_history
Database.clear_gemini_history = _clear_gemini_history


# ============================================================
# Groq AI chat history (Akbots/groq_chat.py) — /groq, /groqmodel, /resetgroq
# Ported from groq-chatbot-main (python-telegram-bot + mongopersistence)
# into this bot's own db.py pattern, mirroring the Gemini history above.
# One document per user_id: capped list of {role, text} turns, plus the
# user's chosen model (defaults to GROQ_MODEL from config if unset).
# ============================================================
GROQ_HISTORY_TURNS = 12  # exchanges kept (each exchange = 1 user + 1 assistant turn)


async def _get_groq_history(self, user_id: int):
    doc = await self.groq_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('turns', [])


async def _append_groq_history(self, user_id: int, prompt: str, answer: str):
    doc = await self.groq_history_col.find_one({'user_id': int(user_id)})
    turns = (doc or {}).get('turns', [])
    turns.append({'role': 'user', 'text': prompt})
    turns.append({'role': 'assistant', 'text': answer})
    # Keep only the most recent GROQ_HISTORY_TURNS exchanges (2 entries each)
    turns = turns[-(GROQ_HISTORY_TURNS * 2):]
    await self.groq_history_col.update_one(
        {'user_id': int(user_id)}, {'$set': {'turns': turns}}, upsert=True
    )


async def _clear_groq_history(self, user_id: int):
    await self.groq_history_col.delete_one({'user_id': int(user_id)})


async def _get_groq_model(self, user_id: int):
    doc = await self.groq_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('model')


async def _set_groq_model(self, user_id: int, model: str):
    await self.groq_history_col.update_one(
        {'user_id': int(user_id)}, {'$set': {'model': model}}, upsert=True
    )


Database.get_groq_history = _get_groq_history
Database.append_groq_history = _append_groq_history
Database.clear_groq_history = _clear_groq_history
Database.get_groq_model = _get_groq_model
Database.set_groq_model = _set_groq_model


# ============================================================
# GPT chat history (Akbots/openai_chat.py) — /gpt, /gptmodel, /gptmode, /resetgpt
# Ported from karfly/chatgpt_telegram_bot's database.py (originally a
# per-dialog Mongo document) into this bot's flatter per-user pattern,
# mirroring the Gemini/Groq history above. One document per user_id:
# capped list of {role, text} turns, chosen model, chosen chat mode
# (persona system prompt).
# ============================================================
GPT_HISTORY_TURNS = 12  # exchanges kept (each exchange = 1 user + 1 assistant turn)


async def _get_gpt_history(self, user_id: int):
    doc = await self.gpt_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('turns', [])


async def _append_gpt_history(self, user_id: int, prompt: str, answer: str):
    doc = await self.gpt_history_col.find_one({'user_id': int(user_id)})
    turns = (doc or {}).get('turns', [])
    turns.append({'role': 'user', 'text': prompt})
    turns.append({'role': 'assistant', 'text': answer})
    # Keep only the most recent GPT_HISTORY_TURNS exchanges (2 entries each)
    turns = turns[-(GPT_HISTORY_TURNS * 2):]
    await self.gpt_history_col.update_one(
        {'user_id': int(user_id)}, {'$set': {'turns': turns}}, upsert=True
    )


async def _clear_gpt_history(self, user_id: int):
    await self.gpt_history_col.delete_one({'user_id': int(user_id)})


async def _get_gpt_model(self, user_id: int):
    doc = await self.gpt_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('model')


async def _set_gpt_model(self, user_id: int, model: str):
    await self.gpt_history_col.update_one(
        {'user_id': int(user_id)}, {'$set': {'model': model}}, upsert=True
    )


async def _get_gpt_mode(self, user_id: int):
    doc = await self.gpt_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('mode')


async def _set_gpt_mode(self, user_id: int, mode: str):
    await self.gpt_history_col.update_one(
        {'user_id': int(user_id)}, {'$set': {'mode': mode}}, upsert=True
    )


Database.get_gpt_history = _get_gpt_history
Database.append_gpt_history = _append_gpt_history
Database.clear_gpt_history = _clear_gpt_history
Database.get_gpt_model = _get_gpt_model
Database.set_gpt_model = _set_gpt_model
Database.get_gpt_mode = _get_gpt_mode
Database.set_gpt_mode = _set_gpt_mode


# GPT usage/cost tracking — ported from chatgpt_telegram_bot's per-user
# n_used_tokens balance (originally in bot/database.py), flattened into
# the same gpt_history_col document instead of a separate dialogs
# collection. Costs are in USD, computed by the caller from each
# provider's per-model pricing before being added here.
async def _add_gpt_usage(self, user_id: int, input_tokens: int, output_tokens: int, cost: float):
    await self.gpt_history_col.update_one(
        {'user_id': int(user_id)},
        {'$inc': {
...
async def _get_gpt_usage(self, user_id: int):
    doc = await self.gpt_history_col.find_one({'user_id': int(user_id)})
    return (doc or {}).get('usage', {'input_tokens': 0, 'output_tokens': 0, 'cost': 0})


async def _get_all_gpt_usage(self):
    """Admin-only aggregate: total cost/tokens across every user who has
    used /gpt, plus a per-user breakdown sorted by spend (highest first)."""
    cursor = self.gpt_history_col.find({'usage': {'$exists': True}})
    ...
    return {'total_cost': total_cost, 'total_input_tokens': total_in,
            'total_output_tokens': total_out, 'users': rows}


Database.add_gpt_usage = _add_gpt_usage
Database.get_gpt_usage = _get_gpt_usage
Database.get_all_gpt_usage = _get_all_gpt_usage
