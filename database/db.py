...
class Database:
    ...
    def __init__(self, uri, database_name):
        ...
        self.terabox_relay_col = self.db.terabox_relay  # Akbots/terabox.py support-bot upload pool records
    ...
    # --------------------------------------------------------
    # TeraBox support-bot upload pool relay records
    # (uid, msg_id in DB_CHANNEL, filename, size, added_at — one doc per
    # file relayed through a support bot into DB_CHANNEL before being
    # copied to the requesting user)
    # --------------------------------------------------------
    async def set_terabox_relay(self, uid, data: dict):
        data = dict(data)
        data['uid'] = uid
        data['added_at'] = datetime.datetime.utcnow()
        await self.terabox_relay_col.update_one(
            {'uid': uid}, {'$set': data}, upsert=True
        )

    async def get_terabox_relay(self, uid):
        return await self.terabox_relay_col.find_one({'uid': uid})
    ...
    async def add_titanium_bot(self, id, token, username, source="manual"):
        entry = {
            "token": token, "username": username, "source": source,
            "added_at": datetime.datetime.utcnow(), "last_used": 0,
        }
