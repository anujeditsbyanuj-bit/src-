...
async function resolveEmbedUrl(url, parentUrl = null) {
  ...
  // 0. Fast-fail for known-dead player.php URLs (PoseidonHD player service is down)
  if (/player\.(poseidonhd|poseidonhd2)/i.test(host) && /player\.php/i.test(pathname)) {
    debugLog("resolveEmbed", "player.php is known-dead, returning null", url);
    return null;
  }
