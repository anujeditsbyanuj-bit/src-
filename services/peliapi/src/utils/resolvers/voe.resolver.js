...
async function extractVoe(pageUrl) {
  ...
  try {
    ...
    // Detectar CAPTCHA/verificación humana - fallar rápido
    if (html.includes('Confirm you') || html.includes('captcha') || html.includes('altcha-widget') || html.includes('gate-panel')) {
      console.log('[VOE RESOLVER] CAPTCHA detectado - no se puede resolver sin navegador');
      return null;
    }
    ...
    if (html.includes('window.location.href')) {
      ...
      if (match && match[1]) {
        ...
        try {
          ...
          // Verificar CAPTCHA en la página redirigida
          if (html.includes('Confirm you') || html.includes('captcha') || html.includes('altcha-widget') || html.includes('gate-panel')) {
            console.log('[VOE RESOLVER] CAPTCHA en página redirigida - no se puede resolver');
            return null;
          }
        } catch (err) {
