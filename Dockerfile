...
#
# PLAYWRIGHT_BROWSERS_PATH is set to a shared, explicit directory (rather
# than the per-user default cache) so patchright below can reuse this same
# downloaded Chromium instead of needing its own separate install — see
...
# for VOE/SerienStream/Cineby/BurningSeries/HanimeTV.
#
# NOT running `patchright install chromium` here (used to, but removed):
# patchright's bundled installer carries an older/stricter OS-support
# check than current playwright's — it hard-fails with "Patchright does
# not support chromium on debian11-x64" even with --with-deps dropped,
# whereas plain playwright above only *warns* about the same unsupported-
# OS situation and proceeds fine. That's a driver-level limitation with
# no install-time flag to skip.
#
# The fix: patchright-python is itself a source-level patch of Microsoft's
# own playwright-python (same author metadata, same PLAYWRIGHT_BROWSERS_PATH
# mechanism for locating browsers at runtime) — only its Python-side CDP/
# stealth behavior differs, not the actual browser binary, which patchright
# explicitly runs unmodified. So instead of installing a second copy via
# patchright's broken installer, this just points it at the Chromium the
# line above already downloaded, via the same PLAYWRIGHT_BROWSERS_PATH set
# above (persists into the runtime image since this is a single-stage
# build) — no second install, and nothing here is actually missing.
...
# commands work with zero manual deploy/config step. `npm install`
# triggers Puppeteer's normal postinstall download of its own Chrome for
# Testing binary same as it would anywhere; the Playwright --with-deps
# install above already pulled in the Debian library set headless
# Chromium needs to actually launch, so no extra OS packages are needed
# here.
#
# `npm install`, not `npm ci` — neither vendored service ships a
# package-lock.json/npm-shrinkwrap.json (their own standalone
# Dockerfiles use `npm install` too, for the same reason), and `npm ci`
# hard-requires one to exist or it fails immediately with EUSAGE before
# installing anything at all. `npm install` resolves straight from
# package.json's version ranges instead, which needs no lockfile.
# ------------------------------------------------------------------
RUN cd services/anime1v-api && npm install --omit=dev
RUN cd services/peliapi && npm install --omit=dev
