...
# Deliberately NOT --with-deps here: patchright's OS-detection doesn't
# recognize this base image's OS (fails hard with "Patchright does not
# support chromium on debian11-x64" — plain playwright above only *warns*
# about unsupported OS and continues, patchright treats it as fatal). The
# actual OS-level shared libraries chromium needs (libatk-bridge2.0-0,
# libxcomposite1, fonts-*, etc.) are the exact same ones the plain
# `playwright install --with-deps chromium` line right above this already
# installed — patchright uses the same underlying Chromium engine, just a
# separate binary cache — so skipping --with-deps here just avoids
# reinstalling those, not actually missing any of them.
