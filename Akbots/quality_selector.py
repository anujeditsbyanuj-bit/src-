...
def _build_quality_keyboard(formats_data: dict, max_buttons: int = 12) -> InlineKeyboardMarkup:
    ...
    # Video + Audio options
    if video_formats:
        buttons.append([make_button("🎬 ᴠɪᴅᴇᴏ ǫᴜᴀʟɪᴛɪᴇs:", callback_data="qual:noop", style=_BS.PRIMARY if _BS else None)])
        for fmt in video_formats:
            size_str = f" ({_format_size(fmt['filesize'])})" if fmt['filesize'] else ""
            label = f"{fmt['label']}{size_str}"
            buttons.append([
                make_button(label, callback_data=f"qual:vid:{fmt['id']}", style=_BS.PRIMARY if _BS else None)
            ])
    
    # Audio only options
    if audio_formats:
        buttons.append([make_button("🎵 ᴀᴜᴅɪᴏ ᴏɴʟʏ:", callback_data="qual:noop", style=_BS.PRIMARY if _BS else None)])
        for fmt in audio_formats:
            size_str = f" ({_format_size(fmt['filesize'])})" if fmt['filesize'] else ""
            label = f"{fmt['label']}{size_str}"
            buttons.append([
                make_button(label, callback_data=f"qual:aud:{fmt['id']}", style=_BS.PRIMARY if _BS else None)
            ])
    
    # Cancel button
    buttons.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data="qual:cancel", style=_BS.DANGER if _BS else None)])
...
def _format_quality_text(formats_data: dict) -> str:
    ...
    if "error" in formats_data:
        return f"{E_CROSS} <b>ᴇʀʀᴏʀ:</b> {formats_data['error']}"
    ...
    text = f"{E_ROCKET} <b>sᴇʟᴇᴄᴛ ǫᴜᴀʟɪᴛʏ</b>\n\n"
    text += f"<b>ᴛɪᴛʟᴇ:</b> <i>{title[:50]}</i>\n"
    text += f"<b>ᴅᴜʀᴀᴛɪᴏɴ:</b> {duration_str}\n"
    text += f"<b>ᴜᴘʟᴏᴀᴅᴇʀ:</b> {uploader}\n"
    text += f"<b>ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀᴍᴀᴛs:</b> {formats_data.get('total_formats', 0)}\n\n"
    text += "<b>ᴛᴀᴘ ᴀ ǫᴜᴀʟɪᴛʏ ʙᴇʟᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ:</b>"
...
async def show_quality_selector(client: Client, message: Message, url: str):
    ...
    status = await message.reply_text(
        f"{E_ROCKET} <b>ᴀɴᴀʟʏᴢɪɴɢ ᴠɪᴅᴇᴏ ғᴏʀᴍᴀᴛs...</b>",
...
async def quality_selector_callback(client: Client, callback_query: CallbackQuery):
    ...
    try:
        ...
        await safe_edit(callback_query.message.edit_text, 
            f"{E_CHECK} <b>ǫᴜᴀʟɪᴛʏ sᴇʟᴇᴄᴛᴇᴅ!</b>\n\n"
