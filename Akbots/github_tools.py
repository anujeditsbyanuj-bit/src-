...
def _split_owner_repo(url: str):
    ...
    Also handles URLs that point at something *inside* a repo, e.g. a
    branch/tree link like 'https://github.com/owner/repo/tree/branch' or
    '.../tree/branch/subfolder' — only the first two path segments after
    the host are ever the owner/repo, everything after that (tree, blob,
    branch name, subpaths) is ignored here.
    """
...
def _parse_github_url(url: str):
    """Return (clone_url, owner, repo, branch).

    branch is None for a plain repo URL, or the branch name when the URL
    is a GitHub 'tree' link (e.g. '.../owner/repo/tree/some-branch' or
    '.../tree/some-branch/sub/folder').
    """
    owner, repo = _split_owner_repo(url)
    branch = None
    m = re.search(r"github\.com/[^/\s]+/[^/\s]+/tree/([^/?#]+)", url.strip())
    if m:
        branch = m.group(1)
...
NOT_CONFIGURED = (
    f"{E_WARN} <b>ɢɪᴛʜᴜʙ ᴛᴏᴏʟs ᴀʀᴇɴ'ᴛ ᴄᴏɴғɪɢᴜʀᴇᴅ.</b>\n"
...
async def create_repo_command(client: Client, message: Message):
    ...
    if resp.status_code == 201:
        ...
        await message.reply_text(
            f"{E_CHECK} <b>ʀᴇᴘᴏsɪᴛᴏʀʏ ᴄʀᴇᴀᴛᴇᴅ.</b>\n{repo_link}"
        )
    else:
...
async def download_repo_command(client: Client, message: Message):
    ...
    try:
        clone_url, owner, repo_name, branch = _parse_github_url(message.command[1])
    except Exception:
        return await message.reply_text(
            f"{E_WARN} That doesn't look like a valid GitHub repo URL."
        )

    repo_path = os.path.join("/tmp", repo_name)
    label = f"{owner}/{repo_name}" + (f" (branch: {branch})" if branch else "")
    status = await message.reply_text(f"{E_CHECK} Cloning {label}...")
    ...
    try:
        ...
        await safe_edit(status.edit_text, f"{E_CROSS} Unable to download that repo: {e}")
    else:
        await status.delete()
    finally:
        if os.path.exists(repo_path):
...
async def fork_command(client: Client, message: Message):
    ...
    try:
        ...
        if resp.status_code == 202:
            ...
            await message.reply_text(
                f"{E_CROSS} Your GitHub token is invalid or expired.\n"
                "Set a fresh one with <code>/addaccount &lt;token&gt;</code> "
                "(personal) or <code>/setgittoken &lt;token&gt;</code> (bot-wide)."
            )
        else:
            await message.reply_text(
...
async def github_user_lookup_command(client: Client, message: Message):
    ...
    caption = (
        ...
        f"<b>ʙɪᴏ:</b> {data.get('bio') or '-'}\n"
        f"<b>ʟɪɴᴋ:</b> {data.get('html_url')}\n"
        f"<b>ᴄᴏᴍᴘᴀɴʏ:</b> {data.get('company') or '-'}\n"
        f"<b>ᴄʀᴇᴀᴛᴇᴅ ᴏɴ:</b> {data.get('created_at')}\n"
        f"<b>ʀᴇᴘᴏsɪᴛᴏʀɪᴇs:</b> {data.get('public_repos')}\n"
        f"<b>ʙʟᴏɢ:</b> {data.get('blog') or '-'}\n"
        f"<b>ʟᴏᴄᴀᴛɪᴏɴ:</b> {data.get('location') or '-'}\n"
        f"<b>ғᴏʟʟᴏᴡᴇʀs:</b> {data.get('followers')}\n"
        f"<b>ғᴏʟʟᴏᴡɪɴɢ:</b> {data.get('following')}"
        "</blockquote>"
    )
...
async def all_repos_command(client: Client, message: Message):
    ...
    try:
        ...
        repo_info = "\n\n".join(
            f"<b>ʀᴇᴘᴏsɪᴛᴏʀʏ:</b> {repo['full_name']}\n"
            f"<b>ᴅᴇsᴄʀɪᴘᴛɪᴏɴ:</b> {repo.get('description') or '-'}\n"
            f"<b>sᴛᴀʀs:</b> {repo['stargazers_count']}\n"
            f"<b>ғᴏʀᴋs:</b> {repo['forks_count']}\n"
            f"<b>ᴜʀʟ:</b> {repo['html_url']}"
            for repo in repos
        )
