...
#
# Requires GIT_TOKEN (a GitHub Personal Access Token with `repo` +
# `delete_repo` scopes) to be set in config.py / the environment. Without
# it, every command below replies with a short "not configured" message
...
def _auth_headers() -> dict:
    return {"Authorization": f"token {GIT_TOKEN}"}
...
NOT_CONFIGURED = (
    ...
    "Set <code>GIT_TOKEN</code> (a GitHub Personal Access Token with "
    "<code>repo</code> + <code>delete_repo</code> scopes) in the environment "
...
async def delete_repo_command(client: Client, message: Message):
    ...
    try:
        ...
        resp = requests.delete(
            f"{GITHUB_API}/repos/{owner}/{repo_name}", headers=_auth_headers()
        )
...
async def fork_command(client: Client, message: Message):
    ...
    try:
        owner, repo_name = _split_owner_repo(message.command[1])
        resp = requests.post(
            f"{GITHUB_API}/repos/{owner}/{repo_name}/forks", headers=_auth_headers()
        )
...
async def fork_all_command(client: Client, message: Message):
    if not _configured() or Github is None:
        return await message.reply_text(NOT_CONFIGURED)
