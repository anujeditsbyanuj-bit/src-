...
async def qr_handler(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>Usage:</b> <code>/qr your text or link here</code>", parse_mode=enums.ParseMode.HTML
        )
...
async def ip_info(client: Client, message: Message):
    if len(message.command) != 2:
        return await message.reply_text(
            "<b>Usage:</b> <code>/ip 8.8.8.8</code>", parse_mode=enums.ParseMode.HTML
        )
...
async def weather(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>Usage:</b> <code>/weather London</code>", parse_mode=enums.ParseMode.HTML
        )
