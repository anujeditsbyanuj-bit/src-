...
async def hashtag_gen(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>ᴜsᴀɢᴇ:</b> <code>/hashtag python</code>", parse_mode=enums.ParseMode.HTML
        )
...
async def hex_code(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>ᴜsᴀɢᴇ:</b> <code>/code Hello World!</code> (or a hex string to decode)",
    ...
    await message.reply_text(
        f"<b>ɪɴᴘᴜᴛ:</b> {input_text}\n<b>ʜᴇx:</b> <code>{hex_repr}</code>\n<b>ᴅᴇᴄᴏᴅᴇᴅ (ɪғ ɪɴᴘᴜᴛ ᴡᴀs ʜᴇx):</b> {decoded}",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def day_of_week(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>ᴜsᴀɢᴇ:</b> <code>/day 1947-08-15</code>", parse_mode=enums.ParseMode.HTML
        )
    try:
...
async def whatsapp_link(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            "<b>ᴜsᴀɢᴇ:</b> <code>/whatsapp +1234567890</code>", parse_mode=enums.ParseMode.HTML
        )
