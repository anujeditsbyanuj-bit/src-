...
#
# Resolves in-process by default via Akbots/filmyfly_resolver.py (ported
# 1:1 from workers/filmyfly-resolver/src/worker.js) — no separate
# Cloudflare Worker deploy needed, works out of the box on any host.
#
# FILMYFLY_WORKER_URL remains a manual override for anyone who deployed
# the actual Worker anyway (e.g. to keep this traffic off the bot's own
# host/IP) — if set, it's tried first, falling back to the in-process
# resolver if the Worker call fails.
...
from Akbots import filmyfly_resolver
...
def is_available() -> bool:
    # Always available — the in-process resolver needs no configuration.
    return True


async def _resolve_via_worker(page_url: str):
...
async def _resolve(page_url: str):
    """Tries the configured Worker first (if set), then always falls back
    to the in-process resolver (Akbots/filmyfly_resolver.py)."""
    if FILMYFLY_WORKER_URL:
        try:
            return await _resolve_via_worker(page_url)
        except Exception as e:
            logger.info(f"filmyfly: Worker resolve failed ({e}), falling back to in-process resolver")
    return await filmyfly_resolver.resolve(page_url)
