...
def _quality_kb(session_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        make_button("🎧 320 ᴋʙᴘs", callback_data=f"saavnq#{session_id}#320", style=_BS.PRIMARY if _BS else None),
        make_button("🎧 160 ᴋʙᴘs", callback_data=f"saavnq#{session_id}#160", style=_BS.PRIMARY if _BS else None),
    ], [
        make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"saavncancel#{session_id}", style=_BS.DANGER if _BS else None),
    ]])
...
async def _download_one_song(client: Client, message: Message, status: Message, song_id: str, bitrate: int, user_id: int = None):
    ...
    try:
        ...
        if not auth_url:
            await safe_edit(status.edit_text, f"<b>{E_CROSS} No downloadable stream found for:</b> {title}", parse_mode=enums.ParseMode.HTML)
            return False
        ...
        caption_lines = [
            f"<b>🎧 {title}</b>",
            f"<b>📚 ᴀʟʙᴜᴍ:</b> {album}" if album else "",
            f"<b>🎤 ᴀʀᴛɪsᴛ:</b> {singers}" if singers else "",
            f"<b>🎚 ǫᴜᴀʟɪᴛʏ:</b> {bitrate}kbps",
        ]
        caption = "<blockquote>" + "\n".join(filter(None, caption_lines)) + "</blockquote>"
        ...
        await safe_edit(status.edit_text, f"<b>{E_CROSS} Failed:</b> {title}\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
        return False
    finally:
...
async def _run_download(client: Client, message: Message, kind: str, item_id: str, bitrate: int, status: Message, user_id: int = None):
    if kind == "song":
        await safe_edit(status.edit_text, f"<b>{E_BOLT} Starting download...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    while True:
        try:
            ...
            await safe_edit(status.edit_text, f"<b>{E_CROSS} Failed to fetch {kind}:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
            return
    ...
    if total_done == 0:
        await safe_edit(status.edit_text, f"<b>{E_CROSS} No tracks found in this {kind}.</b>", parse_mode=enums.ParseMode.HTML)
    else:
        await safe_edit(status.edit_text, f"<b>{E_CHECK} Done — {total_done} track(s) sent.</b>", parse_mode=enums.ParseMode.HTML)
...
async def _do_saavn_search(client: Client, message: Message, query: str, status: Message = None):
    ...
    if status is None:
        ...
        await safe_edit(status.edit_text, f"<b>{E_INFO} Searching JioSaavn for:</b> {query}", parse_mode=enums.ParseMode.HTML)
    ...
    for result in results:
        ...
        buttons.append([make_button(label[:64], callback_data=f"saavnpick#{song_id}", style=_BS.PRIMARY if _BS else None)])
...
async def saavn_quality_callback(client: Client, callback_query: CallbackQuery):
    ...
    await safe_edit(status.edit_text, f"<b>{E_INFO} Preparing download...</b>", parse_mode=enums.ParseMode.HTML)
