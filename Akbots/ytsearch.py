...
E_SEARCH = '<tg-emoji emoji-id="5334544901428229844">🔍</tg-emoji>'
...
def _format_results_text(query: str, results, page: int, exhausted: bool = False) -> str:
    ...
    lines = [f"{E_SEARCH} <b>sᴇᴀʀᴄʜ ʀᴇsᴜʟᴛs ғᴏʀ:</b> <i>{query}</i>\n"]
...
def _results_keyboard(results, page: int, exhausted: bool = False, status_msg_id: int = None) -> InlineKeyboardMarkup:
    ...
    for abs_idx in range(start, end):
        row.append(make_button(str(abs_idx + 1), callback_data=f"ytsr:{abs_idx}", style=_BS.PRIMARY if _BS else None))
        if len(row) == 5:
            buttons.append(row)
            row = []
    if row:
    ...
    nav_row = []
    if page > 0:
        nav_row.append(make_button("◀️ ᴘʀᴇᴠ", callback_data=f"ytsrpg:{page - 1}", style=_BS.PRIMARY if _BS else None))
    nav_row.append(make_button(f"ᴘᴀɢᴇ {page + 1}", callback_data="ytsr:noop", style=_BS.PRIMARY if _BS else None))
    if not exhausted or end < len(results):
        nav_row.append(make_button("ɴᴇxᴛ ▶️", callback_data=f"ytsrpg:{page + 1}", style=_BS.PRIMARY if _BS else None))
    ...
    if status_msg_id is not None:
        buttons.append([make_button("🎵 sᴇᴀʀᴄʜ ᴏɴ ᴊɪᴏsᴀᴀᴠɴ ɪɴsᴛᴇᴀᴅ", callback_data=f"saavnfromtext#{status_msg_id}", style=_BS.PRIMARY if _BS else None)])
...
async def _do_search(client: Client, message: Message, query: str, status: Message = None):
    ...
    if status is None:
        ...
        await safe_edit(status.edit_text, f"<b>{E_ROCKET} Searching YouTube...</b>", parse_mode=enums.ParseMode.HTML)
    try:
    ...
    if not results:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} No results found for:</b> <i>{query}</i>", parse_mode=enums.ParseMode.HTML)
...
# Plain-text auto-search should only fire when the message actually looks
# like someone naming a piece of media to find — "lofi song", "pathaan
# movie", "money heist episode 5", "trending reel", "whatsapp status",
# "tom and jerry cartoon", "travel vlog" — not on every random line of chat
# sent to the bot (e.g. "how are you" has none of these words, so it's left
# alone). Grouped by category, each with its close synonyms/variants
# (plurals, short forms, common alternates).
_MEDIA_KEYWORDS = (
    # song
    "song", "songs", "track", "tracks", "audio", "mp3", "music",
    "gana", "gaana", "gaane", "geet", "tune",
    # video
    "video", "videos", "vid", "vids", "clip", "clips", "footage",
    # episode
    "episode", "episodes", "ep", "eps", "season", "seasons", "series", "webseries",
    # movie
    "movie", "movies", "film", "films", "cinema", "flick",
    # reel
    "reel", "reels", "shorts", "short",
    # stories
    "story", "stories", "status",
    # cartoon
    "cartoon", "cartoons", "anime", "animated", "animation", "toon", "toons",
    # vlog
    "vlog", "vlogs", "vlogger", "vlogging",
    # trailer
    "trailer", "trailers", "teaser", "teasers",
    # lyrics
    "lyrics", "lyric", "lyrical",
    # ringtone
    "ringtone", "ringtones", "bgm", "tone",
    # podcast
    "podcast", "podcasts",
    # audiobook
    "audiobook", "audiobooks",
    # remix / cover
    "remix", "remixes", "mashup", "mashups", "cover", "covers", "unplugged",
    # documentary
    "documentary", "documentaries", "docu",
    # concert
    "concert", "concerts", "performance",
    # interview
    "interview", "interviews",
    # tutorial
    "tutorial", "tutorials", "howto", "guide",
    # sports highlights
    "highlights", "highlight", "match", "matches",
    # prank
    "prank", "pranks",
    # gameplay
    "gameplay", "gaming", "playthrough", "walkthrough",
    # unboxing / review
    "unboxing", "review", "reviews",
    # workout
    "workout", "workouts", "gym",
    # recipe
...
_MEDIA_KEYWORD_HINT = filters.regex(
    r"(?i)\b(" + "|".join(_MEDIA_KEYWORDS) + r")\b"
)
...
def _engine_picker_kb(token: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        make_button("▶️ ʏᴏᴜᴛᴜʙᴇ", callback_data=f"enginepick#yt#{token}", style=_BS.PRIMARY if _BS else None),
        make_button("🎵 ᴊɪᴏsᴀᴀᴠɴ", callback_data=f"enginepick#saavn#{token}", style=_BS.PRIMARY if _BS else None),
        make_button("🟢 sᴘᴏᴛɪғʏ", callback_data=f"enginepick#spotify#{token}", style=_BS.PRIMARY if _BS else None),
    ]])
...
async def plain_text_auto_search(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<b>{E_SEARCH} Search</b> <i>{query}</i> <b>ᴏɴ:</b>",
        parse_mode=enums.ParseMode.HTML,
