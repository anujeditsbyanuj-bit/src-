...
def _menu_page(urls: dict, page: int = 1):
    ...
    for offset, (url, info) in enumerate(page_items):
        ...
        number_buttons.append(InlineKeyboardButton(str(global_idx + 1), callback_data=f"ut_info:{global_idx}"))

    buttons = _split_rows(number_buttons, 4)

    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton("◀️", callback_data=f"ut_menu:{page - 1}"))
    if page < total_pages:
        nav.append(InlineKeyboardButton("▶️", callback_data=f"ut_menu:{page + 1}"))
    if nav:
        buttons.append(nav)

    buttons.append([InlineKeyboardButton("➕ Add URL", callback_data="ut_add")])
    buttons.append([
        InlineKeyboardButton("♻️ Refresh", callback_data=f"ut_menu:{page}"),
        InlineKeyboardButton("➖ Remove All", callback_data="ut_removeall"),
    ])
    buttons.append([InlineKeyboardButton(" ❌ Close ", callback_data="ut_close")])
...
def _empty_menu():
    ...
    buttons = [
        [InlineKeyboardButton("➕ Add URL", callback_data="ut_add")],
        [InlineKeyboardButton(" ❌ Close ", callback_data="ut_close")],
    ]
    return text, buttons
...
async def ut_add_cb(client: Client, query: CallbackQuery):
    ...
    await _photo_edit(
        ...
        [[InlineKeyboardButton(" Cancel ", callback_data="ut_cancel_add")]],
    )
...
async def ut_add_receive(client: Client, message: Message):
    ...
    if not added:
        return await _photo_reply(
            ...
            [[InlineKeyboardButton("🌐 Open Menu", callback_data="ut_menu:1")]],
        )
...
async def ut_info_cb(client: Client, query: CallbackQuery):
    ...
    text = (
        ...
        f"<b>Status:</b> {'🟢 Online' if info.get('status') else '🔴 Offline'}\n"
        f"<b>HTTP Status:</b> <code>{info.get('response_status', '—')}</code>\n"
        f"<b>Last checked:</b> <code>{ago}</code>"
    )
    buttons = [
        [InlineKeyboardButton("➖ Remove", callback_data=f"ut_remove:{idx}")],
        [InlineKeyboardButton("🏠 Back", callback_data="ut_menu:1"), InlineKeyboardButton(" ❌ Close ", callback_data="ut_close")],
    ]
