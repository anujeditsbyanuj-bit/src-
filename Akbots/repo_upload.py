...
async def upload_repo_command(client: Client, message: Message):
    ...
    try:
        await doc_message.download(file_name=zip_path, progress=make_download_progress(status, file_name="repo.zip"))
        os.makedirs(extract_dir, exist_ok=True)
        try:
            _safe_extract(zip_path, extract_dir)
        except Exception as e:
            return await safe_edit(status.edit_text, f"{E_CROSS} Couldn't extract that zip: {e}")
        ...
        if findings:
            ...
            return await safe_edit(status.edit_text, 
                f"{E_CROSS} <b>ᴜᴘʟᴏᴀᴅ ʙʟᴏᴄᴋᴇᴅ — ᴘᴏssɪʙʟᴇ sᴇᴄʀᴇᴛs ғᴏᴜɴᴅ:</b>\n{rows}\n\n"
        ...
        await safe_edit(status.edit_text, f"{E_WARN} Preparing repository…")
        ...
        if push.returncode == 0:
            ...
            await safe_edit(status.edit_text, f"{E_CROSS} Push failed:\n<code>{safe_err[:1500]}</code>")
    except Exception as e:
        await safe_edit(status.edit_text, f"{E_CROSS} Error: {redact_sensitive_text(e)}")
    finally:
