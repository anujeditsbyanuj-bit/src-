...
from Akbots.direct_utils import make_sample_clip, VIDEO_EXTS, safe_edit, make_download_progress
...
async def sample_cmd(client: Client, message: Message):
    ...
    try:
        await client.download_media(replied, file_name=in_path, progress=make_download_progress(status))
    ...
    await safe_edit(status.edit_text, f"<b>{E_FILM} Generating {clip_seconds}s sample...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    try:
        await client.send_video(
            ...
            caption=f"<blockquote>{E_FILM} <b>sᴀᴍᴘʟᴇ ᴄʟɪᴘ</b> ({clip_seconds}s)</blockquote>",
            reply_to_message_id=message.id,
            supports_streaming=True, parse_mode=enums.ParseMode.HTML,
        )
        await status.delete()
    except Exception as e:
        await safe_edit(status.edit_text, f"<b>{E_CROSS} Failed to send:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)
...
async def autosample_cmd(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        ...
        return await message.reply_text(
            f"<blockquote>{E_FILM} <b>ᴀᴜᴛᴏ sᴀᴍᴘʟᴇ:</b> <code>{'ON' if current else 'OFF'}</code>\n\n"
            f"<b>ᴜsᴀɢᴇ:</b> <code>/autosample on</code> or <code>/autosample off</code>\n"
