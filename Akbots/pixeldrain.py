...
PATTERN = re.compile(r"(https?://)?(www\.)?pixeldrain\.(com|dev|net)/[ul]/\S+", re.IGNORECASE)
...
def _parse(link: str):
    m = re.search(r'pixeldrain\.(com|dev|net)/u/([a-zA-Z0-9]+)', link)
    if m:
        return "file", m.group(2), m.group(1)
    m = re.search(r'pixeldrain\.(com|dev|net)/l/([a-zA-Z0-9]+)', link)
    if m:
        return "list", m.group(2), m.group(1)
...
async def _handle(client: Client, message: Message, url: str):
    ...
    url_type, item_id, _domain = _parse(url)
    if not url_type:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Invalid Pixeldrain link.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    try:
        if url_type == "file":
            ...
            if not files:
                return await safe_edit(status.edit_text, f"<b>{E_CROSS} Empty Pixeldrain list.</b>", parse_mode=enums.ParseMode.HTML)
