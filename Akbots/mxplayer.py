...
def _audio_keyboard(session_id: str, audios, selected: set, is_final_step: bool):
    ...
    action_row = [make_button("🚀 ᴅᴏɴᴇ", callback_data=f"mxdone:{session_id}", style=_BS.SUCCESS if _BS else None)]
...
async def _handle(client: Client, message: Message, url: str):
    ...
    buttons = [[make_button(v["ʟᴀʙᴇʟ"], callback_data=f"mxv:{session_id}:{v['id']}", style=_BS.PRIMARY if _BS else None)] for v in videos[:20]]
    if audios:
        buttons.append([make_button("🎵 ᴀᴜᴅɪᴏ ᴏɴʟʏ", callback_data=f"mxaudioonly:{session_id}", style=_BS.SUCCESS if _BS else None)])
