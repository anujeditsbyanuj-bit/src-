...
async def download_stream(stream: dict, title: str, status: Message,
                           quality: str | None = None, out_dir: str | None = None,
    ...
    stream: dict shaped like meow*_provider.fetch_stream_url()'s return
            value: {"videoUrl": str, "qualities": [{"quality","url"}, ...],
                    "headers": {...}}.
    format_override: if given, used as yt-dlp's `format` string instead of
            the default bestvideo+bestaudio chain — e.g. a caller that
            picked a specific already-quality-locked variant URL (like
            Akbots/goon_commands.py does after parsing a master playlist)
            can pass this to skip yt-dlp trying to re-select within it.
            Every other caller is unaffected — this only changes behavior
