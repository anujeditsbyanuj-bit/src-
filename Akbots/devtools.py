...
@Client.on_message(filters.command(["checkdeps"]) & filters.user(ADMINS))
async def checkdeps_command(client: Client, message: Message):
    ...
    lines = ["<b>ʀᴜɴᴛɪᴍᴇ ᴅᴇᴘᴇɴᴅᴇɴᴄʏ ᴄʜᴇᴄᴋ</b>\n"]
    ...
    try:
        import yt_dlp
        lines.append(f"{E_CHECK} <b>ʏᴛ-ᴅʟᴘ:</b> <code>{yt_dlp.version.__version__}</code>")
    except Exception:
        lines.append(f"{E_CROSS} <b>ʏᴛ-ᴅʟᴘ:</b> <code>not importable</code>")

    lines.append("")
    try:
        from playwright.async_api import async_playwright  # noqa: F401
        lines.append(f"{E_CHECK} <b>ᴘʟᴀʏᴡʀɪɢʜᴛ (ᴘɪᴘ ᴘᴀᴄᴋᴀɢᴇ):</b> installed")
    except ImportError:
        lines.append(f"{E_CROSS} <b>ᴘʟᴀʏᴡʀɪɢʜᴛ (ᴘɪᴘ ᴘᴀᴄᴋᴀɢᴇ):</b> not installed")
    ...
    if sys_chromium:
        lines.append(f"{E_CHECK} <b>ᴄʜʀᴏᴍɪᴜᴍ:</b> system binary at <code>{sys_chromium}</code>")
    else:
        cache_dir = os.path.expanduser("~/.cache/ms-playwright")
        found = glob.glob(os.path.join(cache_dir, "chromium-*"))
        if found:
            lines.append(f"{E_CHECK} <b>ᴄʜʀᴏᴍɪᴜᴍ:</b> self-installed at <code>{found[0]}</code>")
        else:
            lines.append(f"{E_CROSS} <b>ᴄʜʀᴏᴍɪᴜᴍ:</b> not found (neither system nor self-installed)")
    ...
    if _cf_available():
        lines.append(f"{E_CHECK} <b>ᴄғ_ʙʏᴘᴀss (ᴀɴᴛɪ-ʙᴏᴛ/ᴄʟᴏᴜᴅғʟᴀʀᴇ):</b> ready — <code>/cfcheck &lt;url&gt;</code> to test")
    else:
        lines.append(f"{E_CROSS} <b>ᴄғ_ʙʏᴘᴀss (ᴀɴᴛɪ-ʙᴏᴛ/ᴄʟᴏᴜᴅғʟᴀʀᴇ):</b> not available")
...
async def cfcheck_command(client: Client, message: Message):
    ...
    if not result:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Bypass failed or timed out.</b>", parse_mode=enums.ParseMode.HTML)

    cookie_count = len(result["cookies"])
    await safe_edit(status.edit_text, 
        f"<b>{E_CHECK} Bypass successful.</b>\n\n"
        f"<b>ғɪɴᴀʟ ᴜʀʟ:</b> <code>{result['final_url']}</code>\n"
        f"<b>ᴄᴏᴏᴋɪᴇs:</b> <code>{cookie_count}</code>\n"
        f"<b>ᴜsᴇʀ-ᴀɢᴇɴᴛ:</b> <code>{result['user_agent']}</code>\n"
        f"<b>ʜᴛᴍʟ ʟᴇɴɢᴛʜ:</b> <code>{len(result['html'])}</code> chars\n\n"
...
async def cfdetect_command(client: Client, message: Message):
    ...
    if info["error"]:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Request failed:</b> <code>{info['error']}</code>", parse_mode=enums.ParseMode.HTML)

    if not info["protected"]:
        return await safe_edit(status.edit_text, 
            f"<b>{E_CHECK} No anti-bot protection detected.</b>\n<b>sᴛᴀᴛᴜs:</b> <code>{info['status']}</code>",
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    await safe_edit(status.edit_text, 
        ...
        f"<b>ᴠᴇɴᴅᴏʀ:</b> <code>{info['vendor']}</code>\n"
        f"<b>sᴛᴀᴛᴜs:</b> <code>{info['status']}</code>\n\n"
        f"{bypass_note}",
        parse_mode=enums.ParseMode.HTML,
    )
