...
def _database_menu(uri: str | None) -> InlineKeyboardMarkup:
    ...
    if uri:
        rows = [[make_button("🗑 Remove Url", callback_data="database_del_url", style=(S.DANGER if S else None))]]
    else:
        rows = [[make_button("➕ Add Url ➕", callback_data="database_add_url", style=(S.SUCCESS if S else None))]]
    rows.append([make_button("🔁 Back", callback_data="settings_back_btn", style=(S.DANGER if S else None))])
...
async def database_add_url_callback(client: Client, callback_query: CallbackQuery):
    ...
    await safe_edit(callback_query.message.edit_text,
        f"<b>📥 Send me your MongoDB URI</b>\n\n"
        f"📝 <b>Example:</b> <code>mongodb+srv://user:pass@cluster0.xxxxx.mongodb.net/mydb</code>\n\n"
        f"/cancel - cancel this process",
        reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="database_btn", style=(_BS.DANGER if _BS else None))]]),
        parse_mode=enums.ParseMode.HTML,
    )
    ...
    if not (uri.startswith("mongodb://") or uri.startswith("mongodb+srv://")):
        return await reply.reply_text(
            f"<b>❌ Invalid URI! Please send a valid MongoDB URI.</b>",
            reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="database_btn", style=(_BS.DANGER if _BS else None))]]),
