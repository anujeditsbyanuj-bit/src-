...
async def run_db_backup(app: Client) -> bool:
    ...
    try:
        ...
        await app.send_document(
            ...
            caption=f"<blockquote><b>🗄 ᴅᴀᴛᴀʙᴀsᴇ ʙᴀᴄᴋᴜᴘ</b>\n<b>👤 ᴜsᴇʀs:</b> <code>{count}</code>\n<b>🕒 {stamp}</b></blockquote>",
            parse_mode=enums.ParseMode.HTML,
        )
...
async def backupdb_command(client: Client, message: Message):
    status = await message.reply_text("<b>🗄 ʙᴜɪʟᴅɪɴɢ ᴅᴀᴛᴀʙᴀsᴇ ʙᴀᴄᴋᴜᴘ...</b>", parse_mode=enums.ParseMode.HTML)
    ok = await run_db_backup(client)
    await safe_edit(status.edit_text, 
        "<b>✅ ʙᴀᴄᴋᴜᴘ ᴘᴏsᴛᴇᴅ ᴛᴏ ᴅʙ_ᴄʜᴀɴɴᴇʟ.</b>" if ok else
        "<b>❌ ʙᴀᴄᴋᴜᴘ ғᴀɪʟᴇᴅ — ᴄʜᴇᴄᴋ ᴛʜᴇ ʟᴏɢs.</b>",
        parse_mode=enums.ParseMode.HTML,
    )
