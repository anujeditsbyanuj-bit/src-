...
def scrape_gdflix(url):
    ...
    # A single-file page repeats its own filename multiple times in the
    # markup (<title>, heading, download-button attributes, etc.) — that's
    # 2+ raw regex matches but only ONE distinct filename. Only treat the
    # page as an actual season/episode pack when 2+ DISTINCT filenames are
    # present; otherwise every repeat of the same name gets misread as a
    # separate "episode" (with the first N-1 copies having no links yet,
    # since the real download section only follows the last occurrence).
    distinct_names = {m.group(0).lower() for m in filename_matches}

    # Single file (0 or 1 filename match on the page, or all matches are
    # just repeats of the same name) — original behaviour, unchanged, just
    # routed through the shared _extract_one() helper.
    if len(filename_matches) <= 1 or len(distinct_names) <= 1:
