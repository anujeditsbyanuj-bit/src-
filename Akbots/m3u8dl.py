...
# bot's Pyrogram plugin style, on top of what Akbots/mxplayer.py already does
...
# reimplemented here — Akbots/direct_utils.upload_file() already does all
...
def _quality_kb(session_id, qualities):
    rows = []
    if len(qualities) > 1:
        rows.append([make_button(f"📥 ᴀʟʟ ǫᴜᴀʟɪᴛɪᴇs ({len(qualities)})", callback_data=f"m3qall:{session_id}", style=_BS.PRIMARY if _BS else None)])
    for i, q in enumerate(qualities):
        label = f"📺 {q['name']} • {q['resolution']} • {q['bandwidth'] // 1000}kbps" if q["name"] != "default" else "📺 Default"
        rows.append([make_button(label, callback_data=f"m3q:{session_id}:{i}", style=_BS.PRIMARY if _BS else None)])
    rows.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"m3cancel:{session_id}", style=_BS.DANGER if _BS else None)])
...
def _audio_kb(session_id, tracks):
    ...
    for i, t in enumerate(tracks):
        ...
        rows.append([make_button(label, callback_data=f"m3a:{session_id}:{i}", style=_BS.PRIMARY if _BS else None)])
    rows.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"m3cancel:{session_id}", style=_BS.DANGER if _BS else None)])
    return InlineKeyboardMarkup(rows)
...
def _compress_kb(session_id):
    return InlineKeyboardMarkup([
        [make_button("🗜 ᴄᴏᴍᴘʀᴇss (ʜ.265, sᴍᴀʟʟᴇʀ/sʟᴏᴡᴇʀ)", callback_data=f"m3c:{session_id}:yes", style=_BS.PRIMARY if _BS else None)],
        [make_button("⚡ sᴋɪᴘ — ᴏʀɪɢɪɴᴀʟ ǫᴜᴀʟɪᴛʏ/sᴘᴇᴇᴅ", callback_data=f"m3c:{session_id}:no", style=_BS.PRIMARY if _BS else None)],
        [make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"m3cancel:{session_id}", style=_BS.DANGER if _BS else None)],
    ])
...
def _batch_kb(multi: bool):
    if multi:
        return InlineKeyboardMarkup([[
            make_button("⏭ sᴋɪᴘ", callback_data="m3skip", style=_BS.PRIMARY if _BS else None),
            make_button("❌ ᴄᴀɴᴄᴇʟ ᴀʟʟ", callback_data="m3cancelall", style=_BS.DANGER if _BS else None),
        ]])
    return InlineKeyboardMarkup([[make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data="m3skip", style=_BS.DANGER if _BS else None)]])
...
async def _handle_m3u8_urls(client: Client, message: Message, urls: list):
    ...
    try:
        ...
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Failed to parse:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)

    qualities = result["qualities"]
    if not qualities:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} No playable qualities found.</b>", parse_mode=enums.ParseMode.HTML)
...
async def _cancel_selection(client: Client, cq: CallbackQuery):
    ...
    await safe_edit(cq.message.edit_text, f"{E_CROSS} Cancelled.")
    await cq.answer()
...
async def _after_quality_pick(client, cq, session_id, is_all, height):
    ...
    if len(tracks) > 1:
        return await safe_edit(cq.message.edit_text, 
...
async def process_batch(client: Client, cid: int, task_info: dict):
    ...
    def make_progress_cb(kb):
        def cb(text):
            async def _edit():
                try:
                    await safe_edit(status.edit_text, text, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
                except Exception:
                    pass
    ...
    try:
        ...
        label = f"M3U8 batch: {len(urls)} URL(s)" if len(urls) > 1 else f"M3U8: {get_video_name(urls[0], 0, 1)}"
        task_id = task_manager.register(orig_message.from_user.id, asyncio.current_task(), label)
    except Exception:
        task_id = None
    ...
    try:
        for ui, url in enumerate(urls):
            ...
            try:
                await safe_edit(status.edit_text, f"<b>{E_INFO} Parsing {tag} {vname}</b>", reply_markup=kb, parse_mode=enums.ParseMode.HTML)
        ...
        if task_id is not None:
            try:
                from Akbots import task_manager
                task_manager.unregister(orig_message.from_user.id, task_id)
            except Exception:
                pass
