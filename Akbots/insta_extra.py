...
async def _download_profile_picture(client: Client, message: Message, username_or_url: str):
    ...
    if not pic_url:
        return await status.edit_text(f"<b>{E_CROSS} Could not find a profile picture for @{username}.</b>", parse_mode=enums.ParseMode.HTML)
...
async def _download_user_stories(client: Client, message: Message, username_or_url: str):
    ...
    cookies = _load_cookies()
    if not cookies:
        return await status.edit_text(f"<b>{E_CROSS} Login required for Stories.</b>{_COOKIE_HINT}", parse_mode=enums.ParseMode.HTML)
...
async def _download_highlight_by_id(client: Client, message: Message, highlight_id: str, title_hint: str = None):
    ...
    cookies = _load_cookies()
    if not cookies:
        return await status.edit_text(f"<b>{E_CROSS} Login required for Highlights.</b>{_COOKIE_HINT}", parse_mode=enums.ParseMode.HTML)
    ...
    if not items:
        return await status.edit_text(f"<b>{E_CROSS} This highlight is empty, private, or the link is invalid.</b>", parse_mode=enums.ParseMode.HTML)
...
async def _download_all_highlights(client: Client, message: Message, username_or_url: str):
    ...
    if not tray:
        return await status.edit_text(f"<b>{E_INFO} @{username} has no highlights.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    for idx, (reel_id, title) in enumerate(tray, start=1):
        ...
        try:
            ...
            await hl_status.edit_text(f"<b>{E_CROSS} '{title}' failed:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
            continue
        if not items:
            await hl_status.edit_text(f"<b>{E_CROSS} '{title}' is empty.</b>", parse_mode=enums.ParseMode.HTML)
            continue
        preview = await _build_media_preview(items, f"{E_INFO} {title}", include_sizes=False)
        await hl_status.edit_text(preview, parse_mode=enums.ParseMode.HTML)
