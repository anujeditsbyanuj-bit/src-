...
async def _build_reply_markup(user_id: int):
    ...
    if not btn or not btn.get("enabled"):
        return None
    return InlineKeyboardMarkup([[make_button(btn["ᴛᴇxᴛ"], url=btn["url"], style=_BS.PRIMARY if _BS else None)]])
...
async def forwardstatus_command(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_ROCKET} <b>ғᴏʀᴡᴀʀᴅ ᴇɴɢɪɴᴇ sᴛᴀᴛᴜs</b>\n\n"
        f"<b>ᴍᴏᴅᴇ:</b> {'🟢 On' if mode else '🔴 Off'} {'(live)' if live else ''}\n"
        f"<b>sᴏᴜʀᴄᴇs:</b> {len(sources)}\n"
        f"<b>ᴛᴀʀɢᴇᴛs:</b> {len(targets)}</blockquote>",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def setfilters_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ᴜsᴀɢᴇ:</b> <code>/setfilters photo,video,document</code>\n\n"
            f"<b>ᴀᴠᴀɪʟᴀʙʟᴇ ᴛʏᴘᴇs:</b> {', '.join(FILTER_TYPES)}</blockquote>",
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    if invalid:
        return await message.reply_text(
            ...
            f"<b>ᴀᴠᴀɪʟᴀʙʟᴇ:</b> {', '.join(FILTER_TYPES)}",
            parse_mode=enums.ParseMode.HTML,
        )
...
async def pimode_command(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_INFO} <b>ᴘɪ ᴍᴏᴅᴇ</b>\n\n"
...
async def numbering_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        extra = await db.get_forward_extra(user_id)
        return await message.reply_text(
            f"<blockquote>{E_INFO} <b>ᴀᴜᴛᴏ-ɴᴜᴍʙᴇʀɪɴɢ</b>\n\n"
            f"<b>ᴄᴜʀʀᴇɴᴛ:</b> {'🟢 On' if extra['numbering_enabled'] else '🔴 Off'} "
...
async def bullets_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        extra = await db.get_forward_extra(user_id)
        return await message.reply_text(
            f"<blockquote>{E_INFO} <b>ʙᴜʟʟᴇᴛs</b>\n\n"
            f"<b>ᴄᴜʀʀᴇɴᴛ:</b> {'🟢 On' if extra['bullets_enabled'] else '🔴 Off'} "
...
async def deltamode_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        extra = await db.get_forward_extra(user_id)
        return await message.reply_text(
            f"<blockquote>{E_INFO} <b>ᴅᴇʟᴛᴀ ᴍᴏᴅᴇ</b> — adds a source-message link to the caption\n\n"
            f"<b>ᴄᴜʀʀᴇɴᴛ:</b> {'🟢 On' if extra['delta_enabled'] else '🔴 Off'} "
