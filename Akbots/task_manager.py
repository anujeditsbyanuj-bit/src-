...
class queue_slot:
    ...
    def __init__(self, user_id: int, status_msg: Message = None,
                 waiting_text: str = "<b>⏳ ǫᴜᴇᴜᴇᴅ — ᴡᴀɪᴛɪɴɢ ғᴏʀ ᴀ ғʀᴇᴇ ᴅᴏᴡɴʟᴏᴀᴅ sʟᴏᴛ...</b>"):
    ...
    async def __aenter__(self):
        if self.sem.locked():
            ...
            if self.status_msg is not None:
                try:
                    await safe_edit(self.status_msg.edit_text, self.waiting_text, parse_mode=enums.ParseMode.HTML)
                except Exception:
                    pass
        try:
...
async def queue_command(client: Client, message: Message):
    ...
    if user_id in ADMINS and len(message.command) > 1 and message.command[1].lower() == "all":
        everyone = all_tasks()
        if not everyone:
            return await message.reply_text("<b>📭 ɴᴏ ᴀᴄᴛɪᴠᴇ ᴛᴀsᴋs ғᴏʀ ᴀɴʏᴏɴᴇ ʀɪɢʜᴛ ɴᴏᴡ.</b>")
        lines = ["<b>📋 ᴀᴄᴛɪᴠᴇ ᴛᴀsᴋs (ᴀʟʟ ᴜsᴇʀs):</b>", ""]
    ...
    if not items and not queued:
        return await message.reply_text("<b>📭 ʏᴏᴜ ʜᴀᴠᴇ ɴᴏ ᴀᴄᴛɪᴠᴇ ᴛᴀsᴋs ʀɪɢʜᴛ ɴᴏᴡ.</b>")
