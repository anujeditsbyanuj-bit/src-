...
def _caption_menu_buttons():
    ...
    return [
        [make_button("👀 See Caption", callback_data="caption_see_btn", style=S.PRIMARY if S else None),
         make_button("🗑 Delete Caption", callback_data="caption_del_btn", style=S.DANGER if S else None)],
        [make_button("🔁 Back", callback_data="settings_back_btn", style=S.DANGER if S else None)],
    ]
...
def build_upload_type_view(mode: str):
    ...
    text = (
        f"<blockquote>📤 <b>Upload Type</b>\n\n"
        f"{E_INFO} <b>Current:</b> <code>{current_label}</code>\n\n"
        f"{E_TIP} Choose how you want to receive media files:</blockquote>"
    )
    rows = [
        [make_button(
            " 📄 Document ", callback_data="um:document",
        ...
        [make_button(
            " 🎬 Media ", callback_data="um:auto",
            icon_custom_emoji_id=ICON_EDIT if BUTTON_STYLE_SUPPORTED else None,
            style=ButtonStyle.PRIMARY if BUTTON_STYLE_SUPPORTED else None,
        )],
        [make_button(
            " ⬅️ Back ", callback_data="settings_back_btn",
...
def get_settings_buttons():
    if BUTTON_STYLE_SUPPORTED:
        ...
        return InlineKeyboardMarkup([
            ...
            [make_button(" 🚀 1 ᴍᴀɴᴀɢᴇʀ ", callback_data="akanager_btn", icon_custom_emoji_id=ICON_LIST, style=S.PRIMARY)],
        ...
        return InlineKeyboardMarkup([
            ...
            [make_button(" 🚀 1 ᴍᴀɴᴀɢᴇʀ ", callback_data="akanager_btn", icon_custom_emoji_id=ICON_LIST, style=ButtonStyle.PRIMARY if BUTTON_STYLE_SUPPORTED else None)],
...
@Client.on_callback_query(filters.regex("^(cmd_list_btn|dump_chat_btn|thumb_btn|caption_btn|caption_see_btn|caption_del_btn|user_stats_btn|batch_limit_btn|upload_mode_btn|settings_back_btn|close_btn)$"))
async def settings_callbacks(client: Client, callback_query: CallbackQuery):
    ...
    if data == "cmd_list_btn":
        ...
        text = (
            f"<u><b>✏️ CUSTOM CAPTION ✏️</b></u>\n\n"
            f"📝 <b>CUSTOM CAPTION SETTINGS</b>\n"
        ...
        if caption:
            ...
            text = f"<blockquote>{E_CROSS} <b>No Caption Set</b>\n\n{E_INFO} You are currently using the default bot caption.</blockquote>"
        await safe_edit(callback_query.edit_message_text, 
            text, reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="caption_btn", style=ButtonStyle.DANGER if BUTTON_STYLE_SUPPORTED else None)]]),
            parse_mode=enums.ParseMode.HTML
        )
