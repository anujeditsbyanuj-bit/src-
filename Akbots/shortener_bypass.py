...
async def _bump_bypass_count():
    ...
    try:
        ...
        current = await db._fs_get_config("akbots_bypass_count", 0) or 0
        await db._fs_set_config("akbots_bypass_count", current + 1)
    except Exception:
        pass
...
async def run_bypass_card(url: str) -> tuple[str, bool]:
    ...
    /bypass below and Akbots/akbypass.py's group auto-detect, so the
    ...
    if not is_available() and not browser_tier_might_work:
        return (f"⚠️ <b>No Script Found for:</b>\n{url}\n\n{footer}", False)
...
# Chat ids that sent a bare "/bypass" (no url) and are now waiting for the
# next message to contain the link — lets people do /bypass then paste the
# link separately, instead of only /bypass <url> in one message.
...
async def _do_bypass(client: Client, message: Message, url: str):
    """Shared worker for both '/bypass <url>' and a pasted link that
    follows a bare '/bypass' — keeps the two paths byte-for-byte identical."""
    try:
        from Akbots.playwright_bypass import is_playwright_bypass_site
        browser_tier_might_work = is_playwright_bypass_site(url)
    except Exception:
...
@Client.on_message(filters.command(["bypass", "shortbypass"]))
...
async def bypass_pending_link(client: Client, message: Message):
    """Catches the follow-up message after a bare '/bypass' and treats the
    first link found in it as the url to bypass. Ignored entirely unless
