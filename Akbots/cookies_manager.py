...
# (client identity, user_id) -> domain, set by /setcookies while waiting
# for the file to follow as the user's next message.
#
# Keyed by (id(client), user_id) rather than bare user_id: the main bot
# and every Titanium clone (Akbots/titanium.py) run as separate Client
# instances INSIDE THE SAME PYTHON PROCESS, so a bare user_id key would be
# shared state across all of them. An owner who tests /setcookies on the
# main bot and then on their clone (or vice versa) within the same
# ADD_TIMEOUT window would have the second command's entry silently
# overwrite/clear the first's, so the first bot's cookies paste falls
# through to ytdl.py's URL auto-detector instead of being saved (it grabs
# the stray "https://curl.se/docs/http-cookies.html" URL from the
# standard cookies.txt header comment and tries to download THAT page).
_pending_setcookies: dict[tuple[int, int], str] = {}

# (client identity, user_id) -> {"expires": monotonic deadline, "chat_id": int, "msg_id": int}
# set by the "Add Cookie" button while waiting for the next document. Same
# per-client keying reasoning as _pending_setcookies above.
_pending_panel: dict[tuple[int, int], dict] = {}


def _pk(client, user_id: int) -> tuple[int, int]:
    """Per-(bot, user) key for the pending-flow dicts above — see the
    comment on _pending_setcookies for why this can't just be user_id."""
...
def has_pending_cookie_flow(client, user_id: int) -> bool:
    ...
    panel = _pending_panel.get(key)
    if panel and time.monotonic() <= panel["expires"]:
        return True
    return key in _pending_setcookies
...
async def setcookies_command(client: Client, message: Message):
    ...
    _pending_setcookies[_pk(client, message.from_user.id)] = domain
    await message.reply_text(
...
async def cookie_panel_callback(client: Client, cq: CallbackQuery):
    ...
    if action == "root":
        _pending_panel.pop(_pk(client, cq.from_user.id), None)
        try:
    ...
    if action == "add":
        _pending_panel[_pk(client, cq.from_user.id)] = {
...
async def setcookies_file_receive(client: Client, message: Message):
    ...
    if user_id not in ADMINS:
        # MUST continue propagation, not just `return` — a bare return
        # here silently swallows every document from non-admins, blocking
        # any lower-group handler (e.g. direct_utils.wait_for_reply) that
        # was waiting on it.
        message.continue_propagation()

    # Panel-driven "Add Cookie" flow (auto-detects domain from the file).
    key = _pk(client, user_id)
    panel_pending = _pending_panel.get(key)
    if panel_pending:
    ...
    domain = _pending_setcookies.pop(key, None)
    if domain:
...
async def setcookies_text_receive(client: Client, message: Message):
    user_id = message.from_user.id
    if user_id not in ADMINS:
        # MUST continue propagation, not just `return`. This handler is
        # group=-1, same group (and registered BEFORE) direct_utils.
        # wait_for_reply's dynamic handler used by "Add Bot Using Token".
        # A bare `return` here silently ate every non-admin's plain text
        # message — including bot tokens pasted for that flow — before
        # wait_for_reply ever got a chance to see it.
        message.continue_propagation()
    key = _pk(client, user_id)
    if key not in _pending_panel and key not in _pending_setcookies:
        ...
        if len(text.strip()) < 20:
            message.continue_propagation()  # ordinary text message, not cookies — ignore
        ...
        if _looks_cookie_ish(text):
            ...
            message.stop_propagation()
            return
        # Ordinary text message, not cookies — MUST continue propagation
        # (this is the exact path a bot token sent for "Add Bot Using
        # Token" falls into: >=20 chars, doesn't look like a cookie).
        message.continue_propagation()

    text = message.text or ""
    if len(text.strip()) < 20:
        message.continue_propagation()  # too short to plausibly be a cookies.txt paste — ignore, not our concern
