...
E_DIAMOND = '<tg-emoji emoji-id="5217822164362739968">💎</tg-emoji>'
...
async def pay_command(client: Client, message: Message):
    buttons = InlineKeyboardMarkup([
        [make_button(f"{E_STAR} {p['label']} — {p['stars']} sᴛᴀʀs", callback_data=f"paystar_{key}", style=_BS.PRIMARY if _BS else None)]
        for key, p in STAR_PLANS.items()
    ])
    text = (
        f"<blockquote>{E_DIAMOND} <b>ᴄʜᴏᴏsᴇ ʏᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴘʟᴀɴ</b>\n\n"
...
async def successful_payment_handler(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        f"{E_DIAMOND} <b>ᴘʟᴀɴ:</b> {plan['label']}\n"
        f"{E_STAR} <b>ᴘᴀɪᴅ:</b> {plan['stars']} Stars\n"
        f"⏳ <b>ᴠᴀʟɪᴅ ᴜɴᴛɪʟ:</b> <code>{expiry_date}</code>\n\n"
        f"<i>Enjoy! 🎉</i>",
