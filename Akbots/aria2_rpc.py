...
def _task_kb(gid: str, paused: bool = False) -> InlineKeyboardMarkup:
    toggle = (
        make_button("▶️ Resume", callback_data=f"rpcresume#{gid}", style=_BS.SUCCESS if _BS else None)
        if paused else
        make_button("⏸ Pause", callback_data=f"rpcpause#{gid}", style=_BS.PRIMARY if _BS else None)
    )
    return InlineKeyboardMarkup([
        [toggle, make_button("✏️ Edit", callback_data=f"rpcedit#{gid}")],
        [make_button("❌ Remove", callback_data=f"rpcremove#{gid}", style=_BS.DANGER if _BS else None)],
    ])
    ...
    while True:
        ...
        if st == "active":
            ...
            try:
                ...
                with contextlib.suppress(Exception):
                    await status.edit_text(f"<b>{E_CROSS} Upload failed:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
            finally:
                with contextlib.suppress(Exception):
...
async def rpc_remove_callback(client: Client, callback_query: CallbackQuery):
    ...
    with contextlib.suppress(Exception):
        await callback_query.message.edit_text(f"<b>{E_CROSS} Task removed.</b>", parse_mode=enums.ParseMode.HTML)
...
async def rpc_edit_menu_callback(client: Client, callback_query: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup([
        [make_button("🔗 Change URL", callback_data=f"rpcediturl#{gid}"),
         make_button("🌐 Change Proxy", callback_data=f"rpceditproxy#{gid}")],
        [make_button("⬅️ Back", callback_data=f"rpceditback#{gid}")],
    ])
    with contextlib.suppress(Exception):
...
async def _prompt_and_apply(client: Client, callback_query: CallbackQuery, gid: str,
    ...
    if gid not in _TASKS:
        return await callback_query.message.edit_text(f"<b>{E_CROSS} Task no longer exists.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    try:
        resp = await client.listen(
            ...
            filters=filters.user(callback_query.from_user.id), timeout=60,
        )
    except Exception:
        with contextlib.suppress(Exception):
            await ask.edit_text(f"<b>{E_CROSS} Timed out — nothing changed.</b>", parse_mode=enums.ParseMode.HTML)
        if was_active:
            with contextlib.suppress(Exception):
                await _rpc_call("aria2.unpause", [gid])
        return
