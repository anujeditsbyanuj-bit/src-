...
async def _handle(client: Client, message: Message, link: str):
    ...
    await safe_edit(status.edit_text, f"<b>{E_INFO} Downloading via aria2c...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    if returncode != 0:
        ...
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Torrent download failed:</b>\n<code>{err}</code>", parse_mode=enums.ParseMode.HTML)
