...
def _status_text() -> str:
    ...
    lines.append("<b>ᴘʏᴛʜᴏɴ ᴘᴀᴄᴋᴀɢᴇs (ᴘɪᴘ)</b>")
    for key, (label, check, args, used_by, post) in PIP_DEPS.items():
    ...
    if missing:
        ...
        lines.append(f"{E_CHECK} <b>ᴇᴠᴇʀʏᴛʜɪɴɢ's ɪɴsᴛᴀʟʟᴇᴅ.</b>")
    return "\n".join(lines)
...
def _status_buttons() -> InlineKeyboardMarkup:
    ...
    if row:
        rows.append(row)
    if rows:
        rows.append([make_button("⬇️ ɪɴsᴛᴀʟʟ ᴀʟʟ ᴍɪssɪɴɢ", callback_data="depinstall:all",
                                  style=_BS.SUCCESS if _BS else None)])
    rows.append([
        make_button("🔄 ʀᴇғʀᴇsʜ", callback_data="depinstall:refresh", style=_BS.PRIMARY if _BS else None),
        make_button("❌ ᴄʟᴏsᴇ", callback_data="depinstall:close", style=_BS.DANGER if _BS else None),
    ])
...
async def _install_one(key: str, target_message: Message):
    ...
    async def status_cb(tail):
        try:
            await safe_edit(target_message.edit_text, 
    ...
    if still_ok:
        result = f"{E_CHECK} <b>ɪɴsᴛᴀʟʟᴇᴅ.</b> Run /restart so plugins that use it pick it up."
    else:
        result = f"{E_CROSS} <b>ɪɴsᴛᴀʟʟ ғᴀɪʟᴇᴅ</b> — see the output above, or check /logs."
    await safe_edit(target_message.edit_text, 
        f"{icon} <b>{label}</b>\n<code>{key}</code>\n\n{result}",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def install_cmd(client: Client, message: Message):
    ...
    if arg == "all":
        ...
        if not missing:
            return await safe_edit(status_msg.edit_text, f"{E_CHECK} <b>ᴇᴠᴇʀʏᴛʜɪɴɢ's ᴀʟʀᴇᴀᴅʏ ɪɴsᴛᴀʟʟᴇᴅ.</b>", parse_mode=enums.ParseMode.HTML)
        for key in missing:
    ...
    if arg not in PIP_DEPS and arg not in APT_DEPS:
        return await safe_edit(status_msg.edit_text, 
            f"{E_CROSS} <b>ᴜɴᴋɴᴏᴡɴ ᴋᴇʏ:</b> <code>{arg}</code>. Run /deps or /install with no args to see valid keys.",
            parse_mode=enums.ParseMode.HTML,
        )
...
async def deps_callback(client: Client, callback_query: CallbackQuery):
    ...
    if action == "refresh":
        await safe_edit(callback_query.message.edit_text, _status_text(), reply_markup=_status_buttons(), parse_mode=enums.ParseMode.HTML)
    ...
    if action == "all":
        missing = _missing_keys()
        if not missing:
            return await safe_edit(callback_query.message.edit_text, f"{E_CHECK} <b>ᴇᴠᴇʀʏᴛʜɪɴɢ's ᴀʟʀᴇᴀᴅʏ ɪɴsᴛᴀʟʟᴇᴅ.</b>", parse_mode=enums.ParseMode.HTML)
        for key in missing:
