...
async def install_cmd(client: Client, message: Message):
    ...
    if arg == "all":
        ...
        if not missing:
            return await safe_edit(status_msg.edit_text, f"{E_CHECK} <b>Everything's already installed.</b>", parse_mode=enums.ParseMode.HTML)
        for key in missing:
...
async def deps_callback(client: Client, callback_query: CallbackQuery):
    ...
    if action == "all":
        missing = _missing_keys()
        if not missing:
            return await safe_edit(callback_query.message.edit_text, f"{E_CHECK} <b>Everything's already installed.</b>", parse_mode=enums.ParseMode.HTML)
        for key in missing:
