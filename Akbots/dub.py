...
async def dub_command(client: Client, message: Message):
    ...
    if not has_video:
        return await message.reply_text(
            ...
            f"<b>Cloned voices:</b> <code>male1</code>, <code>male2</code>, <code>female</code> "
            f"(falls back to a stock voice if omitted or cloning isn't installed)\n"
            f"<b>speed:</b> 0.8–1.3 (default 1.0, only affects pacing, not pitch)\n"
            f"<b>expressiveness:</b> 0.4–1.0 (default 0.75 — higher = more emotive/varied, "
    ...
    try:
        video_path = await video_msg.download(file_name=os.path.join(folder, "input.mp4"))

        await status.edit_text(f"<b>{E_GEAR} Extracting audio...</b>", parse_mode=enums.ParseMode.HTML)
        audio_path = os.path.join(folder, "audio.wav")
        await asyncio.to_thread(_extract_audio, video_path, audio_path)

        await status.edit_text(f"<b>{E_GEAR} Transcribing (can take a bit for longer videos)...</b>", parse_mode=enums.ParseMode.HTML)
        model = await _get_model()
        transcription = await asyncio.to_thread(_transcribe_sync, model, audio_path)
        text = transcription["text"]
        if not text:
            return await status.edit_text(f"<b>{E_CROSS} Couldn't detect any speech in this video.</b>", parse_mode=enums.ParseMode.HTML)

        await status.edit_text(f"<b>{E_GEAR} Translating to '{lang}'...</b>", parse_mode=enums.ParseMode.HTML)
        ...
        await status.edit_text(f"<b>{E_CROSS} Dubbing failed:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
...
async def _generate_and_send(client: Client, session: dict):
    ...
    await status.edit_text(f"<b>{E_GEAR} Generating dubbed voice ({voice_label})...</b>", parse_mode=enums.ParseMode.HTML)
    dub_audio_path = await _generate_dub_audio(translated, lang, voice, folder, temperature=temperature, speed=speed)

    await status.edit_text(f"<b>{E_GEAR} Merging into video...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    await status.edit_text(f"<b>{E_GEAR} Uploading preview...</b>", parse_mode=enums.ParseMode.HTML)
    keyboard = InlineKeyboardMarkup([[
        make_button("✅ Accept", callback_data="dubreview_accept", style=_BS.SUCCESS if _BS else None),
        make_button("🔄 Regenerate", callback_data="dubreview_regen", style=_BS.PRIMARY if _BS else None),
        make_button("❌ Cancel", callback_data="dubreview_cancel", style=_BS.DANGER if _BS else None),
    ], [
        make_button("📝 Get SRT (original text)", callback_data="dubreview_srt", style=_BS.PRIMARY if _BS else None),
    ]])
    preview = await client.send_video(
        session["chat_id"], out_path,
        caption=f"<b>{E_CHECK} Dubbed preview ({lang}, {voice_label})</b>\n"
                f"Accept to keep it, Regenerate for a new take on the same text, or Cancel.",
...
async def dub_review_callback(client: Client, callback_query: CallbackQuery):
    ...
    if choice == "accept":
        ...
        await client.send_document(
            ...
            caption=f"<b>{E_CHECK} Original-language transcript (SRT)</b>",
...
async def dubpdf_command(client: Client, message: Message):
    ...
    try:
        await doc_msg.download(file_name=pdf_path)

        await status.edit_text(f"<b>{E_GEAR} Reading PDF text (OCR fallback if scanned)...</b>", parse_mode=enums.ParseMode.HTML)
        ...
        await status.edit_text(f"<b>{E_GEAR} Translating {len(blocks)} block(s) to '{lang}'...</b>", parse_mode=enums.ParseMode.HTML)
        ...
        await status.edit_text(f"<b>{E_GEAR} Rebuilding PDF...</b>", parse_mode=enums.ParseMode.HTML)
        await asyncio.to_thread(_pdf_rebuild_sync, pdf_path, out_path, blocks)

        await status.edit_text(f"<b>{E_GEAR} Uploading...</b>", parse_mode=enums.ParseMode.HTML)
        await client.send_document(
            message.chat.id, out_path,
            caption=f"<b>{E_CHECK} PDF translated ({lang})</b>",
        ...
        await status.edit_text(f"<b>{E_CROSS} PDF translation failed:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    finally:
        shutil.rmtree(folder, ignore_errors=True)
