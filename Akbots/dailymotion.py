...
from Akbots.direct_utils import upload_file, format_progress, E_CHECK, E_CROSS, E_INFO, E_ROCKET, E_BOLT, safe_edit, strip_ansi
...
def _extract_formats_sync(stream_url: str):
    ...
    BUG FIX: Dailymotion's CDN 403s any request that doesn't carry a
    Referer: https://www.dailymotion.com/ header (and a normal browser
    User-Agent) — including requests for the .m3u8 manifest itself, not
    just the page. Without http_headers here, yt-dlp fetched the manifest
    bare and got "[generic] <id>: Unable to download webpage: HTTP Error
    403: Forbidden" every time. Reusing this file's own HEADERS (already
    used for the metadata API call above) fixes it."""
    if yt_dlp is None:
        return []
    with yt_dlp.YoutubeDL({"quiet": True, "no_warnings": True, "http_headers": HEADERS}) as ydl:
...
def _quality_keyboard(session_id: str, formats):
    rows = [
        [make_button(f["ʟᴀʙᴇʟ"], callback_data=f"dmq:{session_id}:{f['id']}", style=_BS.PRIMARY if _BS else None)]
        for f in formats[:20]
    ]
    if not formats:
        rows.append([make_button("🚀 ᴅᴏᴡɴʟᴏᴀᴅ (ʙᴇsᴛ ᴀᴠᴀɪʟᴀʙʟᴇ)", callback_data=f"dmq:{session_id}:best", style=_BS.SUCCESS if _BS else None)])
    rows.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"dmcancel:{session_id}", style=_BS.DANGER if _BS else None)])
...
async def _handle(client: Client, message: Message, url: str):
    ...
    try:
        ...
        # Listing individual bitrate variants failed (Dailymotion's CDN can
        # 403 the manifest fetch even with the Referer/UA fix below, e.g.
        # from a datacenter IP it's decided to block outright) — that
        # doesn't necessarily mean the actual download will fail too, so
        # degrade to a single "best" button instead of aborting entirely.
        formats = []
...
async def _edit_status(status_msg: Message, text: str):
    """Works whether status_msg is a plain text message or a photo message."""
    if status_msg.photo:
        return await safe_edit(status_msg.edit_caption, text, parse_mode=enums.ParseMode.HTML)
    return await safe_edit(status_msg.edit_text, text, parse_mode=enums.ParseMode.HTML)


def _make_download_progress_hook(status: Message, loop: asyncio.AbstractEventLoop,
                                  file_name=None, quality=None):
    """yt-dlp calls progress_hooks synchronously from the worker thread doing
    the actual download (see _download_sync, run via asyncio.to_thread), so
    we can't `await` a status edit here directly — hand it back to the main
    event loop with run_coroutine_threadsafe instead. Throttled the same way
    as ytdl.py's version so it doesn't hammer Telegram's edit rate limit."""
...
def _download_sync(session: dict, out_dir: str, fmt_id: str, progress_hook=None):
    ...
    opts = {
        ...
        "http_headers": HEADERS,  # same CDN-403 fix as _extract_formats_sync above
    }
    if progress_hook:
...
async def _start_download(client: Client, cq: CallbackQuery, session_id: str, fmt_id: str):
    ...
    try:
        async with task_manager.queue_slot(cq.from_user.id, status_msg=status):
            if status.photo:
                await safe_edit(status.edit_caption, f"<b>{E_ROCKET} Downloading...</b>", parse_mode=enums.ParseMode.HTML)
            else:
                await safe_edit(status.edit_text, f"<b>{E_ROCKET} Downloading...</b>", parse_mode=enums.ParseMode.HTML)

            # BUG FIX: Dailymotion's m3u8 manifest URL is a time-limited signed
            # link. It was resolved once (in _handle) when the quality picker
            # was first shown, and if the user takes even a little while to tap
            # a quality button, that cached URL can expire by download time —
            # surfacing as "unable to download video data: HTTP Error 404: Not
            # Found" even though the SAME link worked fine seconds earlier for
            # listing qualities. Re-resolving right before the real download
            # gets a fresh, valid signed URL every time instead of reusing a
            # possibly-stale one from the session.
            try:
                fresh = await _resolve(session["page_url"])
                session["stream_url"] = fresh["m3u8_url"]
            except Exception:
                pass  # fall back to the cached URL if a fresh resolve fails

            quality_label_for_hook = next(
                (f["label"] for f in session.get("formats", []) if f["id"] == fmt_id),
                None
            )
            loop = asyncio.get_running_loop()
            hook = _make_download_progress_hook(
                status, loop, file_name=session.get("title"), quality=quality_label_for_hook
            )
            filepath = await asyncio.to_thread(_download_sync, session, session_dir, fmt_id, hook)
            ...
            caption = f"<blockquote><b>{E_CHECK} {title[:100]}</b></blockquote>"
        ...
        try:
            if status.photo:
                await safe_edit(status.edit_caption, err_text, parse_mode=enums.ParseMode.HTML)
            else:
                await safe_edit(status.edit_text, err_text, parse_mode=enums.ParseMode.HTML)
        except Exception:
            pass
    finally:
