...
async def _generate_and_reply(client: Client, message: Message, media_msg: Message):
    ...
    caption = (
        f"✅ <b>ʏᴏᴜʀ ʟɪɴᴋ ɪs ʀᴇᴀᴅʏ!</b>\n\n"
        f"📄 <b>ғɪʟᴇ:</b> <code>{file_name}</code>\n"
        f"💾 <b>sɪᴢᴇ:</b> {file_size}\n\n"
        f"🎬 <b>sᴛʀᴇᴀᴍ:</b> {links['stream']}\n"
        f"⬇️ <b>ᴅᴏᴡɴʟᴏᴀᴅ:</b> {links['download']}"
    )
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [
                make_button("▶️ sᴛʀᴇᴀᴍ", url=links["stream"], style=_BS.PRIMARY if _BS else None),
                make_button("⬇️ ᴅᴏᴡɴʟᴏᴀᴅ", url=links["download"], style=_BS.PRIMARY if _BS else None),
            ],
            [make_button("✖️ ᴄʟᴏsᴇ", callback_data="ftl_close", style=_BS.DANGER if _BS else None)],
        ]),
    )
...
async def link_cmd(client: Client, message: Message):
    ...
    try:
        ...
        # Without this, any unexpected crash here (bad STREAM_BIN_CHANNEL
        # permissions, a DB hiccup, a Telegram API error) left the user with
        # zero response and nothing to go on — this guarantees /link always
        # says *something* instead of silently doing nothing.
        logger.exception(f"[filetolink] /link crashed: {e}")
        try:
            await message.reply_text(f"❌ Something went wrong: {e}", quote=True)
        except Exception:
            pass
