...
async def set_watermark_cmd(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ᴜsᴀɢᴇ:</b> <code>/set_watermark Your Text</code>\n\n"
...
async def see_watermark_cmd(client: Client, message: Message):
    ...
    if text:
        await message.reply_text(
            ...
            f"<b>ᴘᴏsɪᴛɪᴏɴ:</b> {POSITION_LABELS.get(position, position)}",
            parse_mode=enums.ParseMode.HTML
        )
    else:
...
async def apply_watermark_cmd(client: Client, message: Message):
    ...
    if not text:
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ɴᴏ ᴡᴀᴛᴇʀᴍᴀʀᴋ ᴛᴇxᴛ sᴇᴛ!</b>\n\n"
