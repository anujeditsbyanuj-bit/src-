...
async def domains_command(client: Client, message: Message):
    ...
    header = (
        f"<b>☰ sᴜᴘᴘᴏʀᴛᴇᴅ sɪᴛᴇs ɪɴ ᴛʜɪs ʙᴏᴛ!</b>\n"
...
@Client.on_message(filters.command(["akstats"]) & filters.user(ADMINS))
async def ak_stats_command(client: Client, message: Message):
    try:
        from database.db import db
        bypass_count = await db.get_fs_config("akbots_bypass_count", 0) or 0
    except Exception:
        bypass_count = "N/A"

    await message.reply_text(
        f"<b>📊 ᴀᴋʙᴏᴛs ʙʏᴘᴀss sᴛᴀᴛs</b>\n\n"
        f"✅ <b>ᴛᴏᴛᴀʟ ʙʏᴘᴀssᴇs:</b> <code>{bypass_count}</code>\n"
        f"💬 <b>ᴛʀᴀᴄᴋᴇᴅ ᴀᴄᴛɪᴠᴇ ᴄʜᴀᴛs:</b> <code>{len(_last_activity)}</code>\n"
        f"🌐 <b>sᴜᴘᴘᴏʀᴛᴇᴅ ᴅᴏᴍᴀɪɴs:</b> <code>{len(supported_domains())}</code> direct + <code>{supported_site_count()}</code> browser-tier\n"
        f"⚡ <b>ғᴀsᴛ ʙʏᴘᴀss ᴇɴɢɪɴᴇ:</b> <code>{'available' if bypass_available() else 'unavailable'}</code>",
        parse_mode=enums.ParseMode.HTML,
    )
