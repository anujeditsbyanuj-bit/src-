...
def _menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            make_button(" 🖼 ᴜᴘʟᴏᴀᴅ ɪᴍᴀɢᴇ ", callback_data="i2l_menu_image",
                        style=_BS.PRIMARY if _BS else None),
            make_button(" 🔗 ᴜᴘʟᴏᴀᴅ ᴜʀʟ ", callback_data="i2l_menu_url",
                        style=_BS.PRIMARY if _BS else None),
        ],
        [make_button(" ⏰ sᴇᴛ ᴇxᴘɪʀʏ ", callback_data="i2l_menu_expiry",
                     style=_BS.PRIMARY if _BS else None)],
    ])
...
def _expiry_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            make_button(" ⏰ 1 ʜᴏᴜʀ ",  callback_data="i2l_exp_3600",    style=_BS.PRIMARY if _BS else None),
            make_button(" 📅 1 ᴅᴀʏ ",   callback_data="i2l_exp_86400",   style=_BS.PRIMARY if _BS else None),
        ],
        [
            make_button(" 📅 7 ᴅᴀʏs ",  callback_data="i2l_exp_604800",  style=_BS.PRIMARY if _BS else None),
            make_button(" 📅 30 ᴅᴀʏs ", callback_data="i2l_exp_2592000", style=_BS.PRIMARY if _BS else None),
        ],
        [make_button(" ∞ ɴᴇᴠᴇʀ ", callback_data="i2l_exp_never", style=_BS.PRIMARY if _BS else None)],
        [make_button(" ⬅️ ʙᴀᴄᴋ ", callback_data="i2l_back", style=_BS.DANGER if _BS else None)],
    ])


def _result_kb(direct_url: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        make_button(" 📋 ᴏᴘᴇɴ ʟɪɴᴋ ", url=direct_url, style=_BS.PRIMARY if _BS else None),
        make_button(" ↗️ sʜᴀʀᴇ ʟɪɴᴋ ", url=f"https://t.me/share/url?url={direct_url}",
                    style=_BS.PRIMARY if _BS else None),
    ]])
...
def _build_result_text(r: dict, user_name: str, user_id: int, elapsed: float) -> str:
    return (
        "<blockquote>"
        f"{E_CHECK} <b>ɪᴍᴀɢᴇ ᴜᴘʟᴏᴀᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ!</b>\n\n"
        f"{E_IMAGE} <b>ɴᴀᴍᴇ:</b> {r['name']}\n"
        f"{E_LINK} <b>ᴅɪʀᴇᴄᴛ ᴜʀʟ:</b> <a href=\"{r['url']}\">{r['url']}</a>\n"
        f"{E_INFO} <b>ᴠɪᴇᴡᴇʀ:</b> <a href=\"{r['viewer']}\">{r['viewer']}</a>\n"
        f"{E_INFO} <b>sɪᴢᴇ:</b> {r['width']}x{r['height']} | {r['size_kb']} KB\n"
        f"{E_PENCIL} <b>ғɪʟᴇ:</b> {r['filename']} ({r['ext']})\n"
        f"{E_EXPIRY} <b>ᴇxᴘɪʀʏ:</b> {r['expiry']}\n"
        f"{E_CLOCK} <b>ᴛɪᴍᴇ:</b> {r['upload_time']} UTC\n\n"
        f"{E_ROCKET} <b>ᴛᴏᴏᴋ:</b> {elapsed:.1f}s"
        "</blockquote>"
    )
...
async def i2l_menu_image_cb(client: Client, cb: CallbackQuery):
    uid = cb.from_user.id
    _PENDING[uid] = "image"
    await safe_edit(cb.edit_message_text, 
        f"{E_IMAGE} <b>sᴇɴᴅ ᴍᴇ ᴀɴ ɪᴍᴀɢᴇ ɴᴏᴡ</b> (photo or file).\n"
...
async def i2l_menu_url_cb(client: Client, cb: CallbackQuery):
    ...
    _PENDING[uid] = "url"
    await safe_edit(cb.edit_message_text, 
        f"{E_LINK} <b>sᴇɴᴅ ᴍᴇ ᴀɴ ɪᴍᴀɢᴇ ᴜʀʟ ɴᴏᴡ.</b>\n"
        f"{E_TIP} Send /cancel to abort.",
        parse_mode=enums.ParseMode.HTML,
    )
    await cb.answer()
...
async def i2l_menu_expiry_cb(client: Client, cb: CallbackQuery):
    uid = cb.from_user.id
    await safe_edit(cb.edit_message_text, 
        f"{E_EXPIRY} <b>sᴇʟᴇᴄᴛ ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ ᴛɪᴍᴇʀ</b>\n\n"
...
async def i2l_incoming_media(client: Client, message: Message):
    ...
    if _PENDING.get(uid) != "image":
        # Not our concern — let it fall through to lower-priority groups
        # (e.g. direct_utils.wait_for_reply's group=-1 handler used by the
        # "Add Bot Using Token" flow, cookies_manager.py, etc.). Without
        # this, a bare `return` here silently swallows EVERY private
        # photo/document whenever no imgtolink session is pending.
...
async def i2l_incoming_url(client: Client, message: Message):
    ...
    if _PENDING.get(uid) != "url":
        # Not our concern — MUST continue propagation, not just `return`.
        # This filter matches almost every private text message (any text
        # that isn't one of a handful of imgtolink commands), and it runs
        # at group=-3 — before direct_utils.wait_for_reply's group=-1
        # handler. A bare `return` here silently ate every bot-token
        # message sent for the "Add Bot Using Token" flow (and any other
        # text-based conversation flow in the bot), since nothing lower
        # ever got a chance to see the message.
        message.continue_propagation()
