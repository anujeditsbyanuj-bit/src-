...
E_PLAY = '<emoji id=5215516614769609401>▶️</emoji>'
E_BOLT = '⚡'
...
async def meow_callback(client: Client, query: CallbackQuery):
    ...
    if payload["kind"] == "details":
        ...
        for ep in episodes[:40]:
            token = _store({"provider": payload["provider"], "kind": "stream",
            ...
            buttons.append([make_button(f"S{season}E{number} · {title}", callback_data=f"meow:{token}",
                                         style=_style("SECONDARY"))])
...
async def _resolve_and_reply(query: CallbackQuery, provider, label: str, movie_id: str, episode_id: str,
    ...
    buttons = [[make_button(f"{E_PLAY} {q.get('quality') or 'Play'}", url=q["url"],
...
async def _meowly_send_watch_links(query: CallbackQuery, details: dict, media_type: str,
    ...
    buttons = [[make_button(f"{E_PLAY} {l['name']}", url=l["url"], style=_style("PRIMARY"))] for l in links]
