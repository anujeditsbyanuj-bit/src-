...
def _build_track_keyboard(status_id: int) -> InlineKeyboardMarkup:
    ...
    rows.append([make_button("✅ ᴅᴏɴᴇ", callback_data=f"asel:done:{status_id}", style=_BS.SUCCESS if _BS else None)])
    rows.append([make_button(f"{E_CROSS} ᴄᴀɴᴄᴇʟ", callback_data=f"asel:cancel:{status_id}", style=_BS.PRIMARY if _BS else None)])
...
def _build_format_keyboard(status_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [make_button(f"{E_CROSS} ᴄᴀɴᴄᴇʟ", callback_data=f"asel:cancel:{status_id}", style=_BS.PRIMARY if _BS else None)],
    ])
