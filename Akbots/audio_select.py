...
def _build_track_keyboard(status_id: int) -> InlineKeyboardMarkup:
    ...
    for idx, label in sess["tracks"]:
        mark = "✔️ " if idx in sess["selected"] else ""
        rows.append([make_button(f"{mark}{label}", callback_data=f"asel:trk:{status_id}:{idx}", style=_BS.PRIMARY if _BS else None)])
    rows.append([make_button("✅ ᴅᴏɴᴇ", callback_data=f"asel:done:{status_id}", style=_BS.PRIMARY if _BS else None)])
    rows.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"asel:cancel:{status_id}", style=_BS.PRIMARY if _BS else None)])
    return InlineKeyboardMarkup(rows)


def _build_format_keyboard(status_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [make_button("🎬 ᴠɪᴅᴇᴏ (.ᴍᴘ4)", callback_data=f"asel:fmt:{status_id}:video", style=_BS.PRIMARY if _BS else None)],
        [make_button("📄 ᴅᴏᴄᴜᴍᴇɴᴛ (.ᴍᴋᴠ)", callback_data=f"asel:fmt:{status_id}:mkv", style=_BS.PRIMARY if _BS else None)],
        [make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"asel:cancel:{status_id}", style=_BS.PRIMARY if _BS else None)],
    ])
...
async def audio_select_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_MUSIC} <b>ᴀᴜᴅɪᴏ ᴛʀᴀᴄᴋ sᴇʟᴇᴄᴛᴏʀ</b>\n\n"
            f"Reply to a <b>ᴠɪᴅᴇᴏ ғɪʟᴇ</b> with <code>/audio_select</code> to choose "
    ...
    if replied.document:
        ...
        if not file_name.lower().endswith(VIDEO_EXTS):
            return await message.reply_text(
                f"<blockquote>{E_CROSS} <b>ɴᴏᴛ ᴀ ᴠɪᴅᴇᴏ ғɪʟᴇ!</b>\n\n"
    ...
    if await db.check_audio_select_limit(user_id):
        ...
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ᴅᴀɪʟʏ ʟɪᴍɪᴛ ʀᴇᴀᴄʜᴇᴅ.</b>\n\n"
    ...
    if user_id in _USER_ACTIVE:
        ...
        return await message.reply_text(
            f"<blockquote>{E_MUSIC} <b>ʏᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴀ ᴊᴏʙ ʀᴜɴɴɪɴɢ.</b>\n\n"
...
async def audio_select_callback(client: Client, cq: CallbackQuery):
    ...
    if action == "trk":
        ...
        await safe_edit(cq.message.edit_reply_markup, _build_track_keyboard(status_id))
...
async def audio_queue_cmd(client: Client, message: Message):
    ...
    if status_id:
        ...
        lines.append(f"{E_BOLT} <b>ʀᴜɴɴɪɴɢ:</b> {name}")
    if queue:
        lines.append("")
    ...
    limit = await db.get_audio_select_limit(user_id)
    lines.append("")
    lines.append(f"{E_TIP} <b>ᴛᴏᴅᴀʏ's ᴜsᴀɢᴇ:</b> <code>{usage}/{limit}</code>")
...
async def audio_defaults_cmd(client: Client, message: Message):
    ...
    if not args:
        ...
        return await message.reply_text(
            ...
            f"<b>ᴅᴇғᴀᴜʟᴛ ғɪʟᴇɴᴀᴍᴇ:</b> {name or 'Not set (keeps original name)'}\n"
            f"<b>ᴄᴀᴘᴛɪᴏɴ:</b> {'Set (via /set_caption)' if caption else 'Not set'}\n"
            f"<b>ᴛᴏᴅᴀʏ's ᴜsᴀɢᴇ:</b> <code>{usage}/{limit}</code></blockquote>\n\n"
...
async def get_id_cmd(client: Client, message: Message):
    chat = message.chat
    lines = [f"<b>ᴄʜᴀᴛ ɪᴅ:</b> <code>{chat.id}</code>", f"<b>ᴄʜᴀᴛ ᴛʏᴘᴇ:</b> <code>{chat.type}</code>"]
    if message.reply_to_message and message.reply_to_message.from_user:
        u = message.reply_to_message.from_user
        lines.append(f"<b>ʀᴇᴘʟɪᴇᴅ ᴜsᴇʀ ɪᴅ:</b> <code>{u.id}</code>")
