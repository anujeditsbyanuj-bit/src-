...
from Akbots.direct_utils import (
    ...
    make_ffmpeg_progress_parser, get_video_metadata, VIDEO_EXTS,
)
...
async def _start_job(client: Client, message: Message):
    ...
    try:
        ...
        await status.edit_text(f"<b>{E_CROSS} Download failed:</b> <code>{e}</code>",
                                parse_mode=enums.ParseMode.HTML)
    ...
    if not tracks:
        shutil.rmtree(temp_dir, ignore_errors=True)
        await status.edit_text(f"<b>{E_CROSS} No audio tracks found in this video.</b>",
                                parse_mode=enums.ParseMode.HTML)
        return await _release_and_advance(client, user_id)
...
async def audio_select_callback(client: Client, cq: CallbackQuery):
    ...
    if action == "cancel":
        ...
        await cq.message.edit_text(f"<b>{E_CROSS} Cancelled and temporary files deleted.</b>",
                                    parse_mode=enums.ParseMode.HTML)
    ...
    if action == "fmt":
        ...
        await cq.message.edit_text(f"<b>{E_BOLT} Selecting audio tracks...</b>", parse_mode=enums.ParseMode.HTML)
...
async def _process_and_upload(client: Client, status: Message, status_id: int, fmt: str):
    ...
    try:
        ...
        caption = default_caption if default_caption else f"<b>{E_MUSIC} {base_name}{ext}</b>"
        await upload_file(
        ...
        with contextlib.suppress(Exception):
            await status.edit_text(f"<b>{E_CROSS} Job cancelled.</b>", parse_mode=enums.ParseMode.HTML)
        raise
    finally:
        await _cleanup_session(status_id)
