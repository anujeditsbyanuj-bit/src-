...
async def start_amerge(client: Client, message: Message):
    ...
    if user_id in _MERGE_SESSIONS:
        ...
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ᴀɴ ᴀᴜᴅɪᴏ ᴍᴇʀɢᴇ sᴇssɪᴏɴ ɪs ᴀʟʀᴇᴀᴅʏ ʀᴜɴɴɪɴɢ.</b>\n\n"
    ...
    await message.reply_text(
        f"<blockquote>{E_MUS} <b>ᴀᴜᴅɪᴏ ᴍᴇʀɢᴇ — sᴇssɪᴏɴ sᴛᴀʀᴛᴇᴅ</b>\n\n"
...
async def amerge_status(client: Client, message: Message):
    ...
    await message.reply_text(f"<blockquote>{E_GEAR} <b>ᴀᴜᴅɪᴏ ᴍᴇʀɢᴇ sᴇssɪᴏɴ</b>\n\n{listing}</blockquote>",
                              parse_mode=enums.ParseMode.HTML)
...
async def cancel_amerge(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_TRASH} <b>ᴀᴜᴅɪᴏ ᴍᴇʀɢᴇ sᴇssɪᴏɴ ᴄᴀɴᴄᴇʟʟᴇᴅ.</b>\n"
...
async def finish_amerge(client: Client, message: Message):
    ...
    if not ok:
        await safe_edit(status.edit_text, f"<b>{E_GEAR} Fast merge failed (mismatched sources) — re-encoding instead...</b>",
                                parse_mode=enums.ParseMode.HTML)
    ...
    if not ok:
        _cleanup(user_id)
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Merge failed.</b>\n\n<code>{tail[-300:]}</code>",
                                       parse_mode=enums.ParseMode.HTML)
