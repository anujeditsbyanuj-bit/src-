...
from Akbots.direct_utils import (
    ...
    E_CHECK, E_CROSS, E_INFO, _looks_like_html_error, _extract_html_reason,
    play_download_intro,
)
...
async def aria2c_download(url: str, task_folder: str, status, label: str = "Downloading",
    ...
    # aria2c only fails on a bad HTTP status/connection error — a host that
    # returns HTTP 200 with an HTML "access denied"/login/expired-link page
    # instead of the real file looks like a totally successful download to
    # it, and it happily saves that HTML as the output file. Peek at the
    # start of the file ourselves and catch that case here, the same way
    # stream_download() already does for the non-aria2c path.
    try:
    ...
    if _looks_like_html_error(head):
        reason = _extract_html_reason(head)
        try:
            os.remove(path)
        except Exception:
            pass
        raise RuntimeError(f"Download failed — {reason}.")
