...
class Database:
    ...
    def _init_database(self):
        ...
        # Generic persistent key/value cache (with TTL) — used by hdhub.py to
        # persist /hdhub listing and link-list data (the callback_data behind
        # its buttons) so a bot restart between "show buttons" and "user taps
        # one" doesn't turn every button into a dead "expired, run /hdhub
        # again" — same table survives it, unlike the plain in-memory dict.
        cursor.execute('''
    ...
    def set_cache_value(self, key: str, value: str, ttl_seconds: int = 86400):
        """Store a value under `key`, expiring after ttl_seconds."""
        expires_at = datetime.utcnow().timestamp() + ttl_seconds
        expires_at_iso = datetime.utcfromtimestamp(expires_at).strftime('%Y-%m-%d %H:%M:%S')
        cursor = self.conn.cursor()
        cursor.execute('''
            INSERT OR REPLACE INTO ui_cache (key, value, expires_at)
            VALUES (?, ?, ?)
        ''', (key, value, expires_at_iso))
        self.conn.commit()

    def get_cache_value(self, key: str) -> Optional[str]:
        """Fetch a value stored with set_cache_value, or None if missing/expired."""
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT value FROM ui_cache
    ...
    def clear_expired_cache(self):
        """Drop expired ui_cache rows — called opportunistically, not on a
        schedule, so this table never grows unbounded."""
        cursor = self.conn.cursor()
        cursor.execute("DELETE FROM ui_cache WHERE expires_at <= datetime('now')")
        self.conn.commit()
