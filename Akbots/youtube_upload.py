...
from Akbots.direct_utils import make_output_folder, safe_filename, VIDEO_EXTS
...
async def ytupload_cmd(client: Client, message: Message):
    ...
    if not is_video or len(message.command) < 2:
        return await message.reply_text(
            f"<blockquote>{E_INFO} Reply to a <b>video</b> with:\n"
    ...
    await status.edit_text(f"<b>{E_YT} Uploading to YouTube ({privacy})...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    try:
        ...
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} YouTube API error:</b> <code>{e}</code>",
                                       parse_mode=enums.ParseMode.HTML)
    except Exception as e:
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} Upload failed:</b> <code>{e}</code>",
                                       parse_mode=enums.ParseMode.HTML)

    shutil.rmtree(temp_dir, ignore_errors=True)
    video_id = response.get("id") if response else None
    if not video_id:
        return await status.edit_text(f"<b>{E_CROSS} Upload finished but no video ID returned.</b>",
                                       parse_mode=enums.ParseMode.HTML)
