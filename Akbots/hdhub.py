...
# Kept in-memory for fast access within this process, AND write-through
# persisted to the bot's shared MongoDB (database/db.py's set_ui_cache/
# get_ui_cache — see there) — so if the bot process restarts between "show
# buttons" and "user taps one" (a redeploy, a Render/Railway free-tier
# sleep-wake, a crash, ...), the tap still resolves instead of every
# button dying with "expired, run /hdhub again".
#
# This USED to write through to hdhub_lib/database.py's bundled SQLite
# file instead. That looked durable but wasn't: on Render/Railway/Heroku/
# most container hosts the local filesystem doesn't survive a redeploy
# (sometimes not even a plain restart), so the "restart-proof" fallback
# was silently a no-op there — this is exactly the "link list expired"
# bug from a fresh deploy. MongoDB (same cluster as everything else in
# this bot) actually does survive that. Entries expire after
# _CACHE_TTL_SECONDS either way.
_LISTINGS: dict = {}
_LINKS: dict = {}
_CACHE_TTL_SECONDS = 24 * 3600


async def _save_listing(listing_id: str, items: list):
    _LISTINGS[listing_id] = items
    try:
        await db.set_ui_cache(f"hdhub_listing:{listing_id}", json.dumps(items), _CACHE_TTL_SECONDS)
    except Exception as e:
        logger.warning(f"hdhub: failed to persist listing {listing_id}: {e}")
...
async def _load_listing(listing_id: str):
    ...
    if items is not None:
        return items
    try:
        raw = await db.get_ui_cache(f"hdhub_listing:{listing_id}")
        if raw:
            items = json.loads(raw)
            _LISTINGS[listing_id] = items
            return items
    except Exception as e:
        logger.warning(f"hdhub: failed to load persisted listing {listing_id}: {e}")
    return None


async def _save_links(links_id: str, links: list):
    _LINKS[links_id] = links
    try:
        await db.set_ui_cache(f"hdhub_links:{links_id}", json.dumps(links), _CACHE_TTL_SECONDS)
    except Exception as e:
        logger.warning(f"hdhub: failed to persist links {links_id}: {e}")


async def _load_links(links_id: str):
    links = _LINKS.get(links_id)
    if links is not None:
        return links
    try:
        raw = await db.get_ui_cache(f"hdhub_links:{links_id}")
        if raw:
            links = json.loads(raw)
            _LINKS[links_id] = links
            return links
    except Exception as e:
        logger.warning(f"hdhub: failed to load persisted links {links_id}: {e}")
    return None
...
async def _resolve_links_for_url(status: Message, title: str, url: str, links_id: str):
    ...
    await _save_links(links_id, links)
    buttons = [
...
async def hdhub_command(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        return await _handle_direct_link(client, message, url)
    ...
    await _save_listing(listing_id, items)
...
# Mirror/download-host domains — a link on one of these is already the
# FINAL page (what scraper.get_download_links() would have extracted from
# an HDhub4u article), not an article to scrape links out of. Passing one
# of these straight to the scraper always produces a false "No download
# links found" (its h3/h4/.entry-content selectors are built for HDhub4u's
# article layout, not these hosts' own pages) — route it straight to the
# same resolve+download flow the "hdl:" button callback uses instead.
_MIRROR_HOST_DOMAINS = (
    "hdstream4u", "hubstream", "hubdrive", "hubcloud",
    "hubcdn", "hblinks", "buzzserver",
)


async def _handle_direct_link(client: Client, message: Message, url: str):
    if any(d in url.lower() for d in _MIRROR_HOST_DOMAINS):
        status = await message.reply_text(f"<b>{E_GEAR} Resolving:</b>\n<code>{url}</code>", parse_mode=enums.ParseMode.HTML)
        return await _resolve_and_download(client, status, url)
...
async def _resolve_and_download(client: Client, status: Message, link: str):
    """Shared by the 'hdl:' button callback and a directly-pasted/typed
    mirror-host link — bypasses the mirror's ad-wall/crypt-token page, then
    dispatches to whichever existing plugin handles that resolved domain."""
...
async def hdhub_auto_detect(client: Client, message: Message):
    ...
    if url:
        await _handle_direct_link(client, message, url)
...
async def hdhub_link_callback(client: Client, query: CallbackQuery):
    ...
    links = await _load_links(links_id)
    ...
    await query.answer()
    await safe_edit(query.message.edit_text, f"<b>{E_GEAR} Resolving:</b>\n<code>{link}</code>", parse_mode=enums.ParseMode.HTML)
    await _resolve_and_download(client, query.message, link)
...
_QUALITY_LABELS = {
    "4K": "🎥 4ᴋ ᴜʜᴅ", "2160p": "🎥 4ᴋ ᴜʜᴅ", "1080p": "📺 1080ᴘ ғʜᴅ",
...
async def hdhub_status(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        f"<b>ɪɴᴛᴇʀᴠᴀʟ:</b> <code>{interval} min</code>\n"
        f"<b>ʀᴜɴɴɪɴɢ:</b> {'✅ Yes' if running else '❌ No'}\n"
        f"<b>ʟᴀsᴛ ᴘᴏsᴛ:</b> <code>{last_post}</code>",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def hdhub_stats(client: Client, message: Message):
    await message.reply_text(
        ...
        f"<b>ᴛᴏᴛᴀʟ ᴘᴏsᴛs:</b> <code>{_db.get_total_posts()}</code>\n"
        f"<b>ᴛᴏᴅᴀʏ:</b> <code>{_db.get_posts_count_today()}</code>\n"
        f"<b>ᴜɴɪǫᴜᴇ ᴄᴏɴᴛᴇɴᴛ:</b> <code>{_db.get_unique_content_count()}</code>\n"
        f"<b>ᴅʙ sɪᴢᴇ:</b> <code>{_db.get_size_mb():.2f} MB</code>",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def hdhub_forcepost(client: Client, message: Message):
    ...
    await safe_edit(status.edit_text, f"<b>{E_CHECK} Posted {posted} new item(s).</b>", parse_mode=enums.ParseMode.HTML)
