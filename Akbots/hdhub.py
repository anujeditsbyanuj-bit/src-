...
# persisted to _db's ui_cache table (see hdhub_lib/database.py) — so if the
# bot process restarts between "show buttons" and "user taps one" (a
# redeploy, a Render/Railway free-tier sleep-wake, a crash, ...), the tap
# still resolves instead of every button dying with "expired, run /hdhub
# again". Entries expire after _CACHE_TTL_SECONDS either way.
_LISTINGS: dict = {}
_LINKS: dict = {}
_CACHE_TTL_SECONDS = 24 * 3600


def _save_listing(listing_id: str, items: list):
    _LISTINGS[listing_id] = items
    if _db is not None:
        try:
            import json
            _db.set_cache_value(f"hdhub_listing:{listing_id}", json.dumps(items), _CACHE_TTL_SECONDS)
        except Exception as e:
            logger.warning(f"hdhub: failed to persist listing {listing_id}: {e}")


def _load_listing(listing_id: str):
    ...
    if _db is not None:
        try:
            import json
            raw = _db.get_cache_value(f"hdhub_listing:{listing_id}")
            if raw:
...
def _save_links(links_id: str, links: list):
    _LINKS[links_id] = links
    if _db is not None:
        try:
            import json
            _db.set_cache_value(f"hdhub_links:{links_id}", json.dumps(links), _CACHE_TTL_SECONDS)
        except Exception as e:
            logger.warning(f"hdhub: failed to persist links {links_id}: {e}")


def _load_links(links_id: str):
    ...
    if _db is not None:
        try:
            import json
            raw = _db.get_cache_value(f"hdhub_links:{links_id}")
            if raw:
...
async def hdhub_command(client: Client, message: Message):
    ...
    _save_listing(listing_id, items)
...
async def hdhub_item_callback(client: Client, query: CallbackQuery):
    ...
    items = _load_listing(listing_id)
