...
# in-memory per-user sessions: (bot identity, user_id) -> {"mode": str, "history": [...]}
...
# conversation history bleed into another's for the same owner. See
...
async def coach_command(client: Client, message: Message):
    ...
    if arg.lower() == "reset":
        _sessions.pop(_pk(client, user_id), None)
        await message.reply_text(f"<b>{E_CHECK} Session reset ho gaya.</b>", parse_mode=enums.ParseMode.HTML)
        return

    session = _sessions.setdefault(_pk(client, user_id), {"mode": "chat", "history": []})
