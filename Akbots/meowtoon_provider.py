...
async def search(query: str) -> list[dict]:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            res = await _get_json(session, f"{MAIN_URL}/api/search/suggestions?q={query}&limit=20")
            ...
            logger.debug(f"meowtoon: kartoons search failed: {e}")
    ...
    try:
        ...
        logger.debug(f"meowtoon: xon search failed: {e}")
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            for link in links:
                ...
                # given this link — same "blob:" convention the original
                # Next.js frontend used to signal client-side handling.
                decrypt_url = _to_stream_url(str(encoded))
                return {"videoUrl": f"blob:{decrypt_url}", "headers": {}, "qualities": []}
            return None
        except Exception:
            return None
