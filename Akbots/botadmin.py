...
# Shared "/" command menu list — used by /set (main bot, owner-triggered)
# AND by Akbots/titanium.py, which applies this same list automatically to
# every Titanium clone bot right after it starts, so clone owners get the
# full menu without needing ADMINS access to run /set themselves.
COMMAND_MENU = [
...
async def bot_stats(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_INFO} <b>ʙᴏᴛ sᴛᴀᴛɪsᴛɪᴄs</b>\n\n"
        f"🏓 <b>ᴘɪɴɢ:</b> <code>{ping_ms} ms</code>\n"
        f"👥 <b>ᴛᴏᴛᴀʟ ᴜsᴇʀs:</b> <code>{total_users}</code>\n"
        f"{E_CROWN} <b>ᴘʀᴇᴍɪᴜᴍ ᴜsᴇʀs:</b> <code>{premium_count}</code>\n"
        f"{E_CLOCK} <b>ᴜᴘᴛɪᴍᴇ:</b> <code>{_uptime()}</code>\n\n"
        f"🐍 <b>ᴘʏᴛʜᴏɴ:</b> <code>{sys.version.split()[0]}</code></blockquote>",
...
async def check_user_premium(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_CROWN} <b>ᴘʀᴇᴍɪᴜᴍ sᴛᴀᴛᴜs</b>\n\n"
        f"⚡ <b>ᴜsᴇʀ ɪᴅ:</b> <code>{user_id}</code>\n"
        f"{E_CLOCK} <b>ᴇxᴘɪʀᴇs:</b> <code>{expiry_text}</code>\n"
        f"{E_INFO} <b>ᴛɪᴍᴇ ʟᴇғᴛ:</b> <code>{left_text}</code></blockquote>",
        parse_mode=enums.ParseMode.HTML
    )
...
async def freez_expired_premium(client: Client, message: Message):
    ...
    await safe_edit(status.edit_text, 
        ...
        f"🧊 <b>ʀᴇᴍᴏᴠᴇᴅ (ᴇxᴘɪʀᴇᴅ):</b> <code>{len(removed)}</code>\n"
        f"➡️ <code>{removed_text}</code>\n\n"
        f"{E_CROWN} <b>sᴛɪʟʟ ᴀᴄᴛɪᴠᴇ:</b> <code>{kept}</code>",
        parse_mode=enums.ParseMode.HTML
    )
...
def _mode_view(current: str):
    ...
    text = (
        f"<blockquote>{E_GEAR} <b>ʙᴏᴛ ᴍᴏᴅᴇ</b>\n\n"
        f"<b>ᴄᴜʀʀᴇɴᴛ:</b> <code>{current.upper()}</code>\n\n"
        f"🆓 <b>ғʀᴇᴇᴍɪᴜᴍ</b> — everyone gets premium features free\n"
        f"{E_CROWN} <b>ᴘᴀɪᴅ</b> — only premium users get full access</blockquote>"
    )
    buttons = InlineKeyboardMarkup([[
        make_button(f"{free_mark} ꜰʀᴇᴇᴍɪᴜᴍ", callback_data="set_mode_freemium", style=_BS.PRIMARY if _BS else None),
        make_button(f"{paid_mark} ᴘᴀɪᴅ", callback_data="set_mode_paid", style=_BS.PRIMARY if _BS else None),
    ]])
    return text, buttons
...
async def ytdl_auth_status(client: Client, message: Message):
    ...
    lines = []
    if pot_installed:
        lines.append(f"{E_CHECK} <b>ᴘᴏ ᴛᴏᴋᴇɴ ᴘʟᴜɢɪɴ</b> — auto-generating tokens (primary)")
    else:
        lines.append(f"{E_CROSS} <b>ᴘᴏ ᴛᴏᴋᴇɴ ᴘʟᴜɢɪɴ</b> — not installed")

    if YT_COOKIES and os.path.exists(YT_COOKIES):
        lines.append(f"{E_CHECK} <b>ᴄᴏᴏᴋɪᴇs ғɪʟᴇ</b> — <code>{YT_COOKIES}</code>")
    else:
        lines.append(f"{E_WARN} <b>ᴄᴏᴏᴋɪᴇs</b> — not configured (fallback)")
    ...
    text = (
        ...
        f"{status}\n\n"
        f"<b>─── ʜᴏᴡ ɪᴛ ᴡᴏʀᴋs ───</b>\n"
        f"• <b>ᴘᴏ ᴛᴏᴋᴇɴ ᴘʟᴜɢɪɴ</b> (auto) — generates tokens in background, "
        f"no manual setup\n"
        f"• <b>ᴄᴏᴏᴋɪᴇs</b> (manual fallback) — only needed if PO tokens fail\n\n"
        f"<b>ɪғ ʏᴏᴜᴛᴜʙᴇ sᴛɪʟʟ ғᴀɪʟs:</b> upload a fresh <code>cookies.txt</code> "
...
async def set_bot_commands(client: Client, message: Message):
    await client.set_bot_commands(COMMAND_MENU)
...
def _build_admin_page(page: int):
    ...
    nav = []
    if page > 1:
        nav.append(make_button("⬅️ ᴘʀᴇᴠ", callback_data=f"admincmds:{page-1}", style=_BS.PRIMARY if _BS else None))
    if page < total_pages:
        nav.append(make_button("➡️ ɴᴇxᴛ", callback_data=f"admincmds:{page+1}", style=_BS.PRIMARY if _BS else None))
