...
def _quality_keyboard(session_id: str, tiers, minimal: bool = False) -> InlineKeyboardMarkup:
    ...
    rows.append([
        make_button("🎵 MP3 64kbps", callback_data=f"ytq:{session_id}:mp3_64", style=_BS.PRIMARY if _BS else None),
        make_button("🎵 MP3 128kbps", callback_data=f"ytq:{session_id}:mp3_128", style=_BS.PRIMARY if _BS else None),
    ])
    rows.append([make_button("🎵 MP3 320kbps", callback_data=f"ytq:{session_id}:mp3_320", style=_BS.PRIMARY if _BS else None)])
    rows.append([make_button("❌ Cancel", callback_data=f"ytq:{session_id}:cancel", style=_BS.DANGER if _BS else None)])
    ...
    if not tiers:
        if not notify_on_failure:
            try:
                ...
                pass
            return False
        await fetching.edit_text(f"<b>{E_CROSS} No downloadable video formats found.</b>", parse_mode=enums.ParseMode.HTML)
        return False
...
async def subtitle_command(client: Client, message: Message):
    ...
    try:
        ...
        await status.edit_text(f"<b>{E_ROCKET} Uploading subtitle & transcript...</b>", parse_mode=enums.ParseMode.HTML)
        await client.send_document(
            message.chat.id, sub_path, caption=f"<b>{E_CHECK} {title[:100]} — raw subtitle file</b>",
            reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML,
        )
        await client.send_document(
            message.chat.id, transcript_path, caption=f"<b>{E_CHECK} {title[:100]} — plain-text transcript</b>",
            reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML,
        )
        try:
            from database.db import db as _db
        ...
        await status.edit_text(f"<b>{E_CROSS} Failed to fetch subtitles:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    finally:
        shutil.rmtree(session_dir, ignore_errors=True)
