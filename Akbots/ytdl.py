...
from config import YTDL_MAX_FILESIZE, YT_COOKIES, INSTA_COOKIES, FB_COOKIES, VK_COOKIES, BILI_COOKIES, YTDL_MAX_DURATION_SECONDS
...
YOUTUBE_PATTERN = re.compile(
    r"(https?://)?(www\.|m\.)?(youtube\.com/(watch|shorts|live|clip)\S+|youtu\.be/\S+|youtube-nocookie\.com/\S+)",
    re.IGNORECASE,
)
PLAYLIST_REGEX = re.compile(r'(.*)youtube\.com/(.*)[&|?]list=(?P<playlist>[^&]*)(.*)', re.IGNORECASE)
# Aparat (Iranian video platform) — previously only handled deep inside the
# vendored ytdl_legacy/extractor/aparat.py; no dedicated top-level pattern
# existed, so aparat.com links fell through to the generic yt-dlp path
# instead of getting a fast/explicit route like YouTube/Instagram above.
APARAT_PATTERN = re.compile(r"(https?://)?(www\.)?aparat\.com/\S+", re.IGNORECASE)
...
_EXCLUDED_DOMAINS = (
    "youtube.com", "youtu.be", "youtube-nocookie.com", "instagram.com", "instagr.am",
    "pinterest.", "pin.it", "aparat.com",
    ...
    *BYPASS_DOMAINS,
    ...
    "pocketfm.com",
    "bilibili.com", "b23.tv", "bilibili.tv", "biliintl.com", "bili.im",
...
def _cookies_for(url: str):
    ...
    if ("vk.com" in url or "vk.ru" in url) and VK_COOKIES and os.path.exists(VK_COOKIES):
        return VK_COOKIES
    if (
        "bilibili.com" in url or "b23.tv" in url
        or "bilibili.tv" in url or "biliintl.com" in url or "bili.im" in url
    ) and BILI_COOKIES and os.path.exists(BILI_COOKIES):
        return BILI_COOKIES
...
def _base_opts(url: str, force_referer: bool = False) -> dict:
    ...
    if "instagram.com" in url or "instagr.am" in url:
        ...
        # Bilibili's API 412s ("请求被拒绝" / anti-bot) a request that's
        # missing its own site-specific headers, even though the generic
        # same-origin Referer branch below would already send a plausible
        # Referer. Explicit Origin + Accept + a zh-CN Accept-Language (its
        # API is stricter about this than most - a non-Chinese
        # Accept-Language is itself a bot signal) reliably avoids the 412.
        opts["http_headers"] = {
    ...
    elif "bilibili.tv" in url or "biliintl.com" in url or "bili.im" in url:
        # Global/Bstation edition is a separate backend on its own domain -
        # same 412 anti-bot behavior as the CN site, but it wants an
        # English/international Accept-Language and its own Origin/Referer,
        # not the CN site's zh-CN one.
        opts["http_headers"] = {
            "User-Agent": _DESKTOP_UA,
...
async def _show_quality_picker(client: Client, message: Message, url: str, fallback_candidates=None, minimal: bool = False, title_override: str = None, notify_on_failure: bool = True):
    ...
    if YTDL_MAX_DURATION_SECONDS and (info.get("duration") or 0) > YTDL_MAX_DURATION_SECONDS:
        limit_str = str(datetime.timedelta(seconds=YTDL_MAX_DURATION_SECONDS))
        actual_str = _fmt_duration(info.get("duration"))
        if not notify_on_failure:
            try:
                await fetching.delete()
            except Exception:
            ...
            return False
        await safe_edit(fetching.edit_text,
            f"<b>{E_CROSS} Too long to download</b> ({actual_str} — max allowed is {limit_str}).",
            parse_mode=enums.ParseMode.HTML)
        return False
...
async def quality_pick_callback(client: Client, callback_query: CallbackQuery):
    ...
    try:
        ...
        error_text = strip_ansi(e)
        if YOUTUBE_PATTERN.search(session.get("url", "")):
            try:
                from Akbots.brokenx_fallback import looks_like_ip_block, try_brokenx_fallback
                if looks_like_ip_block(error_text):
                    delivered = await try_brokenx_fallback(
                        client, session["chat_id"], session["reply_to"],
                        session["url"], audio_only, status=status,
                    )
                    if delivered:
                        return
            except ImportError:
                pass  # brokenxapi package not installed — fall through to the normal error below
    ...
    try:
        ...
        if YOUTUBE_PATTERN.search(url):
            try:
                ...
                if looks_like_ip_block(error_text):
                    delivered = await try_brokenx_fallback(
                        client, message.chat.id, message.id, url, True, status=status,
                    )
                    if delivered:
                        return
            except ImportError:
                pass
        await safe_edit(status.edit_text, f"<b>{E_CROSS} Download failed:</b>\n<code>{error_text}</code>", parse_mode=enums.ParseMode.HTML)
    finally:
        shutil.rmtree(session_dir, ignore_errors=True)
...
async def pinterest_auto_detect(client: Client, message: Message):
    from Akbots.cookies_manager import has_pending_cookie_flow
    if has_pending_cookie_flow(client, message.from_user.id):
        return  # let cookies_manager.py's own handler claim this update
...
@Client.on_message(filters.text & filters.private & filters.regex(APARAT_PATTERN) & ~filters.regex(r"^/"), group=1)
async def aparat_auto_detect(client: Client, message: Message):
    """Dedicated Aparat support — straight to the yt-dlp quality picker,
    same as YouTube/Instagram/MPD above, instead of falling through to the
    slower generic fallback (which now excludes aparat.com — see
    _EXCLUDED_DOMAINS)."""
    from Akbots.cookies_manager import has_pending_cookie_flow
    if has_pending_cookie_flow(client, message.from_user.id):
        return  # let cookies_manager.py's own handler claim this update
    m = APARAT_PATTERN.search(message.text)
    if m:
        await _show_quality_picker(client, message, m.group(0))
...
async def generic_ytdlp_auto_detect(client: Client, message: Message):
    ...
    # Not a site yt-dlp has a NAMED extractor for (i.e. yt-dlp's *Generic*
    # HTML-scraper would be the only thing that could handle it) - policy
    # changed: yt-dlp no longer gets a blind first try on every random
    # link anymore. Only sites it explicitly recognises (Tier 1 above -
    # the same sites that need Akbots/cookies_manager.py cookies wired up)
    # auto-trigger it. Everything else is left for gallery-dl's own probe
    # (group=3) / the raw-file fallback (group=4) to handle - no yt-dlp
    # attempt, no headless render here.
    return
