...
def format_caption(details, directors, top_actors, languages):
    ...
    return (
        "<blockquote>"
        f"🎬 <b>{title}</b> ({year})\n\n"
        f"<b>🗓 ʀᴇʟᴇᴀsᴇ ᴅᴀᴛᴇ:</b> <code>{release_date}</code>\n"
        f"<b>⏱ ʀᴜɴᴛɪᴍᴇ:</b> <code>{runtime} min</code>\n"
        f"<b>🌐 ʟᴀɴɢᴜᴀɢᴇs:</b> <code>{languages}</code>\n"
        f"<b>🎭 ɢᴇɴʀᴇs:</b> <code>{genres}</code>\n"
        f"<b>🎬 ᴅɪʀᴇᴄᴛᴏʀ:</b> <code>{directors}</code>\n"
        f"<b>⭐ ᴄᴀsᴛ:</b> <code>{top_actors}</code>\n\n"
...
def format_series_caption(details, creators, top_actors, languages):
    ...
    return (
        "<blockquote>"
        f"📺 <b>{title}</b> ({year})\n\n"
        f"<b>🗓 ғɪʀsᴛ ᴀɪʀᴇᴅ:</b> <code>{first_air}</code>\n"
        f"<b>📡 sᴛᴀᴛᴜs:</b> <code>{status}</code>\n"
        f"<b>🎞 sᴇᴀsᴏɴs/ᴇᴘɪsᴏᴅᴇs:</b> <code>{seasons} / {episodes}</code>\n"
        f"<b>🌐 ʟᴀɴɢᴜᴀɢᴇs:</b> <code>{languages}</code>\n"
        f"<b>🎭 ɢᴇɴʀᴇs:</b> <code>{genres}</code>\n"
        f"<b>📡 ɴᴇᴛᴡᴏʀᴋ:</b> <code>{networks}</code>\n"
        f"<b>👨‍💼 ᴄʀᴇᴀᴛᴏʀ:</b> <code>{creators}</code>\n"
        f"<b>⭐ ᴄᴀsᴛ:</b> <code>{top_actors}</code>\n\n"
        f"📝 <code>{overview}</code>"
        "</blockquote>"
    )
...
def format_quick_caption(details, media_type="movie"):
    ...
    return (
        ...
        f"<b>⭐ ʀᴀᴛɪɴɢ:</b> <code>{rating:.1f}/10</code> <i>({votes} votes)</i>\n"
        f"<b>🎭 ɢᴇɴʀᴇs:</b> <code>{genres}</code>\n\n"
        f"📝 <code>{overview}</code>"
        "</blockquote>"
    )
...
async def movieinfo_command(client: Client, message: Message):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if not movie:
                return await safe_edit(status.edit_text, f"❌ No results found for {name} ({year or ''}).")
        ...
        await safe_edit(status.edit_text, f"❌ Error: <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
...
async def poster_command(client: Client, message: Message):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            result, media_type = await _search_movie_or_tv(session, name, year)
            if not result:
                return await safe_edit(status.edit_text, f"❌ '{name}' not found.")
            poster_url = await get_poster_url(session, result["id"], media_type)

        await status.delete()
        title = result.get("title") or result.get("name") or name
        if poster_url:
            await message.reply_photo(poster_url, caption=f"<blockquote>🎬 <b>{title}</b></blockquote>", parse_mode=enums.ParseMode.HTML)
        else:
