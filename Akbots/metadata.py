...
from Akbots.direct_utils import upload_file, safe_edit, make_download_progress
...
async def set_metadata_cmd(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ᴜsᴀɢᴇ:</b> <code>/set_metadata Your Text</code>\n\n"
...
async def _remux_with_metadata(input_path: str, output_path: str, text: str) -> bool:
    ...
    cmd = [
        ...
        "-metadata", "comment=Renamed via Akbots",
        output_path,
    ]
...
async def apply_metadata_cmd(client: Client, message: Message):
    ...
    if not text:
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>ɴᴏ ᴍᴇᴛᴀᴅᴀᴛᴀ ᴛᴇxᴛ sᴇᴛ!</b>\n\n"
    ...
    try:
        await client.download_media(replied, file_name=in_path, progress=make_download_progress(status, file_name=file_name))
    ...
    await safe_edit(status.edit_text, f"<b>{E_BOLT} Writing metadata...</b>", parse_mode=enums.ParseMode.HTML)
