...
Ported in-process from the standalone TurnstileSolver/ project: this file
is that project's actual solving logic (Camoufox automation only), with no
changes, imported directly by Akbots/cf_bypass.py instead of being called
over HTTP. The rest of that project — its FastAPI service.py, db.py
(API-key store), __main__.py, and web/ UI — existed only to expose this
module as a standalone HTTP microservice and were dropped, since nothing
here runs a second process any more; cf_bypass.py just awaits these
functions directly. Needs `camoufox[geoip]` + `playwright==1.54.0` (see
requirements.txt) and a running Xvfb (see entrypoint.sh) for the headed
browser this relies on — see the notes below for why.
