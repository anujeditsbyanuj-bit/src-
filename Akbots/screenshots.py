...
from Akbots.direct_utils import extract_screenshots, VIDEO_EXTS, safe_edit, make_download_progress
...
async def screenshots_cmd(client: Client, message: Message):
    ...
    await safe_edit(status.edit_text, f"<b>{E_CAM} Generating {count} screenshots...</b>", parse_mode=enums.ParseMode.HTML)
...
async def autoscreenshots_cmd(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        ...
        return await message.reply_text(
            f"<blockquote>{E_CAM} <b>ᴀᴜᴛᴏ sᴄʀᴇᴇɴsʜᴏᴛs:</b> <code>{'ON' if current else 'OFF'}</code>\n\n"
            f"<b>ᴜsᴀɢᴇ:</b> <code>/autoscreenshots on</code> or <code>/autoscreenshots off</code>\n"
