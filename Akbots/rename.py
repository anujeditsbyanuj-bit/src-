...
# branch of plugins/file_rename.py), simplified to fit Akbots' straight
...
# word lists from Akbots/words.py) and re-uploaded — reusing
# Akbots/direct_utils.upload_file for the actual upload, so it gets the
...
from Akbots.autodetect import apply_autorename_template, sanitize_filename
from Akbots.direct_utils import upload_file, format_progress, fmt_bytes, _status_edit, safe_edit
...
AUTORENAME_HELP = (
    f"<blockquote>{E_PENCIL} <b>ᴀᴜᴛᴏ-ʀᴇɴᴀᴍᴇ ᴛᴇᴍᴘʟᴀᴛᴇ</b>\n\n"
...
async def set_autorename_cmd(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_CHECK} <b>ᴀᴜᴛᴏ-ʀᴇɴᴀᴍᴇ ᴛᴇᴍᴘʟᴀᴛᴇ sᴀᴠᴇᴅ.</b>\n\n"
        f"<b>ᴛᴇᴍᴘʟᴀᴛᴇ:</b> <code>{template}</code></blockquote>",
...
async def see_autorename_cmd(client: Client, message: Message):
    ...
    if template:
        await message.reply_text(
            f"<blockquote>{E_GEAR} <b>ʏᴏᴜʀ ᴀᴜᴛᴏ-ʀᴇɴᴀᴍᴇ ᴛᴇᴍᴘʟᴀᴛᴇ:</b>\n\n<code>{template}</code></blockquote>",
            parse_mode=enums.ParseMode.HTML
        )
    else:
        await message.reply_text(
...
async def auto_rename_incoming_media(client: Client, message: Message):
    ...
    status = await message.reply_text(
        f"<blockquote>{E_BOLT} <b>ᴀᴜᴛᴏ-ʀᴇɴᴀᴍᴇ ᴀᴘᴘʟɪᴇᴅ</b>\n\n"
        f"<b>ᴏʟᴅ:</b> <code>{original_name}</code>\n"
        f"<b>ɴᴇᴡ:</b> <code>{new_name}</code>\n\n"
        f"{E_ROCKET} <b>ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ...</b></blockquote>",
    ...
    if not path:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Download failed.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    if custom_caption:
        ...
        caption = render_caption(
            custom_caption, filename=new_name, size=fmt_bytes(file_size),
            caption=(message.caption.html if message.caption else ""), media_type=media_type,
        )
    else:
        caption = f"<blockquote><b>{new_name}</b></blockquote>"
