...
async def linkbatch_cmd(client: Client, message: Message):
    ...
    async for msg in _fetch_messages_in_range(client, f_chat, f_id, l_id):
        ...
        if processed % 10 == 0:
            try:
                await safe_edit(status.edit, f"⏳ Processed {processed}/{span} — {len(results)} link(s) so far...")
            except Exception:
                pass

    if not results:
        await safe_edit(status.edit, "No document/video/audio files found in that range.")
        return
    ...
    await safe_edit(status.edit, chunks[0])
