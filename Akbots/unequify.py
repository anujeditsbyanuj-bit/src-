...
from Akbots.direct_utils import E_CHECK, E_CROSS, E_INFO, E_CLOCK, safe_edit
...
_RUNNING: dict[tuple[int, int], asyncio.Task] = {}
_PENDING: dict[tuple[int, int], tuple] = {}   # (bot identity, user_id) -> (chat_ref, via_hint) awaiting confirm
...
# the SAME PYTHON PROCESS, so a bare user_id key would let a scan started
# via one bot get silently overwritten/untrackable if the same owner runs
# /unequify via another bot before the first finishes. See
...
def _progress_text(total, deleted, elapsed):
    ...
    return (
        ...
        f"<b>sᴄᴀɴɴᴇᴅ:</b> <code>{total}</code>\n"
        f"<b>ᴅᴜᴘʟɪᴄᴀᴛᴇs ᴅᴇʟᴇᴛᴇᴅ:</b> <code>{deleted}</code>\n"
        f"<b>ᴇʟᴀᴘsᴇᴅ:</b> <code>{m}m {s}s</code>"
    )
...
async def unequify_cmd(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        return await message.reply_text(
            ...
            f"<b>ᴜsᴀɢᴇ:</b> <code>/unequify &lt;chat_id or @username&gt;</code>\n\n"
    ...
    _PENDING[key] = (chat.id, via)
    confirm_kb = InlineKeyboardMarkup([[
        make_button("✅ ʏᴇs, sᴄᴀɴ & ᴅᴇʟᴇᴛᴇ", callback_data=f"uneq:yes:{user_id}", style=_BS.DANGER if _BS else None),
        make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data="uneq:no", style=_BS.DANGER if _BS else None),
    ]])
    title = chat.title or chat.first_name or str(chat.id)
    await message.reply_text(
        f"<b>⚠️ ᴛʜɪs ᴡɪʟʟ ᴘᴇʀᴍᴀɴᴇɴᴛʟʏ ᴅᴇʟᴇᴛᴇ ᴅᴜᴘʟɪᴄᴀᴛᴇ ᴍᴇssᴀɢᴇs ɪɴ:</b> {title}\n\n"
...
async def unequify_confirm_cb(client: Client, callback_query: CallbackQuery):
    ...
    if parts[1] == "no":
        _PENDING.pop(_pk(client, callback_query.from_user.id), None)
        return await safe_edit(callback_query.message.edit_text, f"<b>{E_INFO} Cancelled.</b>", parse_mode=enums.ParseMode.HTML)

    owner_id = int(parts[2])
    if callback_query.from_user.id != owner_id:
        return await callback_query.answer("This isn't your request.", show_alert=True)

    pending = _PENDING.pop(_pk(client, owner_id), None)
    if not pending:
    ...
    chat_id, via = pending
    status_msg = await safe_edit(callback_query.message.edit_text, 
    ...
    _RUNNING[_pk(client, owner_id)] = task
...
async def _run_unequify(client: Client, user_id: int, chat_id: int, via: str, status_msg: Message):
    ...
    if via == "user":
        ...
        if not userbot:
            return await safe_edit(status_msg.edit_text, 
    ...
    start = time.time()
    last_edit = 0.0
    cancel_kb = InlineKeyboardMarkup([[make_button("🚫 ᴄᴀɴᴄᴇʟ", callback_data="uneq:stop", style=_BS.DANGER if _BS else None)]])
    ...
    try:
        ...
        await safe_edit(status_msg.edit_text, 
            ...
            f"<b>sᴄᴀɴɴᴇᴅ:</b> <code>{total}</code>\n"
            f"<b>ᴅᴜᴘʟɪᴄᴀᴛᴇs ᴅᴇʟᴇᴛᴇᴅ:</b> <code>{deleted}</code>\n"
            f"<b>ᴛɪᴍᴇ ᴛᴀᴋᴇɴ:</b> <code>{m}m {s}s</code>",
            parse_mode=enums.ParseMode.HTML
        )
    except asyncio.CancelledError:
        await safe_edit(status_msg.edit_text, 
            f"<b>🚫 sᴛᴏᴘᴘᴇᴅ.</b> Scanned <code>{total}</code>, deleted <code>{deleted}</code> so far.",
            parse_mode=enums.ParseMode.HTML
        )
        raise
    finally:
...
async def unequify_stop_cb(client: Client, callback_query: CallbackQuery):
    ...
    task = _RUNNING.get(_pk(client, user_id))
...
async def unequify_cancel_cmd(client: Client, message: Message):
    ...
    task.cancel()
    await message.reply_text(f"<b>🚫 sᴛᴏᴘᴘɪɴɢ...</b>", parse_mode=enums.ParseMode.HTML)
