...
async def start_merge(client: Client, message: Message):
    ...
    if user_id in _MERGE_SESSIONS:
        ...
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>A merge session is already running.</b>\n\n"
    ...
    await message.reply_text(
        f"<blockquote>{E_BOLT} <b>Video Merge — session started</b>\n\n"
...
async def merge_status(client: Client, message: Message):
    ...
    await message.reply_text(
        f"<blockquote>{E_GEAR} <b>Merge Session</b>\n\n{listing}</blockquote>",
        parse_mode=enums.ParseMode.HTML,
    )
...
async def cancel_merge(client: Client, message: Message):
    ...
    _cleanup(user_id)
    await message.reply_text(
        f"<blockquote>{E_TRASH} <b>Merge session cancelled.</b>\n"
