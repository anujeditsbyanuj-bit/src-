...
from Akbots.direct_utils import upload_file, make_output_folder, safe_filename, AUDIO_EXTS
...
async def edittag_cmd(client: Client, message: Message):
    ...
    try:
        ...
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} Couldn't write tags:</b> <code>{e}</code>",
                                       parse_mode=enums.ParseMode.HTML)
...
async def setart_receive(client: Client, message: Message):
    ...
    try:
        ...
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} Couldn't embed cover art:</b> <code>{e}</code>",
                                       parse_mode=enums.ParseMode.HTML)
