...
from Akbots.direct_utils import upload_file, make_output_folder, safe_filename, AUDIO_EXTS, safe_edit, make_download_progress
...
async def edittag_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_WARN} Reply to an <b>ᴀᴜᴅɪᴏ ғɪʟᴇ</b> (.mp3/.m4a) with "
            f"<code>/edittag</code>.\n\n{E_INFO} <b>ᴜsᴀɢᴇ:</b>\n"
...
async def setart_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_WARN} Reply to an <b>ᴀᴜᴅɪᴏ ғɪʟᴇ</b> with <code>/setart</code>, "
    ...
    _ART_PENDING[user_id] = {"replied_id": replied.id,
                              ...
                              "ts": time.time()}
    await message.reply_text(f"<b>{E_TAG} Got it.</b> Now send the <b>ᴄᴏᴠᴇʀ ɪᴍᴀɢᴇ</b> (as a photo).",
                              parse_mode=enums.ParseMode.HTML)
