...
async def ghhf_auto_detect(client: Client, message: Message):
    # _dispatch() already fully handles the link (download+upload or an
    # asset-picker keyboard) — without stopping propagation here, the SAME
    # github.com/huggingface.co link keeps falling through to ytdl.py's
    # group=2 generic yt-dlp catch-all and, worse, urluploader.py's group=4
    # raw-file fallback, which calls stream_download() directly on it.
    # GitHub/HF repo pages return HTML, not a file, so that raw fallback
    # always fails with "server returned an HTML/error page instead of the
    # actual file" — a bogus SECOND reply on a link this file already
    # handled correctly. stop_propagation() as soon as something matched
    # prevents every later group from ever seeing this message.
    if await _dispatch(client, message, message.text):
