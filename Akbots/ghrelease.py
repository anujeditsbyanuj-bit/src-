...
def _pick_keyboard(session_id: str, items, prefix: str) -> InlineKeyboardMarkup:
    ...
    for i, (label, _url, size) in enumerate(items):
        ...
        if len(text) > 60:
            text = text[:57] + "..."
        rows.append([make_button(text, callback_data=f"{prefix}#{session_id}#{i}")])
    rows.append([make_button("❌ Cancel", callback_data=f"{prefix}cancel#{session_id}",
...
async def _handle_gh_repo_or_releases(client: Client, message: Message, m: re.Match):
    ...
    try:
        ...
        return await status.edit_text(f"<b>{E_CROSS} GitHub API error:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
...
async def gh_asset_pick_callback(client: Client, callback_query: CallbackQuery):
    ...
    await status.edit_text(f"<b>{E_INFO} Downloading {name}...</b>", parse_mode=enums.ParseMode.HTML)
...
async def _handle_hf_repo(client: Client, message: Message, m: re.Match):
    ...
    try:
        ...
        return await status.edit_text(f"<b>{E_CROSS} HuggingFace API error:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)

    if not files:
        return await status.edit_text(f"<b>{E_CROSS} No files found (repo may be private or gated).</b>", parse_mode=enums.ParseMode.HTML)
