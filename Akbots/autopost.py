...
from Akbots.movieinfo import get_poster_url, LANG_MAP, BASE_URL
from Akbots.direct_utils import safe_edit

logger = logging.getLogger("Akbots.autopost")
...
async def autotest_command(client: Client, message: Message):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            movies = resp.get("results", [])
            if not movies:
                return await safe_edit(status.edit_text, "❌ No upcoming movies found.")
            movie = random.choice(movies)
            await send_movie_post(client, session, movie, "📢 Test AutoPost")
        await safe_edit(status.edit_text, f"✅ Random test movie posted: <code>{movie.get('title')}</code>", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
