...
async def ak_bots_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(
        ...
        f"⚡ <b>Auto Bot:</b> tap the button and create a bot through Telegram. "
        f"It will be added automatically.\n\n"
        f"<i>The management option must be enabled for this bot in @BotFather.</i>",
...
async def ak_bots_add_token_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await callback_query.message.edit_text(
        f"<blockquote>➕ <b>Add Bot</b>\n\n"
        f"{E_INFO} Get a bot token from @BotFather, then send:\n"
        f"<code>/setbot &lt;bot_token&gt;</code>\n\n"
        f"<i>Your bot will run alongside Akbots for forwarding.</i></blockquote>",
...
async def ak_extra_ext_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    if ask is None:
        return await txt.edit_text(f"<b>{E_SOON} Process has been automatically cancelled.</b>", reply_markup=back_markup, parse_mode=enums.ParseMode.HTML)
    if ask.text.strip().startswith("/cancel"):
        return await txt.edit_text("<b>Process cancelled.</b>", reply_markup=back_markup, parse_mode=enums.ParseMode.HTML)
