...
def _ak_manager_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [make_button("🤖 Bots", callback_data="ak_bots", style=_BS.PRIMARY if _BS else None),
         make_button("🏷 Channels", callback_data="channels_btn", style=_BS.PRIMARY if _BS else None)],
        [make_button("🖋 Caption", callback_data="caption_btn", style=_BS.SUCCESS if _BS else None),
         make_button("🗃 MongoDB", callback_data="database_btn", style=_BS.SUCCESS if _BS else None)],
        [make_button("🕵️ Filters", callback_data="ak_filters", style=_BS.PRIMARY if _BS else None),
         make_button("🔘 Button", callback_data="ak_button", style=_BS.SUCCESS if _BS else None)],
        [make_button("🚀 Manager 🚀", callback_data="ak_core", style=_BS.PRIMARY if _BS else None)],
        [make_button("🧪 Extra Settings", callback_data="ak_extra", style=_BS.PRIMARY if _BS else None)],
        [make_button("🔁 Back", callback_data="settings_back_btn", style=_BS.DANGER if _BS else None)],
    ])
...
def _bots_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [make_button("➕ Add bot", callback_data="ak_bots_add", style=_BS.SUCCESS if _BS else None)],
        [make_button("➕ Add User bot", callback_data="ak_bots_adduser", style=_BS.SUCCESS if _BS else None)],
        [make_button("🗑 Remove Bot", callback_data="ak_bots_remove", style=_BS.DANGER if _BS else None)],
...
async def ak_bots_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    await safe_edit(callback_query.message.edit_text, 
        f"<u><b>Add Bot</b></u>\n\n"
        f"<b>Choose your method:</b>\n\n"
        f"🤖 <b>Add Bot Using Token:</b> paste a token from @BotFather — always works, recommended.\n\n"
        f"⚡ <b>Auto Bot:</b> tap a button and Telegram creates one for you automatically. "
        ...
        reply_markup=InlineKeyboardMarkup([
            [make_button("🤖 Add Bot Using Token", callback_data="ak_bots_add_token", style=_BS.PRIMARY if _BS else None)],
            [make_button("⚡ Add Auto Bot", callback_data="ak_bots_autocreate", style=_BS.SUCCESS if _BS else None)],
            [make_button("🔁 Back", callback_data="ak_bots", style=_BS.PRIMARY if _BS else None)],
        ]),
        parse_mode=enums.ParseMode.HTML,
    )
...
async def ak_bots_add_token_callback(client: Client, callback_query: CallbackQuery):
    ...
    await safe_edit(callback_query.message.edit_text,
        f"📥 <b>Send me your bot token from @BotFather</b>\n\n"
        f"📝 <b>Example:</b> <code>1234567890:ABCdefGhIJKlmNoPQRsTUVwxyZ</code>\n\n"
        f"/cancel - cancel this process",
        reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="ak_bots_add", style=_BS.DANGER if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
    ...
    try:
        ...
        return await client.send_message(
            chat_id, f"<b>❌ Timeout! Please try again.</b>",
            reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="ak_bots_add", style=_BS.DANGER if _BS else None)]]),
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    await safe_edit(status.edit_text, text,
        reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back To Settings", callback_data="akanager_btn", style=_BS.PRIMARY if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
...
async def _ask_target_chat(client: Client, chat_id: int, user_id: int):
    ...
    prompt = await client.send_message(
        chat_id,
        f"<b>( SET TARGET CHAT )</b>\n\n"
        f"📩 <b>Forward a message from your target chat</b>\n\n"
        f"🔗 <b>Or send a link to the channel/group</b>\n"
    ...
    try:
        ...
        return await client.send_message(
            chat_id, f"<b>❌ Timeout! Please try again.</b>",
            reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back To Settings", callback_data="akanager_btn", style=_BS.DANGER if _BS else None)]]),
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    if chat_ref is None:
        return await reply.reply_text(
            f"<b>❌ Invalid target. Forward a message or send a valid t.me link.</b>",
            reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back To Settings", callback_data="akanager_btn", style=_BS.DANGER if _BS else None)]]),
    ...
    if not chat:
        return await safe_edit(status.edit_text, 
            f"<b>❌ Can't access that chat.</b> Make sure your bot (or your logged-in "
...
async def ak_bots_autocreate_callback(client: Client, callback_query: CallbackQuery):
    ...
    try:
        await request_bot_autocreate(
            ...
            purpose="custom_bot",
            header_text="⚡ AUTO-CREATE YOUR FORWARDING BOT\n\n"
...
async def ak_bots_adduser_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await safe_edit(callback_query.message.edit_text, 
        f"<u><b>Add User Bot</b></u>\n\n"
        f"<b>Choose your login method:</b>",
        reply_markup=InlineKeyboardMarkup([
            [make_button("📱 Login Using Phone Number", callback_data="ak_bots_adduser_phone", style=_BS.PRIMARY if _BS else None)],
            [make_button("🔑 Login Using String Session", callback_data="ak_bots_adduser_string", style=_BS.PRIMARY if _BS else None)],
            [make_button("🔁 Back", callback_data="ak_bots", style=_BS.DANGER if _BS else None)],
        ]),
        parse_mode=enums.ParseMode.HTML,
    )
...
async def ak_bots_remove_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await safe_edit(callback_query.message.edit_text, 
        f"<blockquote>🗑 <b>Remove Bot / User Bot</b>\n\n"
        f"{E_INFO} <code>/rembot</code> — Disconnect your bot\n"
        f"{E_INFO} <code>/logout</code> — Log out your user bot session</blockquote>",
        reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="ak_bots", style=_BS.DANGER if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
...
def _extra_settings_buttons(s: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [make_button("📊 Poll", callback_data="ak_extra_toggle:poll", style=S.PRIMARY if S else None),
         make_button(poll_mark, callback_data="ak_extra_toggle:poll", style=(S.SUCCESS if s.get("poll") else S.DANGER) if S else None)],
        [make_button("🔒 Secure message", callback_data="ak_extra_toggle:secure", style=S.PRIMARY if S else None),
        ...
        [make_button("🛑 Size limit", callback_data="ak_extra_size", style=S.PRIMARY if S else None)],
        [make_button("💾 Extension", callback_data="ak_extra_ext", style=S.PRIMARY if S else None)],
        [make_button("♦️ Keywords ♦️", callback_data="ak_extra_kw", style=S.PRIMARY if S else None)],
        [make_button("⫷ Back", callback_data="akanager_btn", style=S.DANGER if S else None)],
    ])
...
async def ak_extra_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_reply_markup(reply_markup=_extra_settings_buttons(s))
...
def _size_limit_buttons(mb: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [b("+50", 50), b("-50", -50)],
        [b("+100", 100), b("-100", -100)],
        [make_button("0 = no limit", callback_data="ak_extra_sl:0", style=S.PRIMARY if S else None)],
        [make_button("⏪ Back", callback_data="ak_extra", style=S.DANGER if S else None)],
    ])
...
def _size_limit_text(mb: int) -> str:
    ...
    return f"<u><b>SIZE LIMIT</b></u>\n\n<b>You can set a file size limit to forward.</b>\n\nStatus: {status}"
...
def _list_buttons(items, add_cb: str, clear_cb: str, back_cb: str) -> InlineKeyboardMarkup:
    ...
    if row:
        rows.append(row)
    rows.append([make_button("➕ Add ➕", callback_data=add_cb, style=S.SUCCESS if S else None)])
    rows.append([make_button("Remove all", callback_data=clear_cb, style=S.DANGER if S else None)])
    rows.append([make_button("⏪ Back", callback_data=back_cb, style=S.DANGER if S else None)])
...
async def ak_extra_ext_callback(client: Client, callback_query: CallbackQuery):
    ...
    s = await db.get_fwd_settings(user_id)
    await safe_edit(callback_query.message.edit_text, 
        "<u><b>EXTENSIONS</b></u>\n\n<b>Files with these extensions will not forward.</b>",
...
async def ak_extra_ext_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    txt = await client.send_message(
        user_id, "<b>Please send your extensions (separate by space).</b>\n\n/cancel — cancel this process",
        parse_mode=enums.ParseMode.HTML,
    )
    ...
    if ask.text.strip().startswith("/cancel"):
        return await safe_edit(txt.edit_text, "<b>Process cancelled.</b>", reply_markup=back_markup, parse_mode=enums.ParseMode.HTML)
...
async def ak_extra_kw_callback(client: Client, callback_query: CallbackQuery):
    ...
    s = await db.get_fwd_settings(user_id)
    await safe_edit(callback_query.message.edit_text, 
        "<u><b>KEYWORDS</b></u>\n\n<b>Files with these keywords in the file name will forward.</b>",
...
async def ak_extra_kw_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.delete()
    txt = await client.send_message(
        user_id, "<b>Please send the keywords (separate by space).</b>\n\n/cancel — cancel this process",
        parse_mode=enums.ParseMode.HTML,
    )
    ask = await _ak_listen(user_id)
    s = await db.get_fwd_settings(user_id)
...
def _filters_buttons(s: dict) -> InlineKeyboardMarkup:
    ...
    rows.append([make_button("⫷ back", callback_data="akanager_btn", style=S.DANGER if S else None)])
    return InlineKeyboardMarkup(rows)
...
async def ak_filters_toggle_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.answer()
    s = await db.get_fwd_settings(user_id)
    await callback_query.message.edit_reply_markup(reply_markup=_filters_buttons(s))
...
def _button_menu_text_and_markup(has_button: bool) -> tuple:
    text = (
        f"<u><b>CUSTOM BUTTON</b></u>\n\n"
        f"<b>You can set a inline button to messages.</b>\n\n"
        f"<u><b>FORMAT:</b></u>\n"
        f"<code>[FtmBotzx][buttonurl:https://t.me/ftmbotzx]</code>\n"
    )
    if has_button:
        rows = [[
            make_button("👀 See Button", callback_data="ak_button_see", style=_BS.PRIMARY if _BS else None),
            make_button("🗑 Remove Button", callback_data="ak_button_delete", style=_BS.DANGER if _BS else None),
        ]]
    else:
        rows = [[make_button("➕ Add Button ➕", callback_data="ak_button_add", style=_BS.SUCCESS if _BS else None)]]
    rows.append([make_button("⏪ Back", callback_data="akanager_btn", style=_BS.DANGER if _BS else None)])
...
async def ak_button_see_callback(client: Client, callback_query: CallbackQuery):
    ...
    rows.append([make_button("⏪ Back", callback_data="ak_button", style=_BS.DANGER if _BS else None)])
    await safe_edit(callback_query.message.edit_text, 
        "<b>YOUR CUSTOM BUTTON</b>", reply_markup=InlineKeyboardMarkup(rows), parse_mode=enums.ParseMode.HTML
    )
...
async def ak_core_callback(client: Client, callback_query: CallbackQuery):
    await callback_query.answer()
    await safe_edit(callback_query.message.edit_text, 
        f"<blockquote>{E_INFO} <b>Manager (Forwarding Core)</b>\n\n"
        ...
        reply_markup=InlineKeyboardMarkup([[make_button("🔁 Back", callback_data="akanager_btn", style=_BS.DANGER if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
