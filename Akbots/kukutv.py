...
_active_kuku_jobs = {}     # (bot identity, user_id) -> True while a batch download is running
_pending_search = {}       # message_id -> [{"id":..., "title":..., "n_episodes":...}, ...]
_awaiting_range = {}       # (bot identity, user_id) -> {"show_id", "show_title", "free_by_num": {num: ep}}
# _active_kuku_jobs / _awaiting_range are keyed by (id(client), user_id), not
# bare user_id: the main bot and every Titanium clone (Akbots/titanium.py)
# run as separate Client instances in the SAME PYTHON PROCESS, so a bare
# user_id key would let one bot's state clobber another's for the same
# owner. See Akbots/cookies_manager.py's _pk() for the original fix.
...
async def _download_one_episode(client: Client, message: Message, ep: dict, show_title: str):
    ...
    if returncode != 0 or not os.path.exists(out_path) or os.path.getsize(out_path) < 1024 * 1024:
        try:
            ...
            pass
        await safe_edit(status.edit_text, f"<b>{E_CROSS} Episode {ep_num} failed to download.</b>",
    ...
    await upload_file(
        ...
        caption=f"<blockquote><b>{safe_show}</b> — Episode {ep_num}</blockquote>",
...
async def kuku_cmd(client: Client, message: Message):
    ...
    if len(text) < 2:
        status = await message.reply_text(f"<b>🔥 ғᴇᴛᴄʜɪɴɢ ᴘᴏᴘᴜʟᴀʀ ᴋᴜᴋᴜᴛᴠ sʜᴏᴡs...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    if not shows:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} No shows found for that search.</b>", parse_mode=enums.ParseMode.HTML)
...
async def _show_pick_list(status: Message, shows: list, title: str):
    ...
    buttons = [
        [make_button(f"{s['title'][:45]} ({s['n_episodes']} ᴇᴘ)", callback_data=f"kuku_pick:{status.id}:{i}", style=_BS.PRIMARY if _BS else None)]
        for i, s in enumerate(shows)
    ]
    buttons.append([make_button("❌ ᴄʟᴏsᴇ", callback_data="kuku_close", style=_BS.DANGER if _BS else None)])
...
async def _open_kuku_show(client: Client, message: Message, show_id: str):
    ...
    if not info:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Show not found or API error.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    lines = [
        ...
        "",
        f"🔓 <b>ғʀᴇᴇ ᴇᴘɪsᴏᴅᴇs:</b> {len(free)}",
        f"🔒 <b>ʟᴏᴄᴋᴇᴅ ᴇᴘɪsᴏᴅᴇs:</b> {len(locked)} <i>(not downloadable — no ad-fraud unlock here)</i>",
    ]
    buttons = []
    if free:
        buttons.append([make_button(
            f"📥 ᴅᴏᴡɴʟᴏᴀᴅ {len(free)} ꜰʀᴇᴇ ᴇᴘɪsᴏᴅᴇ(s)", callback_data=f"kuku_dl:{show_id}", style=_BS.PRIMARY if _BS else None)])
        buttons.append([make_button(
            "🎯 ᴘɪᴄᴋ sᴘᴇᴄɪғɪᴄ ᴇᴘɪsᴏᴅᴇs", callback_data=f"kuku_range:{show_id}", style=_BS.PRIMARY if _BS else None)])
    buttons.append([make_button("❌ ᴄʟᴏsᴇ", callback_data="kuku_close", style=_BS.DANGER if _BS else None)])

    await safe_edit(status.edit_text, "<blockquote>" + "\n".join(lines) + "</blockquote>", reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
...
async def kuku_cancel_cmd(client: Client, message: Message):
    ...
    key = _pk(client, uid)
    had_job = _active_kuku_jobs.pop(key, None)
    had_prompt = _awaiting_range.pop(key, None)
...
async def kuku_download_callback(client: Client, callback_query: CallbackQuery):
    ...
    try:
        for ep in free:
            if not _active_kuku_jobs.get(key):
                break
            try:
        ...
        _active_kuku_jobs.pop(key, None)
...
async def kuku_range_prompt_callback(client: Client, callback_query: CallbackQuery):
    ...
    nums = sorted(_awaiting_range[key]["free_by_num"].keys())
...
def _has_pending_kuku_range(_, client, message: Message) -> bool:
    """True only when this user is actually mid-flow picking free episodes.

    This has to live in the pyrogram filter itself, not just as an early
    `return` inside the handler body: pyrogram only tries the NEXT handler
    in the same group if THIS handler's filter comes back False. If the
    filter matches every plain-text message (as `filters.text` alone does)
    and the handler merely returns early internally, pyrogram still counts
    group=2 as "handled" and never even checks the other group=2 handlers
    — which is what silently broke ytdl.py's generic auto-detect fallback
    for every link that isn't a dedicated site pattern (Instagram, Terabox,
    etc.): kukutv.py loads before ytdl.py alphabetically, so this handler
    was always winning that slot first, for every single link.
    """
    return bool(message.from_user) and _pk(client, message.from_user.id) in _awaiting_range


@Client.on_message(
    filters.private & filters.text & ~filters.regex(r"^/") & filters.create(_has_pending_kuku_range),
    group=2,
)
async def kuku_range_reply(client: Client, message: Message):
    uid = message.from_user.id
    key = _pk(client, uid)
    pending = _awaiting_range.get(key)
    if not pending:
        return
