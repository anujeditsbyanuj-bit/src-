        ...
        try:
            ...
            if needs_password:
                try:
                    ...
                    pwd_msg = await client.listen(
                        chat_id=message.chat.id, filters=filters.user(user_id), timeout=60
                    )
                    ...
                    try:
                        ...
                        pass
                    await status.edit_text(f"<b>{E_BOLT} Extracting...</b>", parse_mode=enums.ParseMode.HTML)
                    code, out, err = await _run_7z_extract(arc_path, out_dir, entered_password)
...
# archive back — no /zip -> send files -> /donezip session needed. An
# optional password can be given as an argument: /zipcreate mypassword123
...
async def zip_create_cmd(client: Client, message: Message):
    ...
    password = message.text.split(None, 1)[1].strip() if len(message.command) > 1 else ""
    ...
    async def _job():
        try:
            await reply.download(file_name=dest)
            await status.edit_text(f"<b>{E_BOLT} Building ZIP archive...</b>", parse_mode=enums.ParseMode.HTML)

            archive_name = os.path.splitext(fname)[0] + ".zip"
            archive_dir = make_output_folder(f"zip_out/{user_id}")
            archive_path = os.path.join(archive_dir, archive_name)

            await asyncio.to_thread(_build_zip, archive_path, [(dest, fname)], password, "aes256")
            ...
            await upload_file(
                ...
                f"<b>Source:</b> {fname} | <b>Size:</b> {fmt_bytes(size)}",
                file_name=archive_name
            )
        except Exception as e:
            await status.edit_text(f"<b>{E_CROSS} Failed to create archive:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
        finally:
            try:
...
async def zip_files_cmd(client: Client, message: Message):
    ...
    header = f"<b>{E_PACK} Queued files:</b> {len(entries)} | <b>Total size:</b> {fmt_bytes(grand_total)}"
...
def _build_zip(archive_path, entries, password, encryption="aes256"):
...
def _build_7z(archive_path, entries, password):
    ...
    with py7zr.SevenZipFile(archive_path, "w", **kwargs) as zf:
        for disk_path, arcname in entries:
            if os.path.exists(disk_path):
                zf.write(disk_path, arcname)


def _build_tar(archive_path, entries, fmt):
    # fmt is "tar" / "tar.gz" / "tar.bz2" -> tarfile mode "w" / "w:gz" / "w:bz2"
    mode = {"tar": "w", "tar.gz": "w:gz", "tar.bz2": "w:bz2"}[fmt]
    with tarfile.open(archive_path, mode) as tf:
        for disk_path, arcname in entries:
            if os.path.exists(disk_path):
                tf.add(disk_path, arcname=arcname)
    ...
    async def _job():
        try:
            if fmt == "rar":
                ...
                await asyncio.to_thread(_build_7z, archive_path, session["paths"], session["password"])
            elif fmt in ("tar", "tar.gz", "tar.bz2"):
                await asyncio.to_thread(_build_tar, archive_path, session["paths"], fmt)
            else:
                await asyncio.to_thread(
                    _build_zip, archive_path, session["paths"],
                    session["password"], session.get("encryption", "aes256")
                )
            ...
            await upload_file(
                client, message, archive_path, status,
                f"<b>{E_PACK} {archive_name}</b>{note}\n"
                f"<b>Format:</b> {fmt_label} | <b>Files:</b> {len(session['paths'])} | <b>Size:</b> {fmt_bytes(size)}",
                file_name=archive_name
            )
        except Exception as e:
