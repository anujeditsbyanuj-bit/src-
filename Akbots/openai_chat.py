...
async def _reply_with_gpt(client: Client, message: Message, prompt: str, image_b64: str = None):
        ...
        async for kind, payload in _stream_gpt(history, prompt, model, mode, image_b64):
            if kind == "chunk":
                ...
                if now - last_edit >= 1.0:  # throttle to ~1 edit/sec, Telegram's comfortable rate
                    last_edit = now
                    try:
                        await status.edit_text(payload[:4000] + " ▍")
                    except Exception:
    ...
    try:
        await status.edit_text(chunks[0], parse_mode=enums.ParseMode.MARKDOWN)
    except Exception:
        await status.edit_text(chunks[0])  # fall back to plain text if the model's output isn't valid Markdown
...
async def _reply_with_vision(client: Client, message: Message, photo_message: Message, caption_prompt: str):
    ...
    try:
        ...
        return await status.edit_text(f"<b>{E_CROSS} Couldn't read that image:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    finally:
...
async def _generate_image(client: Client, message: Message, prompt: str, status: Message = None):
    ...
    if status is None:
        ...
        await status.edit_text(f"<b>{E_ROCKET} Generating image...</b>", parse_mode=enums.ParseMode.HTML)
...
async def voice_to_gpt(client: Client, message: Message):
    ...
    if not text:
        return await status.edit_text(f"<b>{E_CROSS} Couldn't make out any speech in that voice note.</b>", parse_mode=enums.ParseMode.HTML)

    await status.edit_text(f"<b>🎙 Heard:</b> <i>{text}</i>", parse_mode=enums.ParseMode.HTML)
