...
async def splitvideo_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_WARN} Reply to a <b>video</b> with <code>/splitvideo</code>.\n\n"
            f"{E_INFO} <b>Usage:</b>\n"
    ...
    if not duration:
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} Couldn't read video duration.</b>",
                                       parse_mode=enums.ParseMode.HTML)
    ...
    if args[0].lower() == "parts":
        if len(args) < 2 or not args[1].isdigit():
            shutil.rmtree(temp_dir, ignore_errors=True)
            return await status.edit_text(f"<b>{E_CROSS} Usage:</b> <code>/splitvideo parts &lt;n&gt;</code>",
                                           parse_mode=enums.ParseMode.HTML)
        n_parts = int(args[1])
        if n_parts < 2 or n_parts > MAX_PARTS:
            shutil.rmtree(temp_dir, ignore_errors=True)
            return await status.edit_text(f"<b>{E_CROSS} Parts must be between 2 and {MAX_PARTS}.</b>",
                                           parse_mode=enums.ParseMode.HTML)
        ...
        if not part_len or part_len <= 0:
            shutil.rmtree(temp_dir, ignore_errors=True)
            return await status.edit_text(f"<b>{E_CROSS} Couldn't parse that duration.</b>",
                                           parse_mode=enums.ParseMode.HTML)
