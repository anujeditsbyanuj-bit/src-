...
async def pypi_info_command(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            f"<b>Package:</b> {info.get('name')}\n"
            f"<b>Latest version:</b> {info.get('version')}\n"
            f"<b>Summary:</b> {info.get('summary') or '-'}\n"
            f"<b>Project URL:</b> {homepage}"
        )
    except Exception as e:
...
async def telegraph_upload_command(client: Client, message: Message):
    ...
    try:
        ...
        await status_msg.edit_text(f"{E_CHECK} Uploaded: {link}")
    except Exception as e:
        await status_msg.edit_text(f"{E_CROSS} Upload failed: {e}")
    finally:
