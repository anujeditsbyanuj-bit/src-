...
async def spoiler_cmd(client: Client, message: Message):
        ...
        try:
            if replied.video:
                await client.send_video(
                    ...
                    caption=f"<blockquote>{replied.caption}</blockquote>" if replied.caption else "",
    ...
    if len(message.command) < 2:
        ...
        return await message.reply_text(
            f"<blockquote>{E_EYE} <b>sᴘᴏɪʟᴇʀ ᴍᴏᴅᴇ:</b> <code>{'ON' if current else 'OFF'}</code>\n\n"
            f"<b>ᴜsᴀɢᴇ:</b> <code>/spoiler on</code> or <code>/spoiler off</code>\n"
