...
def _routes_keyboard(routes):
    rows = [[make_button(f"📡 {r['label']}", callback_data=f"route_view#{r['label']}", style=_BS.PRIMARY if _BS else None)] for r in routes]
...
async def channels_cmd(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        f"<i>Tap a channel pair below to view, activate, or remove it.</i>",
...
@Client.on_callback_query(filters.regex(r"^route_view#"))
async def route_view_callback(client: Client, callback_query: CallbackQuery):
    ...
    if not route:
        return await _render_channels_menu(
            ...
            f"<b>{E_CROSS} That saved channel no longer exists.</b>",
            InlineKeyboardMarkup(_channels_menu_footer_row()),
        )

    settings = await db.get_fwd_settings(user_id)
    is_active = settings.get("source") == route["source"] and settings.get("target") == route["target"]
    active_line = "\n\n🟢 <b>Currently active</b>" if is_active else ""

    buttons = InlineKeyboardMarkup([
        [make_button("▶️ Activate", callback_data=f"route_activate#{label}", style=_BS.PRIMARY if _BS else None)],
        [make_button("❌ Remove", callback_data=f"route_remove#{label}", style=_BS.DANGER if _BS else None)],
        [make_button("↩ Back", callback_data="channels_btn", style=_BS.PRIMARY if _BS else None)],
    ])
    await _render_channels_menu(
        client, callback_query,
        f"<b>📄 Channel pair details</b>\n\n"
        f"<b>🏷 Label:</b> <code>{route['label']}</code>\n"
        f"<b>📤 Source:</b> <code>{route['source']}</code>\n"
        f"<b>📥 Target:</b> <code>{route['target']}</code>{active_line}",
        buttons,
    )


@Client.on_callback_query(filters.regex(r"^route_activate#"))
async def route_activate_callback(client: Client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    label = callback_query.data.split("#", 1)[1]
    routes = await db.get_fwd_routes(user_id)
    route = next((r for r in routes if r["label"] == label), None)
    ...
    await _render_channels_menu(
        ...
        f"<b>🏷 Label:</b> <code>{route['label']}</code>\n"
        f"<b>📤 Source:</b> <code>{route['source']}</code>\n"
        f"<b>📥 Target:</b> <code>{route['target']}</code>\n\n"
        f"🟢 <b>Currently active</b>",
        buttons,
    )


@Client.on_callback_query(filters.regex(r"^route_remove#"))
async def route_remove_callback(client: Client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    label = callback_query.data.split("#", 1)[1]
    await db.remove_fwd_route(user_id, label)
    await callback_query.answer(f"Removed \"{label}\"", show_alert=False)

    back_btn = InlineKeyboardMarkup([[make_button("↩ Back", callback_data="channels_btn", style=_BS.PRIMARY if _BS else None)]])
    await _render_channels_menu(
        client, callback_query,
        f"<b>✅ Channel pair removed successfully!</b>\n\n<b>🏷 Label:</b> <code>{label}</code>",
        back_btn,
    )
...
def _channels_menu_footer_row():
    return [
        [make_button("➕ ᴀᴅᴅ ᴄʜᴀɴɴᴇʟ ➕", callback_data="ak_channels_add", style=_BS.PRIMARY if _BS else None)],
...
async def channels_btn_callback(client: Client, callback_query: CallbackQuery):
    ...
    rows = [row for row in ([] if not routes else [
        [make_button(f"📡 {r['label']}", callback_data=f"route_view#{r['label']}", style=_BS.PRIMARY if _BS else None)]
        for r in routes
    ])]
...
async def ak_channels_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    status = await callback_query.message.reply_text(f"<b>🔄 Connecting...</b>", parse_mode=enums.ParseMode.HTML)
    await safe_edit(status.edit_text, f"<b>📡 Fetching source chat details...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    src_uname = f"@{source_chat.username}" if getattr(source_chat, "username", None) else "private"
    await safe_edit(status.edit_text, 
        f"<b>✅ Source channel added successfully!</b>\n\n"
        f"<b>📝 Name:</b> <code>{source_chat.title or source_chat.first_name}</code>\n"
        f"<b>🆔 Channel ID:</b> <code>{source_chat.id}</code>\n"
        f"<b>📛 Username:</b> {src_uname}",
        parse_mode=enums.ParseMode.HTML
    )
    ...
    status2 = await callback_query.message.reply_text(f"<b>🔄 Connecting...</b>", parse_mode=enums.ParseMode.HTML)
    await safe_edit(status2.edit_text, f"<b>📡 Fetching target chat details...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    tgt_uname = f"@{target_chat.username}" if getattr(target_chat, "username", None) else "private"
    await safe_edit(status2.edit_text, 
        f"<b>✅ Target channel added successfully!</b>\n\n"
        f"<b>📝 Name:</b> <code>{target_chat.title or target_chat.first_name}</code>\n"
        f"<b>🆔 Channel ID:</b> <code>{target_chat.id}</code>\n"
        f"<b>📛 Username:</b> {tgt_uname}",
        parse_mode=enums.ParseMode.HTML
    )
    ...
    await safe_edit(ask_label.edit_text, f"<b>💾 Saving to database...</b>", parse_mode=enums.ParseMode.HTML)
    await db.add_fwd_route(user_id, label, source_chat.id, source_via, target_chat.id, target_via)
    back_btn = InlineKeyboardMarkup([[make_button("↩ ʙᴀᴄᴋ", callback_data="channels_btn", style=_BS.PRIMARY if _BS else None)]])
    await safe_edit(ask_label.edit_text, 
        f"<b>✅ Channel pair added successfully!</b>\n\n"
        f"<b>🏷 Label:</b> <code>{label}</code>\n"
        f"<b>📤 Source:</b> {source_chat.title or source_chat.first_name} (<code>{source_chat.id}</code>)\n"
        f"<b>📥 Target:</b> {target_chat.title or target_chat.first_name} (<code>{target_chat.id}</code>)\n\n"
        f"<i>Use /channels to activate it.</i>",
        parse_mode=enums.ParseMode.HTML,
