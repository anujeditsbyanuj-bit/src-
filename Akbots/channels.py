...
async def route_go_callback(client: Client, callback_query: CallbackQuery):
    ...
    rows.append(_channels_menu_footer_row())
...
async def route_del_callback(client: Client, callback_query: CallbackQuery):
    ...
    if not routes:
        return await callback_query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([_channels_menu_footer_row()]),
            parse_mode=enums.ParseMode.HTML
        )
    lines = "\n".join(_route_line(r) for r in routes)
    rows = [[make_button(f"▶️ {r['label']}", callback_data=f"route_go#{r['label']}", style=_BS.PRIMARY if _BS else None),
...
def _channels_menu_footer_row():
    return [
        make_button("➕ Add Channel", callback_data="akm_channels_add", style=_BS.SUCCESS if _BS else None),
...
@Client.on_callback_query(filters.regex("^akm_channels_add$"))
async def akm_channels_add_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup([[make_button("⬅️ Back", callback_data="channels_btn", style=_BS.DANGER if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
