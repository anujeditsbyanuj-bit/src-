...
async def process_video_from_url(app: Client, url: str, anime_title: str, episode_num: int):
    ...
    try:
        ...
        try:
            await safe_edit(status.edit_text, f"{E_CROSS} <b>Download failed:</b> {e}")
        except Exception:
            pass
        return
    ...
    try:
        await safe_edit(status.edit_text, f"{E_BOLT} <b>Uploading to Telegram...</b>")
        if final_path.suffix.lower() in VIDEO_EXTS:
        ...
        try:
            await safe_edit(status.edit_text, f"{E_CROSS} <b>Upload failed:</b> {e}")
        except Exception:
            pass
    finally:
...
async def cmd_watch(client: Client, message: Message):
    ...
    await _save_settings(sites=sites)
    await safe_edit(wait_msg.edit_text, f"{E_CHECK} <b>Now watching:</b>\n📺 {title}\n🔗 {url}", disable_web_page_preview=True)
