...
async def process_video_from_url(app: Client, url: str, anime_title: str, episode_num: int):
    ...
    try:
        status = await app.send_message(staging_chat, f"{E_INFO} <b>ᴘʀᴏᴄᴇssɪɴɢ ᴠɪᴅᴇᴏ...</b>")
    except Exception:
        return
    ...
    try:
        ...
        try:
            await safe_edit(status.edit_text, f"{E_CROSS} <b>ᴅᴏᴡɴʟᴏᴀᴅ ғᴀɪʟᴇᴅ:</b> {e}")
        except Exception:
            pass
        return
    ...
    try:
        await safe_edit(status.edit_text, f"{E_BOLT} <b>ᴜᴘʟᴏᴀᴅɪɴɢ ᴛᴏ ᴛᴇʟᴇɢʀᴀᴍ...</b>")
        if final_path.suffix.lower() in VIDEO_EXTS:
        ...
        try:
            await safe_edit(status.edit_text, f"{E_CROSS} <b>ᴜᴘʟᴏᴀᴅ ғᴀɪʟᴇᴅ:</b> {e}")
        except Exception:
            pass
    finally:
...
async def cmd_watch(client: Client, message: Message):
    ...
    await _save_settings(sites=sites)
    await safe_edit(wait_msg.edit_text, f"{E_CHECK} <b>ɴᴏᴡ ᴡᴀᴛᴄʜɪɴɢ:</b>\n📺 {title}\n🔗 {url}", disable_web_page_preview=True)
...
async def _upload_to_user(app: Client, user_id: int, fpath: Path, status: Message,
    ...
    try:
        ...
        await safe_edit(status.edit_text, f"{E_BOLT} Uploading...\n📺 {anime_name} | {quality}")
...
async def cmd_myanime(client: Client, message: Message):
    ...
    await _save_user_watch(user_id, entry)
    await safe_edit(wait_msg.edit_text, f"{E_CHECK} Added to your list:\n📺 {title}", disable_web_page_preview=True)
...
async def cmd_monitor(client: Client, message: Message):
    ...
    settings = await _get_settings()
    await message.reply_text(
        f"{E_ROCKET} <b>ᴍᴏɴɪᴛᴏʀɪɴɢ sᴛᴀʀᴛᴇᴅ!</b>\n\n"
...
async def cmd_batch(client: Client, message: Message):
    ...
    for ep in range(1, count + 1):
        try:
            await safe_edit(status.edit_text, f"{E_BOLT} {anime_name}\n📥 Episode {ep}/{count}")
        except Exception:
            pass
        for q in USER_WATCH_QUALITIES:
            try:
    ...
    try:
        await safe_edit(status.edit_text, f"{E_CHECK} Batch done! Episodes 1-{count}")
    except Exception:
        pass
