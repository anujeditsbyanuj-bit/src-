...
def _resolution_kb(session_id: str) -> InlineKeyboardMarkup:
    ...
    if row:
        rows.append(row)
    rows.append([make_button("❌ Cancel", callback_data=f"enccancel#{session_id}", style=_BS.DANGER if _BS else None)])
...
def _codec_kb(session_id: str) -> InlineKeyboardMarkup:
    ...
    ] + [[make_button("❌ Cancel", callback_data=f"enccancel#{session_id}", style=_BS.DANGER if _BS else None)]])
