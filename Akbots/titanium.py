...
from Akbots.direct_utils import E_CHECK, E_CROSS, E_INFO, safe_edit, strip_ansi, smallcaps, wait_for_reply
from Akbots.botadmin import COMMAND_MENU
...
async def _get_clone_client(token: str, owner_id: int) -> Client:
    ...
    await clone.start()
    try:
        # Clone owners aren't in ADMINS, so they can't run /set themselves
        # — apply the same "/" command menu automatically right after
        # boot so it's never missing on a fresh clone.
        await clone.set_bot_commands(COMMAND_MENU)
    except Exception as e:
        logger.warning(f"Titanium: couldn't set command menu for clone (token ...{token[-6:]}): {e}")
...
async def _revoke_managed_bot_token(bot_id: int) -> str | None:
    ...
    in the same call, so callers that actually want automatic rotation
    (not just revocation) don't need a second round-trip or a manually
    pasted @BotFather token.
...
async def request_bot_autocreate(
    ...
    header_text: str = "⚡ ᴀᴜᴛᴏ-ᴄʀᴇᴀᴛᴇ ʏᴏᴜʀ ᴛɪᴛᴀɴɪᴜᴍ ʙᴏᴛ\n\n"
                        ...
                        "or use a manual @BotFather token instead, which always works.",
...
async def _handle_managed_bot_created(client: Client, update, users, chats):
    ...
    try:
        status_msg = await client.send_message(
            owner_id,
            f"<b>⚡ ᴄʀᴇᴀᴛɪɴɢ ʏᴏᴜʀ ᴛɪᴛᴀɴɪᴜᴍ ʙᴏᴛ...</b>\n\n🔗 <i>Fetching token from Telegram...</i>",
            parse_mode=enums.ParseMode.HTML,
        )
    except Exception:
        status_msg = None
    ...
    if not token:
        try:
            if status_msg is not None:
                await safe_edit(status_msg.edit_text,
    ...
    if status_msg is not None:
        await safe_edit(status_msg.edit_text,
            f"<b>⚡ ᴄʀᴇᴀᴛɪɴɢ ʏᴏᴜʀ ᴛɪᴛᴀɴɪᴜᴍ ʙᴏᴛ...</b>\n\n🔒 <i>Encrypting &amp; storing credentials...</i>",
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    try:
        ...
        if status_msg is not None:
            await safe_edit(status_msg.edit_text,
                f"<b>{E_CROSS} Your clone bot was created, but its token failed to verify.</b>\n\n"
                f"Try /addbot with a manual @BotFather token instead.",
                parse_mode=enums.ParseMode.HTML,
            )
        return
    ...
    if purpose == "custom_bot":
        ...
        if status_msg is not None:
            await safe_edit(status_msg.edit_text,
                f"<b>⚡ ᴄʀᴇᴀᴛɪɴɢ ʏᴏᴜʀ ᴛɪᴛᴀɴɪᴜᴍ ʙᴏᴛ...</b>\n\n⚙️ <i>Configuring clone client...</i>",
                parse_mode=enums.ParseMode.HTML,
            )
        ...
        try:
            ...
            if status_msg is not None:
                await safe_edit(status_msg.edit_text, final_text, parse_mode=enums.ParseMode.HTML)
            else:
                await client.send_message(owner_id, final_text, parse_mode=enums.ParseMode.HTML)
        except Exception:
            pass
        return
    ...
    try:
        final_text = (
            ...
            f"<b>ᴄʀᴇᴀᴛᴇᴅ ᴠɪᴀ:</b> MANAGED BOTS API (BOT API 9.6)\n\n"
            f"Your clone bot is now running. All downloads will use it automatically to avoid floodwait on the main bot.\n\n"
...
def _titanium_panel_buttons(bots):
    rows = [[
        make_button("📊 sᴛᴀᴛᴜs", callback_data="titanium_status_detail", style=_BS.PRIMARY if _BS else None),
        make_button("🏓 ᴘɪɴɢ", callback_data="titanium_ping", style=_BS.PRIMARY if _BS else None),
    ]]
    for b in bots:
        rows.append([make_button(f"🤖 @{b['username']}", callback_data=f"titanium_viewbot:{b['username']}", style=_BS.PRIMARY if _BS else None)])
    if len(bots) < MAX_TITANIUM_BOTS:
        rows.append([make_button("➕ ᴀᴅᴅ ʙᴏᴛ", callback_data="titanium_addbot", style=_BS.PRIMARY if _BS else None)])
    if bots:
        rows.append([make_button("🔄 ʀᴇᴘʟᴀᴄᴇ ʙᴏᴛ", callback_data="titanium_replace", style=_BS.PRIMARY if _BS else None)])
    if _managed_bots_available() and len(bots) < MAX_TITANIUM_BOTS:
        rows.append([make_button("🤖 ᴀᴜᴛᴏ-ᴄʀᴇᴀᴛᴇ ʙᴏᴛ", callback_data="titanium_autocreate", style=_BS.PRIMARY if _BS else None)])
    if bots:
        rows.append([make_button("❌ ᴅɪsᴀʙʟᴇ ᴛɪᴛᴀɴɪᴜᴍ", callback_data="titanium_disable", style=_BS.DANGER if _BS else None)])
    rows.append([make_button("⬅️ ʙᴀᴄᴋ", callback_data="start_btn", style=_BS.PRIMARY if _BS else None)])
    return InlineKeyboardMarkup(rows)


def _titanium_panel_text(bots) -> str:
    """The main Titanium panel — reached from /settings, /titanium, and
    every "⬅️ ᴛɪᴛᴀɴɪᴜᴍ"/"BACK" button in this file. Short summary, one
    line per bot; full details live on the separate STATUS screen below.
    """
    lines = ["<b>⚡ ᴛɪᴛᴀɴɪᴜᴍ ᴄʟᴏɴᴇ ᴍᴏᴅᴇ</b>", ""]
    if not bots:
        lines += [
        ...
        for b in bots:
            src = "🤖 ᴍᴀɴᴀɢᴇᴅ ʙᴏᴛs ᴀᴘɪ" if b.get("source") == "managed" else "🔑 ᴍᴀɴᴜᴀʟ (/addbot)"
            is_running = bool(_CLONE_CACHE.get(b["token"]) and _CLONE_CACHE[b["token"]].is_connected)
            db_status = b.get("status", "active")
            status_line = f"🟢 ʀᴜɴɴɪɴɢ ({db_status})" if is_running else f"🔴 sᴛᴏᴘᴘᴇᴅ ({db_status})"
            lines += [
                f"<b>ʙᴏᴛ:</b> @{b['username']}",
                f"<b>sᴏᴜʀᴄᴇ:</b> {src}",
                f"<b>sᴛᴀᴛᴜs:</b> {status_line}",
                "",
            ]
        lines.append(
...
def _titanium_status_detail_text(bots) -> str:
    """The detailed STATUS screen — reached only by tapping "📊 sᴛᴀᴛᴜs" on
    the panel above. Every field, per bot: source, live connection state,
    DB status, and timestamps.
    """
    if not bots:
        return "<b>📊 ᴛɪᴛᴀɴɪᴜᴍ sᴛᴀᴛᴜs</b>\n\nNo clone bots connected yet."
    lines = ["<b>📊 ᴛɪᴛᴀɴɪᴜᴍ sᴛᴀᴛᴜs</b>", ""]
    for b in bots:
        src = "🤖 ᴍᴀɴᴀɢᴇᴅ ʙᴏᴛs ᴀᴘɪ (ʙᴏᴛ ᴀᴘɪ 9.6)" if b.get("source") == "managed" else "🔑 ᴍᴀɴᴜᴀʟ (/addbot)"
        running = "✅ ʏᴇs" if (_CLONE_CACHE.get(b["token"]) and _CLONE_CACHE[b["token"]].is_connected) else "❌ ɴᴏ"
        db_status = b.get("status", "active")
        ...
        lines += [
            ...
            f"<b>ᴅʙ sᴛᴀᴛᴜs:</b> {db_status}",
            f"<b>ᴄʀᴇᴀᴛᴇᴅ:</b> {created}",
            f"<b>ʟᴀsᴛ ᴠᴀʟɪᴅᴀᴛᴇᴅ:</b> {last_validated}",
            "",
        ]
    return "\n".join(lines).rstrip()
...
@Client.on_callback_query(filters.regex(r"^titanium_status_detail$"))
async def titanium_status_detail_callback(client: Client, query: CallbackQuery):
    bots = await db.get_titanium_bots(query.from_user.id)
    try:
        await safe_edit(query.edit_message_text, 
            _titanium_status_detail_text(bots),
            parse_mode=enums.ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[make_button("⬅️ ᴛɪᴛᴀɴɪᴜᴍ", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]]),
        )
        ...
        if "MESSAGE_NOT_MODIFIED" in str(e):
            ...
            logger.warning(f"Titanium status detail refresh failed: {e}")
            await query.answer("Couldn't refresh, try again.", show_alert=True)
...
async def titanium_replace_callback(client: Client, query: CallbackQuery):
    ...
    rows.append([make_button("⬅️ ʙᴀᴄᴋ", callback_data="titanium_status",
                             style=_BS.PRIMARY if _BS else None)])
    await query.answer()
    await safe_edit(query.edit_message_text, 
        "<b>ᴡʜɪᴄʜ ᴄᴏɴɴᴇᴄᴛᴇᴅ ʙᴏᴛ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ʀᴇᴘʟᴀᴄᴇ?</b>",
        parse_mode=enums.ParseMode.HTML,
...
async def _start_replace_wait(client: Client, query: CallbackQuery, owner_id: int, old_username: str):
    ...
    if old_match and old_match.get("source") == "managed" and old_match.get("bot_id"):
        return await _auto_rotate_managed_bot(client, query, owner_id, old_match)

    # Manual (/addbot) bot — this bot has no API access to mint a fresh
    # token for it, only @BotFather's own /revoke does that. Fall back to
    # asking the owner to paste one.
...
async def _auto_rotate_managed_bot(client: Client, query: CallbackQuery, owner_id: int, old_match: dict):
    """Fully automatic token rotation for Auto-Create ("managed") bots —
    no manual @BotFather paste needed. Telegram's replaceManagedBotToken
    API both kills the old token AND hands back a fresh one in the same
    call, so the whole thing is a handful of progress-bar steps.
    """
    await query.answer()
    old_username = old_match["username"]
    status = query.message
    try:
        await safe_edit(status.edit_text, f"<b>🔄 ʀᴏᴛᴀᴛɪɴɢ ᴛᴏᴋᴇɴ...</b>", parse_mode=enums.ParseMode.HTML)
    except Exception:
        pass

    await safe_edit(status.edit_text, 
        f"<b>🔄 ʀᴏᴛᴀᴛɪɴɢ ᴛᴏᴋᴇɴ...</b>\n\n🔑 <i>Revoking old token...</i>", parse_mode=enums.ParseMode.HTML
    )
    new_token = await _revoke_managed_bot_token(old_match.get("bot_id"))
    if not new_token:
        return await safe_edit(status.edit_text, 
            f"<b>{E_CROSS} Couldn't rotate the token automatically.</b>\n\n"
            f"Telegram's replaceManagedBotToken call failed — try again in a bit, "
            f"or use /addbot with a manual @BotFather token instead.",
            parse_mode=enums.ParseMode.HTML,
        )

    await safe_edit(status.edit_text, 
        f"<b>🔄 ʀᴏᴛᴀᴛɪɴɢ ᴛᴏᴋᴇɴ...</b>\n\n📡 <i>Requesting new token from Telegram...</i>", parse_mode=enums.ParseMode.HTML
    )
    try:
        test_client = Client(
            f"titanium_verify_{owner_id}_{int(time.time())}",
        ...
        return await safe_edit(status.edit_text, 
            f"<b>{E_CROSS} Got a new token but it didn't verify:</b> <code>{strip_ansi(e)}</code>",
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    if cached is not None:
        try:
            ...
            logger.debug(f"auto-rotate: stop() on old clone failed (likely already stopped): {e}")

    await db.add_titanium_bot(owner_id, new_token, me.username, source="managed", bot_id=me.id)

    await safe_edit(status.edit_text, 
        f"<b>🔄 ʀᴏᴛᴀᴛɪɴɢ ᴛᴏᴋᴇɴ...</b>\n\n🚀 <i>Restarting clone client...</i>", parse_mode=enums.ParseMode.HTML
    )
    try:
        await _get_clone_client(new_token, owner_id)
    except Exception as e:
        logger.warning(f"auto-rotate: full clone start failed for @{me.username}: {e}")

    await safe_edit(status.edit_text, 
        f"<b>🔄 ʀᴏᴛᴀᴛɪɴɢ ᴛᴏᴋᴇɴ...</b>\n\n🔒 <i>Encrypting &amp; storing new token...</i>", parse_mode=enums.ParseMode.HTML
    )

    await safe_edit(status.edit_text, 
        f"<b>✅ ᴛᴏᴋᴇɴ ʀᴏᴛᴀᴛᴇᴅ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ.</b>\n\n"
        f"<b>ʙᴏᴛ:</b> @{me.username}\n\n"
        f"The old token was revoked and a new one is active — no manual "
        f"@BotFather step needed.\n\n"
        f"Manage it via /settings → ⚡ Titanium Clone Mode.",
        reply_markup=InlineKeyboardMarkup([[make_button("⬅️ ᴛɪᴛᴀɴɪᴜᴍ", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
...
async def titanium_replace_token_catch(client: Client, message: Message):
    ...
    status = await message.reply_text(f"<b>🔄 ʀᴏᴛᴀᴛɪɴɢ ᴛᴏᴋᴇɴ...</b>", parse_mode=enums.ParseMode.HTML)
    try:
        test_client = Client(
            f"titanium_verify_{owner_id}_{int(time.time())}",
            api_id=API_ID, api_hash=API_HASH, bot_token=new_token, in_memory=True
        ...
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Invalid token:</b> <code>{strip_ansi(e)}</code>", parse_mode=enums.ParseMode.HTML)
    ...
    await asyncio.sleep(0.6)  # purely cosmetic — the token was already supplied above, nothing to actually wait on here
    ...
    await asyncio.sleep(0.5)  # cosmetic — db.add_titanium_bot above already persisted it

    warn_line = ""
    if old_match and not was_managed:
        warn_line = (
            f"\n\nNote: old token was added manually (/addbot), so it couldn't be "
            f"revoked automatically — it's disconnected here, but still valid until you "
            f"revoke it via @BotFather."
        )

    await safe_edit(status.edit_text, 
        f"<b>✅ ᴛᴏᴋᴇɴ ʀᴏᴛᴀᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ.</b>\n\n"
        f"<b>ʙᴏᴛ:</b> @{me.username}\n\n"
        f"The old token has been revoked and a new one is active.\n\n"
        f"Manage it via /settings → ⚡ Titanium Clone Mode.{warn_line}",
        reply_markup=InlineKeyboardMarkup([[make_button("⬅️ ᴛɪᴛᴀɴɪᴜᴍ", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
...
async def titanium_disable_callback(client: Client, query: CallbackQuery):
    ...
    rows = [[make_button(f"@{b['username']}", callback_data=f"titanium_disable_pick:{b['username']}",
                         style=_BS.PRIMARY if _BS else None)] for b in bots]
    rows.append([make_button(" ❌ ᴅɪsᴀʙʟᴇ ᴀʟʟ ", callback_data="titanium_disable_pick:__all__",
                             style=_BS.DANGER if _BS else None)])
    rows.append([make_button("⬅️ ʙᴀᴄᴋ", callback_data="titanium_status",
                             style=_BS.PRIMARY if _BS else None)])
    await safe_edit(query.edit_message_text, 
        "<b>ᴡʜɪᴄʜ ᴄᴏɴɴᴇᴄᴛᴇᴅ ʙᴏᴛ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴅɪsᴀʙʟᴇ?</b>",
        parse_mode=enums.ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(rows),
    )
...
async def _start_disable_confirm(client: Client, query: CallbackQuery, username: str, total: int):
    ...
    await query.answer()
    rows = [[
        make_button(" ✅ ʏᴇs, ᴅɪsᴀʙʟᴇ ", callback_data=f"titanium_disable_confirm:{username}", style=_BS.DANGER if _BS else None),
        make_button(" ❌ ᴄᴀɴᴄᴇʟ ", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None),
    ]]
    if username == "__all__":
        text = (
    ...
    await safe_edit(query.edit_message_text, text, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(rows))
...
async def titanium_disable_confirm_callback(client: Client, query: CallbackQuery):
    ...
    if not targets:
        ...
        status = query.message
        await safe_edit(status.edit_text, 
            f"<b>🔄 ᴅɪsᴀʙʟɪɴɢ...</b>\n\n🛑 <i>Stopping clone client(s)...</i>", parse_mode=enums.ParseMode.HTML
        )
        for b in targets:
        ...
        await safe_edit(status.edit_text, 
            f"<b>🔄 ᴅɪsᴀʙʟɪɴɢ...</b>\n\n🔑 <i>Revoking token(s) with Telegram...</i>", parse_mode=enums.ParseMode.HTML
        )
        any_unrevoked_manual = False
        for b in targets:
            if b.get("source") == "managed" and b.get("bot_id"):
                await _revoke_managed_bot_token(b.get("bot_id"))
            else:
                any_unrevoked_manual = True

        await safe_edit(status.edit_text, 
            f"<b>🔄 ᴅɪsᴀʙʟɪɴɢ...</b>\n\n🗑️ <i>Wiping stored credentials...</i>", parse_mode=enums.ParseMode.HTML
        )
        for b in targets:
            await db.remove_titanium_bot(owner_id, b["username"])

        wiped_line = "Your clone bot has been stopped and the token has been revoked & wiped."
        if any_unrevoked_manual:
            wiped_line = (
                "Your clone bot has been stopped and removed from storage. Note: at least one "
                "of these was added manually (/addbot), so its token couldn't be revoked via the "
                "API — it's disconnected here, but still valid until you revoke it via @BotFather."
            )

        disabled_text = (
            f"<b>✅ ᴛɪᴛᴀɴɪᴜᴍ ᴄʟᴏɴᴇ ᴍᴏᴅᴇ ᴅɪsᴀʙʟᴇᴅ.</b>\n\n"
            f"{wiped_line} "
            f"You can re-enable it anytime via /settings."
        )
        await safe_edit(query.edit_message_text, 
            disabled_text,
            parse_mode=enums.ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[make_button("⬅️ sᴇᴛᴛɪɴɢs", callback_data="settings_back_btn", style=_BS.PRIMARY if _BS else None)]]),
        )
        return
...
async def _do_add_bot(client: Client, user_id: int, token: str, status: Message, ask_msg: Message = None):
    """Shared ❪ SEND ME YOUR BOT TOKEN ❫ pipeline — verifies the token,
    walks through the same staged progress edits as the reference bot
    (Verifying → Connecting → Fetching bot details → Saving to database),
    then edits `status` into the final success/error card. `ask_msg`,
    if given, is the original prompt bubble and gets deleted on success
    (mirrors the reference bot deleting the user's pasted-token message)."""
    ...
    if any(b["token"] == token for b in bots):
        return await safe_edit(status.edit_text, f"<b>{E_INFO} That bot is already connected.</b>", parse_mode=enums.ParseMode.HTML)

    await safe_edit(status.edit_text, f"<b>⏳ Verifying bot token...</b>", parse_mode=enums.ParseMode.HTML)
    try:
        test_client = Client(
        ...
        await test_client.start()
        await safe_edit(status.edit_text, f"<b>🔄 Connecting to bot...</b>", parse_mode=enums.ParseMode.HTML)
        me = await test_client.get_me()
        await safe_edit(status.edit_text, f"<b>📡 Fetching bot details...</b>", parse_mode=enums.ParseMode.HTML)
        await test_client.stop()
    except Exception as e:
        return await safe_edit(status.edit_text, f"<b>⚠️ Bot error:</b> <code>{strip_ansi(e)}</code>", parse_mode=enums.ParseMode.HTML)

    if any(b["username"] == me.username for b in bots):
        return await safe_edit(status.edit_text, f"<b>{E_INFO} @{me.username} is already connected.</b>", parse_mode=enums.ParseMode.HTML)

    await safe_edit(status.edit_text, f"<b>💾 Saving bot to database...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    back_btn = InlineKeyboardMarkup([[make_button("↩ ʙᴀᴄᴋ ᴛᴏ sᴇᴛᴛɪɴɢs", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]])
    success_text = (
        f"<b>✅ Bot added successfully!</b>\n\n"
        f"<b>📝 Name:</b> <code>{me.first_name}</code>\n"
        f"<b>🆔 Bot ID:</b> <code>{me.id}</code>\n"
    )
    if me.username:
        success_text += f"<b>🤖 Username:</b> @{me.username}"
    await safe_edit(status.edit_text, success_text, parse_mode=enums.ParseMode.HTML, reply_markup=back_btn)
...
async def addbot_cmd(client: Client, message: Message):
    ...
    # /addbot <token> still works as a one-shot shortcut...
    if len(message.command) >= 2:
        status = await message.reply_text(f"<b>⏳ Verifying bot token...</b>", parse_mode=enums.ParseMode.HTML)
        return await _do_add_bot(client, user_id, message.command[1].strip(), status)

    # ...but with no token, walk through the same interactive prompt as
    # the reference bot: ask, wait for the pasted token, then verify.
    ask = await message.reply_text(
        f"<b>📤 Send me your bot token from</b> @BotFather\n\n"
        f"<b>📝 Example:</b> <code>1234567890:ABCdefGhIJKlmNoPQRsTUVwxyZ</code>\n\n"
    ...
    match = _re_token.search(r"\d[0-9]{8,10}:[0-9A-Za-z_-]{35}", text)
    if not match:
        return await safe_edit(ask.edit_text, f"<b>{E_CROSS} Invalid bot token — please send a valid token from @BotFather.</b>", parse_mode=enums.ParseMode.HTML)

    await _do_add_bot(client, user_id, match.group(0), ask, ask_msg=resp)


@Client.on_callback_query(filters.regex(r"^titanium_addbot$"))
async def titanium_addbot_callback(client: Client, query: CallbackQuery):
    await query.answer()
    ...
    try:
        resp = await wait_for_reply(client, chat_id=query.message.chat.id, user_id=user_id, timeout=120)
    except asyncio.TimeoutError:
        return await safe_edit(ask.edit_text, f"<b>{E_CROSS} Timed out — nothing changed.</b>", parse_mode=enums.ParseMode.HTML)
...
@Client.on_callback_query(filters.regex(r"^titanium_viewbot:"))
async def titanium_viewbot_callback(client: Client, query: CallbackQuery):
    await query.answer()
    user_id = query.from_user.id
    username = query.data.split(":", 1)[1]
    bots = await db.get_titanium_bots(user_id)
    ...
    if not b:
        return await safe_edit(query.message.edit_text,
            f"<b>{E_CROSS} That bot is no longer connected.</b>",
            parse_mode=enums.ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[make_button("⬅️ ᴛɪᴛᴀɴɪᴜᴍ", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]]),
        )
    ...
    is_running = bool(_CLONE_CACHE.get(b["token"]) and _CLONE_CACHE[b["token"]].is_connected)
    status_line = f"🟢 Running ({b.get('status', 'active')})" if is_running else f"🔴 Stopped ({b.get('status', 'active')})"

    buttons = InlineKeyboardMarkup([
        [make_button("❌ Remove", callback_data=f"titanium_removebot:{username}", style=_BS.DANGER if _BS else None)],
        [make_button("↩ Back", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)],
    ])
...
@Client.on_callback_query(filters.regex(r"^titanium_removebot:"))
async def titanium_removebot_callback(client: Client, query: CallbackQuery):
    await query.answer()
    user_id = query.from_user.id
    username = query.data.split(":", 1)[1]
    ...
    if not removed:
        return await safe_edit(query.message.edit_text, f"<b>{E_INFO} No connected bot found with that username.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    if match:
        ...
        if cached is not None:
            try:
                ...
                logger.debug(f"titanium_removebot: stop() on evicted clone failed (likely already stopped): {e}")

    back_btn = InlineKeyboardMarkup([[make_button("↩ Back", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]])
    await safe_edit(query.message.edit_text, f"<b>✅ Bot removed successfully!</b>\n\n<b>🤖 Username:</b> @{username}", parse_mode=enums.ParseMode.HTML, reply_markup=back_btn)
