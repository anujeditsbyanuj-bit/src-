...
def _titanium_panel_text(bots) -> str:
    ...
    if not bots:
        ...
        lines = ["<b>📊 TITANIUM STATUS</b>", ""]
        for b in bots:
            src = "🤖 MANAGED BOTS API (BOT API 9.6)" if b.get("source") == "managed" else "🔑 MANUAL (/addbot)"
            running = "✅ YES" if (_CLONE_CACHE.get(b["token"]) and _CLONE_CACHE[b["token"]].is_connected) else "❌ NO"
            ...
            lines += [
                ...
                f"<b>LAST VALIDATED:</b> {last_validated}",
                "",
            ]
    return "\n".join(lines).rstrip()
...
async def titanium_replace_token_catch(client: Client, message: Message):
    ...
    try:
        ...
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} Invalid token:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    ...
    await safe_edit(status.edit_text, 
        f"<b>✅ Token rotated successfully.</b>\n\n"
        f"<b>Bot:</b> @{me.username}\n\n"
        f"The old token has been revoked and a new one is active.\n\n"
        f"Manage it via /settings → ⚡ Titanium Clone Mode.{warn_line}",
        reply_markup=InlineKeyboardMarkup([[make_button("⬅️ Titanium", callback_data="titanium_status", style=_BS.PRIMARY if _BS else None)]]),
        parse_mode=enums.ParseMode.HTML,
    )
...
async def titanium_disable_confirm_callback(client: Client, query: CallbackQuery):
    ...
    if not targets:
        ...
        disabled_text = (
            f"<b>✅ Titanium Clone Mode disabled.</b>\n\n"
            f"Your clone bot has been stopped and the token has been wiped. "
            f"You can re-enable it anytime via /settings."
        )
        await safe_edit(query.edit_message_text, 
            disabled_text,
            parse_mode=enums.ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[make_button("⬅️ Settings", callback_data="settings_back_btn", style=_BS.PRIMARY if _BS else None)]]),
        )
        return
