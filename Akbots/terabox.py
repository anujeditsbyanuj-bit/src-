...
async def get_file_info_from_api(share_url: str) -> dict:
    ...
    xAPIverse resolver first, falls back to terabox.beer (no API key
    needed), and finally to the direct-API tier (Akbots/terabox_lib/,
    requires TERABOX_NDUS) if both fail, so a dead/rate-limited key doesn't
    take TeraBox support down entirely."""
    try:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    ...
                    f"terabox.beer: {fallback_err} | direct-API: {direct_err}"
                )
...
async def get_folder_info_from_api(share_url: str, password: str | None = None) -> list[dict]:
    ...
    (no key needed at all) -> direct-API (Akbots/terabox_lib/, needs
    TERABOX_NDUS — transfers into an owned account as a last resort). Each
    one is only tried if the previous one fails, so a missing Pro key just
    means no quality picker — not a broken /terabox command."""
    try:
...
async def _download_hls_via_ffmpeg(url: str, out_path: str, status_msg: Message, name: str, total_size_hint: int = 0) -> float:
...
def _probe_video_metadata(path: str) -> tuple[int, int, int]:
    ...
    duration info at all before. Never raises; returns (0, 0, 0) on any
    failure so a bad probe never blocks the actual upload."""
...
async def _process_terabox(
    ...
    if preselected is not None:
        ...
        if len(files) > 1:
            ...
            buttons.append([make_button("📥 ᴅᴏᴡɴʟᴏᴀᴅ ᴀʟʟ", callback_data=f"teraall:{token}", style=ButtonStyle.PRIMARY if BUTTON_STYLE_SUPPORTED else None)])
