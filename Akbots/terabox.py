...
async def _process_terabox(
    ...
    if preselected is not None:
        ...
        await safe_edit(status_msg.edit, "🔍 Fetching file info...")
        try:
            files = await get_folder_info_from_api(url, password)
        except Exception as e:
            await safe_edit(status_msg.edit, f"❌ Failed to get file info:\n`{e}`")
            return
    ...
    if not info["download_link"]:
        await safe_edit(status_msg.edit, "❌ Could not retrieve download link")
        return
    ...
    try:
                        ...
                        async for chunk in r.content.iter_chunked(chunk_size):
                            if chunk:
                                ...
                                if (current_time - last_update_time >= 4) or (percentage - last_percentage >= 5):
                                    ...
                                    try:
                                        await safe_edit(status_msg.edit, progress_text)
                                    except Exception:
                                        pass

        await safe_edit(status_msg.edit, "📤 **Preparing to upload to Telegram...**")
        ...
        async def upload_progress(current, total):
            ...
            if (current_time - last_upload_update >= 4) or (percentage - last_upload_percentage >= 5):
                ...
                try:
                    await safe_edit(status_msg.edit, progress_text, reply_markup=cancel_button)
                except Exception:
                    pass
        ...
        await safe_edit(status_msg.edit, f"❌ **Download failed:**\n`{str(e)}`")
    except RuntimeError as e:
        await safe_edit(status_msg.edit, f"❌ **HLS download failed (ffmpeg):**\n`{str(e)}`")
    except Exception as e:
        await safe_edit(status_msg.edit, f"❌ **Upload failed:**\n`{str(e)}`")
    finally:
        if _slot_open:
            try:
                await _slot.__aexit__(None, None, None)
...
async def terabox_file_selected(client, callback_query):
    ...
    await safe_edit(callback_query.message.edit, f"🔍 Preparing **{file_info['name']}**...")
...
async def terabox_quality_selected(client, callback_query):
    ...
    await safe_edit(callback_query.message.edit, f"🔍 Preparing **{info['name']}**...")
    await _process_terabox(
        client, callback_query.from_user.id, callback_query.message,
...
async def terabox_download_all(client, callback_query):
    ...
    await callback_query.answer()
    files = entry["files"]
    await safe_edit(callback_query.message.edit, f"📥 **Queuing {len(files)} files from the folder...**")
...
async def terastream_command(client, message: Message):
    ...
    if not stream_url:
        await safe_edit(status_msg.edit, "❌ Could not resolve a stream link for this file.")
        return
