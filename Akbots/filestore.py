...
def cancel_kb():
    return InlineKeyboardMarkup([[make_button(f"{E_CROSS} Cancel", callback_data="fs_cancel", style=_BS.PRIMARY if _BS else None)]])
...
async def genlink_cmd(client: Client, message: Message):
    ...
    if message.reply_to_message:
        ...
        buttons = InlineKeyboardMarkup([[make_button("🔁 Share URL", url=f"https://telegram.me/share/url?url={link}", style=_BS.PRIMARY if _BS else None)]])
    ...
    if auto_gen_on and episode_count > 1:
        proceed = await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                make_button("✅ Generate all qualities", callback_data="fs_batch_genall", style=_BS.SUCCESS if _BS else None),
                make_button("➡️ Plain batch instead", callback_data="fs_batch_plain", style=_BS.PRIMARY if _BS else None),
            ]]),
        )
        try:
...
async def handle_filestore_start(client: Client, message: Message, payload: str) -> bool:
    ...
    if not is_admin and not access_token and await shortener_gate_enabled():
        ...
        buttons = InlineKeyboardMarkup([[make_button("🔓 Get File", url=shortened, style=_BS.PRIMARY if _BS else None)]])
        await message.reply_text(
