...
NOT_CONFIGURED = (
    f"{E_WARN} <b>Heroku tools aren't configured.</b>\n"
...
async def heroku_info_command(client: Client, message: Message):
    ...
    try:
        ...
        info_text = (
            f"<b>Multi-Factor Authentication:</b> {account.get('two_factor_authentication')}\n"
            f"<b>Email:</b> {account.get('email')}\n"
            f"<b>Name:</b> {account.get('name')}\n"
            f"<b>Total Apps:</b> {len(app_rows)}\n"
            f"<b>Total Dynos:</b> {total_dynos}\n"
        )
        for name, dynos_on, dyno_count in app_rows:
            info_text += (
                f"\n<b>App:</b> {name}\n"
                f"<b>Dynos:</b> {'On' if dynos_on else 'Off'}, Total: {dyno_count}\n"
            )
