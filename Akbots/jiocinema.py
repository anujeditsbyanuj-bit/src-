...
async def _open_jiocinema(client: Client, message: Message, url: str):
    ...
    status = await message.reply_text(f"<b>🔎 ʟᴏᴏᴋɪɴɢ ᴜᴘ ᴍᴏᴠɪᴇ...</b>", parse_mode=enums.ParseMode.HTML)
    metadata = await asyncio.to_thread(_get_jio_metadata, m_id)
    if not metadata:
        return await safe_edit(status.edit_text, f"<b>{E_WARN} Movie not found — check the link.</b>",
                                       parse_mode=enums.ParseMode.HTML)
