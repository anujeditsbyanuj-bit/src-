...
def _gen_text(user_name: str, link: str, expiry: int) -> str:
    ...
    return (
        ...
        f"<b>👤 Created by:</b> {user_name}\n\n"
        f"<b>🔗 Link:</b>\n<code>{link}</code>\n\n"
        f"<b>⏳ Expires:</b> {expires} (24 hours from now)"
    )


HELP_TEXT = (
    f"<b>{E_INFO} Voice Chat — How to Use</b>\n\n"
    "<b>1. Generate a link</b> — /vc\n"
    "<b>2. Share it</b> with whoever you want to talk to\n"
    "<b>3. Join</b> — open the link, enter a name, allow mic access\n\n"
...
def _gen_keyboard(link: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🎤 Open Voice Chat", url=link)],
            [InlineKeyboardButton("🔄 New Link", callback_data="vc#gen")],
            [InlineKeyboardButton("📖 Help", callback_data="vc#help")],
        ]
    )
...
def _help_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("🎤 Generate Voice Chat", callback_data="vc#gen")]]
    )
