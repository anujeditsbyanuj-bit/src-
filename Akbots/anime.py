...
def _quality_kb(session_id: str, show_idx: int, ep_idx: int, downloads: list) -> InlineKeyboardMarkup:
    ...
    for d in downloads:
        res = d.get("res", "?")
        row.append(make_button(f"{res}p", callback_data=f"animedl#{session_id}#{show_idx}#{ep_idx}#{res}", style=_BS.PRIMARY if _BS else None))
    return InlineKeyboardMarkup([row, [make_button("⬅️ Back to episodes", callback_data=f"animesh#{session_id}#{show_idx}", style=_BS.DANGER if _BS else None)]])
...
def _episodes_kb(session_id: str, show_idx: int, entries: list) -> InlineKeyboardMarkup:
    ...
    buttons.append([make_button("⬅️ Back to results", callback_data=f"animeback#{session_id}", style=_BS.DANGER if _BS else None)])
