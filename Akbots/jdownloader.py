...
from Akbots.direct_utils import upload_file, format_progress, E_CHECK, E_CROSS, E_INFO, E_ROCKET, safe_edit
...
async def _run_jd_download(client: Client, message: Message, status, link: str):
    ...
    if message.from_user:
        try:
            task_id = task_manager.register(
                message.from_user.id, asyncio.current_task(), f"JDownloader: {link[:40]}"
            )
        except Exception:
            task_id = None
    _slot = task_manager.queue_slot(message.from_user.id, status_msg=status) if message.from_user else None
    ...
    try:
        ...
        last_edit = 0.0
        await safe_edit(status.edit_text, f"<b>{E_ROCKET} Downloading via JDownloader...</b>", parse_mode=enums.ParseMode.HTML)
...
async def try_jd_fallback(client: Client, message: Message, status, url: str) -> bool:
    ...
    if not jdownloader.is_connected:
        return False
    try:
        await safe_edit(status.edit_text, f"<b>{E_INFO} Trying JDownloader...</b>", parse_mode=enums.ParseMode.HTML)
...
async def _handle(client: Client, message: Message, link: str):
    ...
    try:
        ...
        await safe_edit(status.edit_text, f"<b>{E_CROSS} JDownloader error:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
