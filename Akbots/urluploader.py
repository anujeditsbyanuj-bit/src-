...
_EXCLUDED_DOMAINS = (
    ...
    "diskwala.com", "diskwala.app", "flezen.com",
    *TERABOX_DOMAINS,
    *BYPASS_DOMAINS,
    "magnet:", ".torrent",
    ...
    # These 5 were missing here even though ytdl.py already excludes them —
    # own dedicated plugins (jiosaavn.py/spotify.py/dailymotion.py/
    # pocketfm.py) already claim these; without a real yt-dlp probe success
    # (has_quality_formats below only catches Dailymotion, not the others),
    # a link from one of these could slip through and get grabbed here as a
    # raw HTML/audio-page "file" instead of being left for its own plugin.
...
async def _handle(client: Client, message: Message, url: str, custom_name: str = None,
                   username: str = None, password: str = None, cache_key: str = None,
                   caption_builder=None):
    """`cache_key`, when given, overrides what URL caching is keyed on —
    for callers like Akbots/diskwala.py where `url` is a temporary signed
    link that's different on every call (so caching on it would never hit)
    but there's a separate *stable* source URL (the diskwala.com share
    link) worth caching against instead. Without it, behavior is unchanged
    from before this param existed.

    caption_builder: optional callable(download_elapsed, upload_elapsed=None) -> str,
    same shape as terabox.py's own build_caption() — when given, replaces
    the plain "Uploaded\\n<filename>" caption with whatever rich caption
    the caller builds (file size/quality/downloaded-in/uploaded-by/source/
    etc.), and the message is edited again once the real upload_elapsed is
    known, exactly like terabox.py does. Without it, behavior is unchanged."""
    ...
    # Doesn't apply when cache_key is explicitly given — there, custom_name
    # is just the real filename from an API response, not a user rename.
    skip_cache = bool((custom_name or username) and not cache_key)
    effective_cache_url = cache_key or url
    if not skip_cache and await try_send_cached(client, message, effective_cache_url, status):
        return
    filename = safe_filename(url.split("/")[-1].split("?")[0], "downloaded_file")
    if custom_name:
        filename = apply_custom_name(filename, custom_name)
    cache_url = effective_cache_url if not skip_cache else None
    ...
    if _aria2c_available():
        ...
        try:
            ...
            caption = (caption_builder(download_elapsed) if caption_builder
                       else f"<b>{E_CHECK} Uploaded</b>\n<code>{filename}</code>")
            caption_after = (lambda up: caption_builder(download_elapsed, upload_elapsed=up)) if caption_builder else None
            await upload_file(
                client, message, path, status, caption,
                file_name=filename, cache_url=cache_url, caption_after_upload=caption_after
            )
    ...
    try:
        ...
        caption_after = (lambda up: caption_builder(download_elapsed, upload_elapsed=up)) if caption_builder else None
        await upload_file(
            client, message, dest, status, caption,
            file_name=filename, cache_url=cache_url, caption_after_upload=caption_after
        )
    except Exception as e:
        await _try_jd_then_error(client, message, status, url, e)
...
async def url_upload_command(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text(
            ...
            f"<b>ᴇxᴀᴍᴘʟᴇ:</b> <code>/url https://example.com/video.mp4 -n MyVideo</code>\n\n"
            f"<b>ᴘʀɪᴠᴀᴛᴇ/ʟᴏɢɪɴ-ᴡᴀʟʟᴇᴅ ʟɪɴᴋ (ʜᴛᴛᴘ ʙᴀsɪᴄ ᴀᴜᴛʜ):</b>\n"
