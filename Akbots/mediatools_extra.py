...
async def togif_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>Video → GIF</b>\n\n"
            f"Reply to a <b>video</b> with <code>/togif [start] [duration]</code>.\n"
    ...
    await status.edit_text(f"<b>{E_GEAR} Converting to GIF...</b>", parse_mode=enums.ParseMode.HTML)
    ...
    try:
        ...
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} GIF conversion failed:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
...
async def audio8d_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>8D Audio</b>\n\n"
            f"Reply to an <b>audio file</b> with <code>/audio8d</code> to apply an "
...
async def bassboost_cmd(client: Client, message: Message):
    ...
    if not media or not args:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>Bass Booster</b>\n\n"
            f"Reply to an <b>audio file</b> with <code>/bassboost &lt;-20 to 20&gt;</code>.\n"
...
async def trebleboost_cmd(client: Client, message: Message):
    ...
    if not media or not args:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>Treble Booster</b>\n\n"
            f"Reply to an <b>audio file</b> with <code>/trebleboost &lt;-20 to 20&gt;</code>.\n"
...
async def eq_cmd(client: Client, message: Message):
    ...
    if not media or len(args) < 4:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>Music Equalizer</b>\n\n"
            f"Reply to an <b>audio file</b> with:\n"
...
async def slowreverb_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>Slowed + Reverb</b>\n\n"
            f"Reply to an <b>audio file</b> with <code>/slowreverb</code> for the "
...
async def jsonformat_cmd(client: Client, message: Message):
    ...
    if not media:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>JSON Formatter</b>\n\n"
            f"Reply to a <b>.json file</b> with <code>/jsonformat [indent]</code> "
    ...
    try:
        ...
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} Not valid JSON:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
        shutil.rmtree(temp_dir, ignore_errors=True)
...
async def subconvert_cmd(client: Client, message: Message):
    ...
    if media and orig_name.lower().endswith(".sbv"):
        return await message.reply_text(
            f"<blockquote>{E_WARN} <b>.sbv isn't supported.</b>\n\n"
    ...
    if not media or not orig_name.lower().endswith(SUBTITLE_EXTS) or not args:
        return await message.reply_text(
            f"<blockquote>{E_GEAR} <b>Subtitle Converter</b>\n\n"
            f"Reply to a <b>.srt / .vtt / .ass / .ssa</b> file with "
    ...
    await status.edit_text(f"<b>{E_GEAR} Converting to .{target}...</b>", parse_mode=enums.ParseMode.HTML)
    try:
        ...
        shutil.rmtree(temp_dir, ignore_errors=True)
        return await status.edit_text(f"<b>{E_CROSS} Conversion failed:</b>\n<code>{e}</code>", parse_mode=enums.ParseMode.HTML)
