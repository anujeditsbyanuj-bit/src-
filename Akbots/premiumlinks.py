...
from Akbots.bypassers.linkbypass import bypass_linkbypass, is_linkbypass_url, LINKBYPASS_DOMAINS
from Akbots.bypassers.wpsafelink import bypass_wpsafelink, is_wpsafelink_url, WPSAFELINK_DOMAINS
from Akbots.moviesite_scrape import scrape_moviesite, is_moviesite_url, MOVIESITE_DOMAINS
...
BYPASS_DOMAINS = (
    "filepress", "hubdrive", "gdlink", "gdtot", "gdflix", "hubcloud", "vifix.site", "lksfy.com",
    *LINKBYPASS_DOMAINS, *WPSAFELINK_DOMAINS, *MOVIESITE_DOMAINS,
)
...
async def _run_bypass_download(client: Client, message: Message, url: str, scraper_fn, service_label: str):
    ...
    if data.get("is_pack"):
        if data.get("episodes"):
            ...
            for i, ep in enumerate(data["episodes"], start=1):
                lines.append(f"{i}. 📚 <b>ᴛɪᴛʟᴇ:</b> {ep.get('title', 'Unknown')}")
                lines.append(f"┃")
        ...
        return await safe_edit(status.edit_text, text, parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
    ...
    try:
        await safe_edit(status.edit_text, f"<b>{E_ROCKET} Downloading: {title}</b>", parse_mode=enums.ParseMode.HTML)
...
@Client.on_message(filters.text & filters.private & filters.regex(r"(https?://)?[\w.-]*(" + "|".join(re.escape(d) for d in LINKBYPASS_DOMAINS) + r")") & ~filters.regex(r"^/"), group=1)
async def linkbypass_auto_detect(client: Client, message: Message):
    match = re.search(r"https?://\S+", message.text)
    url = match.group(0) if match else None
    if url and is_linkbypass_url(url):
        await _run_linkbypass_bypass(client, message, url)


@Client.on_message(filters.text & filters.private & filters.regex(r"(https?://)?[\w.-]*(" + "|".join(re.escape(d) for d in WPSAFELINK_DOMAINS) + r")") & ~filters.regex(r"^/"), group=1)
async def wpsafelink_auto_detect(client: Client, message: Message):
    match = re.search(r"https?://\S+", message.text)
    url = match.group(0) if match else None
    if url and is_wpsafelink_url(url):
        await _run_wpsafelink_bypass(client, message, url)


@Client.on_message(filters.text & filters.private & filters.regex(r"(https?://)?[\w.-]*(" + "|".join(re.escape(d) for d in MOVIESITE_DOMAINS) + r")") & ~filters.regex(r"^/"), group=1)
async def moviesite_auto_detect(client: Client, message: Message):
    match = re.search(r"https?://\S+", message.text)
    url = match.group(0) if match else None
    if url and is_moviesite_url(url):
        await _run_moviesite_scrape(client, message, url)
...
async def _run_lksfy_bypass(client: Client, message: Message, url: str):
    ...
    dest = os.path.join(OUTPUT_FOLDER, f"{message.id}_{filename}")
    try:
        await safe_edit(status.edit_text, f"<b>{E_ROCKET} Resolved — downloading...</b>", parse_mode=enums.ParseMode.HTML)
...
async def _run_linkbypass_bypass(client: Client, message: Message, url: str):
    status = await message.reply_text(
        f"<b>{E_INFO} Link detected — bypassing...</b>", parse_mode=enums.ParseMode.HTML
    )
    final = await bypass_linkbypass(url)
    if not final:
        return await safe_edit(status.edit_text, 
            f"<b>{E_CROSS} Couldn't bypass this link.</b>\n"
            f"<i>Tried both the fast HTTP method and a real headless-browser "
            f"pass (waits out countdown timers, clicks common \"Continue\"/"
            f"\"Get Link\" buttons). If this page is gated behind a CAPTCHA, "
            f"that's the likely reason — neither tier solves those. See "
            f"Akbots/bypassers/linkbypass.py's docstring for details.</i>",
            parse_mode=enums.ParseMode.HTML,
        )
    await _finish_resolved_link(client, message, status, final, "linkbypass")


async def _run_wpsafelink_bypass(client: Client, message: Message, url: str):
    status = await message.reply_text(
        f"<b>{E_INFO} Link detected — bypassing...</b>", parse_mode=enums.ParseMode.HTML
    )
    final = await bypass_wpsafelink(url)
    if not final:
        return await safe_edit(status.edit_text,
            f"<b>{E_CROSS} Couldn't bypass this link.</b>\n"
            f"<i>Either this site is behind a Cloudflare challenge page (needs a "
            f"real browser to pass), its markup/API changed, or the request was "
            f"rejected. See Akbots/bypassers/wpsafelink.py's docstring for details.</i>",
            parse_mode=enums.ParseMode.HTML,
        )
    await _finish_resolved_link(client, message, status, final, "wpsafelink")


async def _run_moviesite_scrape(client: Client, message: Message, url: str):
    status = await message.reply_text(
        f"<b>{E_INFO} Movie/anime page detected — scraping...</b>", parse_mode=enums.ParseMode.HTML
    )
    result = await scrape_moviesite(url)
    if not result:
        return await safe_edit(status.edit_text,
            f"<b>{E_CROSS} Couldn't find any download links on this page.</b>\n"
            f"<i>The page's layout may have changed, or it's behind an anti-bot "
            f"check this scraper doesn't handle. See Akbots/moviesite_scrape.py's "
            f"docstring for details.</i>",
            parse_mode=enums.ParseMode.HTML,
        )
    # Unlike the single-link bypassers, this is a formatted multi-link
    # summary meant to be read, not chained into a downloader — send it
    # as-is rather than passing it to _finish_resolved_link.
    if len(result) > 4000:
        for i in range(0, len(result), 4000):
            await message.reply_text(result[i:i + 4000], parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
        await status.delete()
    else:
        await safe_edit(status.edit_text, result, parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
...
async def hblinks_option_callback(client: Client, query: CallbackQuery):
    ...
    await query.answer()
    await safe_edit(query.message.edit_text, f"<b>{E_INFO} {label}</b> — resolving...", parse_mode=enums.ParseMode.HTML)
