...
def _quality_kb(session_id: str, audio_count: int, video_count: int) -> InlineKeyboardMarkup:
    ...
    for i in range(audio_count):
        rows.append([make_button(f"ᴀᴜᴅɪᴏ ᴏᴘᴛɪᴏɴ {i + 1}", callback_data=f"pfmq#{session_id}#a{i}", style=_BS.PRIMARY if _BS else None)])
    for i in range(video_count):
        rows.append([make_button(f"ᴠɪᴅᴇᴏ ᴏᴘᴛɪᴏɴ {i + 1}", callback_data=f"pfmq#{session_id}#v{i}", style=_BS.PRIMARY if _BS else None)])
    rows.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"pfmcancel#{session_id}", style=_BS.DANGER if _BS else None)])
...
async def _start_episode(client: Client, message: Message, show_id: str, story_id: str, status: Message = None):
    ...
    try:
        ...
        if not story:
            return await safe_edit(status.edit_text, f"<b>{E_CROSS} Episode not found in this show's story list.</b>", parse_mode=enums.ParseMode.HTML)
        ...
        await safe_edit(status.edit_text, f"<b>{E_INFO} Checking available qualities...</b>", parse_mode=enums.ParseMode.HTML)
        pfm = PocketFM()
        ...
        await safe_edit(status.edit_text, f"<b>{E_CROSS} {e}</b>", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
        await safe_edit(status.edit_text, f"<b>{E_CROSS} Failed:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
...
async def _show_episode_list(client: Client, message: Message, show_id: str):
    ...
    if not stories:
        return await safe_edit(status.edit_text, f"<b>{E_CROSS} No episodes found for this show.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    buttons = [
        [make_button(f"🎧 {title[:60]}", callback_data=f"pfmep#{session_id}#{i}", style=_BS.PRIMARY if _BS else None)]
        for i, (_, title) in enumerate(episodes[:25])
    ]
    buttons.append([make_button("❌ ᴄᴀɴᴄᴇʟ", callback_data=f"pfmcancel#{session_id}", style=_BS.DANGER if _BS else None)])
...
async def _download_and_send(client: Client, message: Message, status: Message,
    ...
    try:
        ...
        caption = f"<blockquote><b>🎧 {story_title}</b>\n<i>{show_title} — via Pocket FM</i></blockquote>"
        ...
        await safe_edit(status.edit_text, f"<b>{E_CROSS} Download failed:</b> <code>{e}</code>", parse_mode=enums.ParseMode.HTML)
    finally:
        try:
...
async def pocketfm_quality_callback(client: Client, callback_query: CallbackQuery):
    ...
    if idx >= len(opt_list):
        return await safe_edit(callback_query.message.edit_text, f"<b>{E_CROSS} Invalid selection.</b>", parse_mode=enums.ParseMode.HTML)
    opt = opt_list[idx]
    ...
    rows = [[make_button(f"🎧 ᴀᴜᴅɪᴏ {i + 1}", callback_data=f"pfmq#{session_id}#a{i}", style=_BS.PRIMARY if _BS else None)] for i in range(len(audio_options))]
    rows += [[make_button(f"🎬 ᴠɪᴅᴇᴏ {i + 1}", callback_data=f"pfmq#{session_id}#v{i}", style=_BS.PRIMARY if _BS else None)] for i in range(len(video_options))]
    rows.append([make_button("✅ ᴅᴏɴᴇ", callback_data=f"pfmcancel#{session_id}", style=_BS.PRIMARY if _BS else None)])
