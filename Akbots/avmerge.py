...
async def addaudio_cmd(client: Client, message: Message):
    ...
    if not video:
        return await message.reply_text(
            f"<blockquote>{E_WARN} Reply to a <b>ᴠɪᴅᴇᴏ</b> with <code>/addaudio</code> "
    ...
    await message.reply_text(
        f"<b>{E_MUS} Got the video.</b> Now send or reply with the <b>ᴀᴜᴅɪᴏ ғɪʟᴇ</b> "
...
async def addaudio_receive(client: Client, message: Message):
    ...
    try:
        ...
        await client.download_media(message, file_name=a_path,
                                     progress=make_download_progress(status, file_name=aname))
