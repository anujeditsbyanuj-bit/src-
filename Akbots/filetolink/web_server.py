...
from ..vcweb.page import routes as vc_routes
from ..hls_proxy_routes import routes as hls_routes
from ..v2hls_routes import routes as v2hls_routes
...
def build_app() -> web.Application:
    ...
    # speedtest_routes and vc_routes MUST be added before `routes` —
    # stream_routes.py ends with a catch-all `/{path:\S+}` handler that
    # would otherwise swallow `/speedtest/*` and `/vc` requests before
    # they ever reach speedtest_routes.py / vcweb/page.py.
    app.add_routes(speedtest_routes)
    app.add_routes(vc_routes)
    app.add_routes(hls_routes)
    app.add_routes(v2hls_routes)
