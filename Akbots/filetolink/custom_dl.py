...
from config import STREAM_BIN_CHANNEL


class ByteStreamer:
    """
    Streams a Telegram media file straight from Telegram's servers in
    range-request-friendly chunks, without ever touching local disk.

    Modified from
    <https://github.com/eyaadh/megadlbot_oss/blob/master/mega/telegram/utils/custom_download.py>
    (credit: Eyaadh), via the FILE-TO-LINK-BOT project.
    """
    ...
    async def generate_file_properties(self, id: int) -> FileId:
        file_id = await get_file_ids(self.client, STREAM_BIN_CHANNEL, id)
        if not file_id:
    ...
    async def clean_cache(self) -> None:
        while True:
            ...
            logging.debug("[filetolink] Cleaned ByteStreamer cache")
