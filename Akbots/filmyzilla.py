...
def _result_keyboard(session_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        make_button("⬇️ Download & send all", callback_data=f"fzdl#{session_id}", style=_BS.SUCCESS if _BS else None),
        make_button("❌ Dismiss", callback_data=f"fzcancel#{session_id}", style=_BS.DANGER if _BS else None),
    ]])
...
async def _resolve_single(session: aiohttp.ClientSession, base: str, url: str):
    ...
    if "/server/" in url:
        server_html = await _fetch_html(session, url)
        download_link = _find_download_link(server_html)
        if not download_link:
            raise ValueError(
    ...
    movie_html = await _fetch_html(session, url)
    server_link = _find_server_link(movie_html, base)
    if not server_link:
