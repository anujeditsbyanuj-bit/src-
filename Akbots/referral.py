...
async def _show_referred_users(target, user_id: int, page: int):
    ...
    if skip + PER_PAGE < total:
        nav.append(make_button("➡️ ɴᴇxᴛ", callback_data=f"reflist_page:{page+1}", style=_BS.SUCCESS if _BS else None))
...
async def _show_leaderboard(target, page: int):
    ...
    if (page + 1) * 10 < total:
        nav.append(make_button("➡️ ɴᴇxᴛ", callback_data=f"reflead:{page+1}", style=_BS.SUCCESS if _BS else None))
