...
def _referral_menu_buttons():
    return InlineKeyboardMarkup([
        [make_button("👥 ᴍʏ ʀᴇғᴇʀʀᴇᴅ ᴜsᴇʀs", callback_data="referral_view", style=_BS.PRIMARY if _BS else None)],
        [make_button("💰 ʀᴇᴅᴇᴇᴍ ʙᴜᴄᴋs", callback_data="referral_redeem", style=_BS.PRIMARY if _BS else None)],
        [make_button("🔄 ʀᴇғʀᴇsʜ", callback_data="referral_refresh", style=_BS.PRIMARY if _BS else None)],
    ])
...
async def _referral_text(user_id: int) -> str:
    ...
    return (
        f"<blockquote>{E_GIFT} <b>ʀᴇғᴇʀʀᴀʟ ᴘʀᴏɢʀᴀᴍ</b>\n\n"
        f"{E_COIN} <b>ʙᴜᴄᴋs ʙᴀʟᴀɴᴄᴇ:</b> <code>{info.get('ak_bucks', 0)}</code>\n"
        f"👥 <b>ᴛᴏᴛᴀʟ ʀᴇғᴇʀʀᴀʟs:</b> <code>{info.get('total_referrals', 0)}</code>\n\n"
        f"<b>ʜᴏᴡ ɪᴛ ᴡᴏʀᴋs:</b>\n"
        ...
        f"<b>ʀᴇᴅᴇᴇᴍ:</b> every <code>{BUCKS_PER_PREMIUM_DAY}</code> bucks = <b>1 ᴅᴀʏ ᴘʀᴇᴍɪᴜᴍ</b></blockquote>"
    )
...
async def referral_refresh_callback(client: Client, callback_query: CallbackQuery):
    ...
    try:
        await safe_edit(callback_query.edit_message_text, text, reply_markup=_referral_menu_buttons(), parse_mode=enums.ParseMode.HTML)
    except Exception:
        pass
...
async def referral_redeem_menu(client: Client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    info = await db.get_referral_info(user_id)
    bucks = info.get("ak_bucks", 0) if info else 0
    ...
    buttons = [[make_button(f"{E_CHECK} {d} ᴅᴀʏ(s) — {d * BUCKS_PER_PREMIUM_DAY} ʙᴜᴄᴋs",
                                      callback_data=f"referral_redeem_confirm:{d}", style=_BS.PRIMARY if _BS else None)] for d in options]
    buttons.append([make_button("↩ ʙᴀᴄᴋ", callback_data="referral_refresh", style=_BS.DANGER if _BS else None)])
...
async def _show_referred_users(target, user_id: int, page: int):
    ...
    if not referred:
        ...
        buttons = InlineKeyboardMarkup([[make_button("↩ ʙᴀᴄᴋ", callback_data="referral_refresh", style=_BS.DANGER if _BS else None)]])
        if isinstance(target, CallbackQuery):
            return await safe_edit(target.edit_message_text, text, reply_markup=buttons, parse_mode=enums.ParseMode.HTML)
    ...
    nav = []
    if page > 0:
        nav.append(make_button("⬅️ ᴘʀᴇᴠ", callback_data=f"reflist_page:{page-1}", style=_BS.PRIMARY if _BS else None))
    if skip + PER_PAGE < total:
        nav.append(make_button("➡️ ɴᴇxᴛ", callback_data=f"reflist_page:{page+1}", style=_BS.PRIMARY if _BS else None))
    rows = [nav] if nav else []
    rows.append([make_button("↩ ʙᴀᴄᴋ", callback_data="referral_refresh", style=_BS.DANGER if _BS else None)])
    buttons = InlineKeyboardMarkup(rows)

    if isinstance(target, CallbackQuery):
        await safe_edit(target.edit_message_text, text, reply_markup=buttons, parse_mode=enums.ParseMode.HTML)
    else:
...
async def _show_leaderboard(target, page: int):
    ...
    for i, u in enumerate(users, start=page * 10 + 1):
        ...
        lines.append(f"{i}. <b>{u.get('name', 'Unknown')}</b> (<code>{u.get('id')}</code>) — "
                      f"{ref.get('total_referrals', 0)} refs, {ref.get('ak_bucks', 0)} bucks")
    text = "\n".join(lines)

    nav = []
    if page > 0:
        nav.append(make_button("⬅️ ᴘʀᴇᴠ", callback_data=f"reflead:{page-1}", style=_BS.PRIMARY if _BS else None))
    if (page + 1) * 10 < total:
        nav.append(make_button("➡️ ɴᴇxᴛ", callback_data=f"reflead:{page+1}", style=_BS.PRIMARY if _BS else None))
